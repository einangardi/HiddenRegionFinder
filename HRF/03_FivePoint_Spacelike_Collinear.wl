...
(* Development-stage loading convention: keep HiddenRegionFinder.wl in the
   same directory as this example file/notebook. *)
...
Seed5ptScan = findObstructions[
  F0Seed5pt, VarsSeed5pt, KinAssump, KinVars, 20,
  "GeneratorMode" -> "SingleProduct",
  "UseExtendedFactors" -> False
];

(* ---------------------------------------------------------------------- *)
(* Graph-opening utilities: split four-point vertices                      *)
(* ---------------------------------------------------------------------- *)
...
addCollinearLeadingData[UFData_, kinAssumptions_, kinVars_] := Table[
  Module[{Fcol, Flead, obs},
    ...
    obs = hrfTimed["findObstructions interior graph " <> ToString[i], findObstructions[
      Flead, UFData[[i, "Variables"]], kinAssumptions, kinVars, 20,
      "GeneratorMode" -> "SingleProduct",
      "UseExtendedFactors" -> False
    ]];
    Join[UFData[[i]], <|
...
ClearAll[boundarySubsetsGeneric, restrictPolynomialToBoundaryGeneric,
  findObstructionsOnBoundaries, extractInterestingBoundaryCases,
...
findObstructionsOnBoundaries[F_, vars_, kinAssumptions_, kinVars_, maxCodim_ : 2] := Module[
  {strata, results = {}, i, res, obs},
  strata = boundarySubsetsGeneric[vars, maxCodim];
  For[i = 1, i <= Length[strata], i++,
    hrfReport["    boundary stratum ", i, "/", Length[strata], " zeroVars=", strata[[i]]];
    res = restrictPolynomialToBoundaryGeneric[F, vars, strata[[i]]];
    obs = If[res["FRestricted"] === 0 || res["RemainingVars"] === {},
      <||>,
      hrfTimed["findObstructions boundary stratum " <> ToString[i], findObstructions[
        res["FRestricted"], res["RemainingVars"], kinAssumptions, kinVars, 20,
        "GeneratorMode" -> "SingleProduct",
        "UseExtendedFactors" -> False
      ]]
    ];
...
BoundaryScan2Loop7Prop = hrfTimed["boundary obstruction scans for 7-propagator graphs", Table[
  ...
  <|
    ...
    "BoundaryScans" -> findObstructionsOnBoundaries[
      ...
      UFCollinear2Loop7Prop[[i, "Variables"]], KinAssump, KinVars, 2
    ]
  |>,
  {i, Length[UFCollinear2Loop7Prop]}
]];

BoundaryScan2Loop8Prop = hrfTimed["boundary obstruction scans for 8-propagator graphs", Table[
  ...
  Module[{graphIndex, scans, selected},
    ...
    selected = Select[scans,
      Module[{osc = Lookup[#, "ObstructionScan", <||>], od},
        od = If[AssociationQ[osc], Lookup[osc, "ObstructionData", <||>], <||>];
        AssociationQ[osc] &&
        KeyExistsQ[osc, "CancellationFactors"] &&
        Length[Lookup[osc, "CancellationFactors", {}]] >= 2 &&
        AssociationQ[od] &&
        KeyExistsQ[od, "Complement"] &&
        Lookup[od, "Complement", 0] =!= 0
      ] &
    ];
    Table[Join[<|
...
scalingForBoundaryCase[case_, UFCollinearData_] := Module[
  {graphIndex, obsData, obstruction, complement, U, vars, newScaling},
  graphIndex = case["GraphIndex"];
  obsData = case["ObstructionScan", "ObstructionData"];
  obstruction = obsData["Obstruction"];
  complement = obsData["Complement"];
  U = UFCollinearData[[graphIndex, "U"]] /. Thread[case["ZeroVars"] -> 0] // Expand;
  vars = case["RemainingVars"];
  newScaling = findCoverageLPScaling[complement, U, vars, 5, obstruction];
  <|
    "GraphIndex" -> graphIndex,
...
scalingForInteriorCase[data_] := Module[
  {scan, obsData, complement, obstruction, U, vars, newScaling},
...
coverageScalingDebugRow[row_] := Module[{data, acc, first},
  data = Lookup[row, "CoverageLPScalingData", <||>];
  ...
  <|
    ...
    "FirstAcceptedGap" -> Lookup[first, "HierarchyGapUminusFSL", Missing["NoAcceptedCandidate"]],
    ...
    "NearMisses" -> If[AssociationQ[data], Lookup[data, "NearMissDiagnostics", {}], {}]
  |>
];
...
acceptedCoverageQ[row_Association] := Module[{d = selectedCoverageData[row]},
  AssociationQ[d] && ! MatchQ[Lookup[d, "Scaling", Missing["NoScaling"]], _Missing]
];

selectedAcceptedDiagnostic[row_Association] := Module[{d = selectedCoverageData[row], acc},
  If[! AssociationQ[d], Return[<||>]];
  acc = Lookup[d, "AcceptedCandidateDiagnostics", {}];
  If[acc === {}, <||>, First[acc]]
];

regionScalingSummaryRow[row_Association, regionType_] := Module[{d, diag},
  d = selectedCoverageData[row];
  diag = selectedAcceptedDiagnostic[row];
  <|
    ...
    "HiddenRegionQ" -> acceptedCoverageQ[row],
    "ScalingVector" -> If[AssociationQ[d], Lookup[d, "Scaling", Missing["NoScaling"]], Missing["NoScalingData"]],
    "WSL" -> Lookup[diag, "FSLWeight", Missing["NoAcceptedCandidate"]],
    "WHR" -> Lookup[diag, "PostCancellationLeadingWeight", Missing["NoAcceptedCandidate"]],
    "GapWHRminusWSL" -> Lookup[diag, "HierarchyGapPostLPminusFSL", Missing["NoAcceptedCandidate"]],
    "CoveredByFSLAtWSL" -> Lookup[diag, "VariablesCoveredByFSLAtWSL", Missing["NoAcceptedCandidate"]],
    "CoveredAtWHR" -> Lookup[diag, "VariablesInPostCancellationLeadingSupport", Missing["NoAcceptedCandidate"]],
    "MissingVars" -> Lookup[diag, "VariablesMissingFromLeadingRegionCoverage", Missing["NoAcceptedCandidate"]]
  |>
];

regionScalingSummaryTable[interiorRows_, boundaryRows_] := Join[
  regionScalingSummaryRow[#, "Interior"] & /@ interiorRows,
  regionScalingSummaryRow[#, "Boundary"] & /@ boundaryRows
];
...
TopologyHiddenRegionRows2Loop7Prop = Select[TopologyRegionScalingSummary2Loop7Prop, TrueQ[#HiddenRegionQ] &];
TopologyHiddenRegionRows2Loop8Prop = Select[TopologyRegionScalingSummary2Loop8Prop, TrueQ[#HiddenRegionQ] &];
...
obstructionRegionPresentQ[scan_] := Module[{od},
  If[! AssociationQ[scan], Return[False]];
  od = Lookup[scan, "ObstructionData", <||>];
  AssociationQ[od] &&
  KeyExistsQ[od, "Superleading"] &&
  Lookup[od, "Superleading", 0] =!= 0
];
