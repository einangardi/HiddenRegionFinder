...
ClearAll[candidateGeneratorSets, candidateGeneratorSetsDiagnostic, candidateGeneratorSetsAdaptive,
  ...
  hrfCompletionFactorsFromQuotient];
...
candidateGeneratorSetsDiagnostic[factors_, maxGenerators_ : 2, vars_, kinAssumptions_, kinVars_, F_,
  ...
  admissiblePairs = Select[Subsets[ff, {2}],
    (* Support/kinematic-sector and degree tests are exact, cheap necessary
       conditions.  Apply them before the joint-domain FindInstance test so
       uncapped long derivative factors are rejected without an algebraic
       feasibility solve whenever they cannot have a multilinear partner. *)
    If[TrueQ[$HRFUseGeneratorPhysicsFilterQ],
      ...
      hrfGeneratorDegreeAdmissibleQ[Expand[Times @@ #], bounds, vars, F, kinVars] &&
      (skipPDFQ ||
        (Length[DownValues[hrfDisjointSquareFreePairPositiveQ]] > 0 &&
          hrfDisjointSquareFreePairPositiveQ[#, vars, kinVars]) ||
        simultaneouslyAdmissibleSubsetQ[#, vars, kinAssumptions, kinVars]) &
  ];
  ...
  pairGens = DeleteDuplicates[Expand /@ (Times @@@ admissiblePairs)];
  If[useSectorQuotientQ && TrueQ[$HRFUseGeneratorPhysicsFilterQ] &&
