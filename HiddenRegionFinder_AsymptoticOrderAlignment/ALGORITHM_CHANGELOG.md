...
- **Beyond binomial \(f_k\):** signed polynomial combinations (up to `$HRFPolynomialMaxMonomials`, default Automatic from raw harvest) from derivative factorization, whole-derivative polynomials, and signed monomial pairs.  The signed-pair derivative-length cap has been removed (the old public symbol is fixed to `Infinity` for display compatibility).  In the two-channel wide-angle case the supplementary pair harvest uses every primitive direction in the coefficient space, namely the coefficient polynomials `A`, `B`, `A+B`, and `A-B` for `F=s12 A+s23 B`.  This is consistent with the crossed Mandelstam bases already used by obstruction reconstruction. Same-channel pairs from Schwinger derivatives are exactly redundant after primitive normalization, while cross-channel pairs are kinematic-dependent and fail the mandatory kin-free generator gate.
...
- **Generator-independent face/pinch certificate:** for the exhaustive no-Crown audit, Eqs. (14) and (17) are imposed before choosing generators.  The exponent points of any admissible `F_SL` must form an exposed face of the `F0` Newton polytope.  `HRF_WideAngle16FacePinchAudit.wl` obtains that complete face lattice from exact rational `cddlib` incidences and tests Eq. (13) directly on every face in positive physical coordinates `s23=-a`, `s12=a+b`, `a,b>0`.  A sign-definite nonzero derivative rejects a face immediately; otherwise an exact positive `FindInstance` test decides the simultaneous pinch equations.  Every positive pinch is finally checked by the exact oriented `(rho;1)` hierarchy LP against `F0-F_SL`, `U`, and every restored-delta layer; this is precisely the test that the `F0` face extends to the required pure lower face of the augmented polytope.  The two-stage construction is exact and much smaller than enumerating every augmented face: a representative thirteen-propagator stratum has 156,217 augmented faces but only 1,207 `F0` faces.  The route assumes neither a factor-length cap nor a generator presentation and finds exactly one positive pinch-and-hierarchy face for the Crown control, with gap 1.
