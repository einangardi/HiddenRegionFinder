...
(* Signed-monomial-pair harvesting is part of the completeness envelope.  The
   historical value 8 silently omitted candidates from longer derivatives.
   The cap has been removed, rather than retained as a run-time option.  Keep
   the public symbol at Infinity only for old notebooks that display it. *)
