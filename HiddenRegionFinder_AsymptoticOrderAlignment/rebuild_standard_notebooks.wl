...
hrfNbStdExport[
  ...
  "Spacelike collinear 5-point examples",
  {
    ...
    hrfNbStdSection["Current preselected 5-point scan results"],
    hrfNbStdText["Collaborator-facing summary of the current polynomial HRF scan. These cells read the saved outputs from HRF_RunPreselected5ptScan.wl; they do not run HRF. Boundary ZeroVars are diagnostic strata where HRF found a hidden region. The graph catalogue deliberately draws the original topologies, because those are the objects passed onward to HRF."],
    hrfNbStdInput[
      "$HRFQuietReports = True;\nGet[FileNameJoin[{NotebookDirectory[], \"HRF_Example03PreselectedResults.wl\"}]];",
      "In[6]:="
    ],
    hrfNbStdInput["hrfEx03PreselectedRunInstructions[]", "In[7]:="],
    hrfNbStdInput["hrfEx03PreselectedScanSummary[]", "In[8]:="],
    hrfNbStdText["All topologies with at least one accepted interior or boundary HR stratum. The internal labels x0,x1,... are the Feynman parameters in the scaling vectors; external leg p_i emerges from vertex i."],
    hrfNbStdInput["hrfEx03PreselectedHRGraphCatalogue[]", "In[9]:="],
    hrfNbStdText["Combined view: each graph drawing, the internal x_i definitions, and the accepted HR strata/scaling vectors for that same topology in one row."],
    hrfNbStdInput["hrfEx03PreselectedHRTopologyPanels[]", "In[9a]:="],
    hrfNbStdText["Interior HR strata and scaling vectors. Current result: 4 interior HR strata: the seed, G_tbullet, G_bullett, and G_tt."],
    hrfNbStdInput["hrfEx03PreselectedInteriorHRTable[]", "In[10]:="],
    hrfNbStdText["Boundary HR strata and scaling vectors. Current result: 21 boundary HR strata through codimension 2; all accepted SL-sector factors are binomial."],
    hrfNbStdInput["hrfEx03PreselectedBoundaryHRTable[]", "In[11]:="],
    hrfNbStdInput["hrfEx03PreselectedAllHRTable[]", "In[12]:="],
    hrfNbStdSection["Regenerate the current preselected scan outputs"],
    hrfNbStdText["Optional. Run from a fresh kernel if the saved CSV/WL files need to be regenerated. Observed wall time on this machine: the 70-topology interior scan took a few minutes; the codimension-1/2 boundary scan took about 8 minutes. The scans overwrite Preselected5ptHRFScan_interior* and Preselected5ptHRFScan_boundary_codim2* outputs."],
    hrfNbStdInput[
    ...
    hrfNbStdSection["Legacy native 7/8-propagator scan (kept for comparison)"],
    hrfNbStdText["Optional and slow (~1-3 h in the older native route). The newer preselected scan above is the current source of truth for the 70-topology handoff and for the Example 03 interior/boundary HR tables. This legacy route reloads the original Example 03 driver with $HRFEx03RunFullTopologyScanQ=True."],
    hrfNbStdInput[
    ...
    hrfNbStdText["Requires the legacy full load above. These tables are independent of the current preselected-scan tables."],
    hrfNbStdInput["TopologyHiddenRegionTable2Loop7Prop", "In[16]:="],
    hrfNbStdInput["TopologyRegionScalingTable2Loop7Prop", "In[17]:="],
    hrfNbStdInput["TopologyScalingCoverageTable2Loop7Prop", "In[18]:="],
    hrfNbStdSection["Eight-propagator summary tables (optional)"],
    hrfNbStdInput["TopologyHiddenRegionTable2Loop8Prop", "In[19]:="],
    hrfNbStdInput["TopologyRegionScalingTable2Loop8Prop", "In[20]:="],
    hrfNbStdInput["TopologyScalingCoverageTable2Loop8Prop", "In[21]:="],
    ...
    hrfNbStdText["Requires In[23] load of 04_PolynomialFactor_Regression.wl for full obstruction runs below."],
    hrfNbStdInput[
    ...
    hrfNbStdInput["hrfEx04FivePointFactorDiffTable[]", "In[24]:="],
    ...
    hrfNbStdInput["hrfSeed5ptReport[\"AllValidTrialsScaling\"]", "In[25b]:="],
    hrfNbStdInput["Ex04Seed5ptInterior[\"GeneratorTrialTable\"][\"Polynomial\"]", "In[26]:="],
    ...
    hrfNbStdText["ThreeLoopVertex interior obstruction via Ex04 harness (slow). Requires In[23]. After fix: UseExtendedFactors=False for Collinear5pt."],
    hrfNbStdInput[
    ...
    hrfNbStdInput["hrfVertexReport[\"AllValidTrialsScaling\"]", "In[28b]:="],
    hrfNbStdInput[
