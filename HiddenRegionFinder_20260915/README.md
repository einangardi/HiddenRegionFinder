...
## 15 September 2026 consolidation

This directory is the current shareable HRF bundle.  It consolidates the
ordinary HRF implementation, asymptotic-order alignment, source-aware
dissection, the numbered physics notebooks and their compact exact results.
The top-level notebooks load the Wolfram Language sources from the same
directory.  Specialised notebooks under `examples/` keep their supporting
code and compact results in the same example directory and reach the core
only through relative paths.

The main additions since the July release are the exhaustive two-loop NMRK
Part-II/dissection audits in notebooks 09--12, the six-point wide-angle
self-crossing example, and exact standalone audits of the `K_{3,3}` and
`K_{4,3}` bipartite topologies.  The current JHEP manuscript and bibliography
are included under `docs/`.

## 21 August 2026 source-aware facet certification

The final alignment audit no longer accepts a candidate by placing the
`F_SL` monomials at `W_HR` by hand.  The original-coordinate ideal-jet
analysis efficiently determines a candidate vector whenever possible, but
HRF now always maps independent cancellation factors to local endpoint
variables, reconstructs the complete LP polynomial with source provenance,
enumerates exact lower facets with `cddlib`, and pulls each admissible normal
back to the original variables.  This source-resolved dissection is the
ultimate certificate.  Its output records whether the first resolved
`F_SL` jet lies on the leading facet or is genuinely subleading.

The saved-assumption audit recertifies all six earlier accepted examples that
had relied on synthetic `F_SL` promotion (one one-loop and five two-loop NMRK
representatives).  It finds 17 exact local facets; `F_SL` is leading in all
17 and subleading in none.  A separate synthetic regression deliberately
contains a deeper cancellation and is correctly classified with
`ResolvedFSLWeight > ResolvedLeadingWeight`.

## 31 July 2026 consolidation

This update sharpens the boundary between candidate discovery and pinch
validation.  Before a decomposition is known, derivative harvesting inspects
the derivatives of the starting polynomial but does not impose their common
vanishing.  In particular, the full gradient ideal must not be saturated when
the starting polynomial may still contain an obstruction.  Saturation is
available only for an already selected leading cancellation sector, or for
explicitly supplied derivative-sector relations.  An unjustified request is
reported as `"ApplicabilityNotEstablished"` and makes the scan incomplete.

The five-point near-planar audit is the positive control for this interface:
on `Gamma5 == 0` its complete leading polynomial is the cancellation sector,
so the candidate-specific saturated ideal is legitimate.  The accompanying
paper and notebook now state the local dissection coordinates explicitly and
separate the original-coordinate parameter-space power count from its
independent momentum-space check.

## Start here

The principal physics notebooks are:
...
| Five-point central-soft rapidity-ordered limit | `08_Current_5pt_MRK_CentralSoft.nb` | A composite rapidity-ordered limit, distinct from standard fixed-transverse MRK; one full-layer-certified HR occurs for exactly the six attachments with the soft gluon on a four-valent vertex. Parameter- and momentum-space power counting agree: `delta^(4 D-20)` |
| Five-point near-planar wide-angle scattering | `examples/five_point_wide_angle/FivePoint_WideAngle_Landshoff_AllLayers.nb` | Generic non-coplanar wide-angle kinematics has no positive pinch for this topology; approaching `Gamma5=0` produces a three-generator HR with vector `(-2,-2,-2,-2,-2,-2;1)` and matching parameter- and momentum-space power `lambda^(-1-4 epsilon)` |
| Five-point simultaneous MRK and near-planarity | `examples/five_point_mrk_planar/FivePoint_MRK_Planar_Composite_Limit.nb` | Fixed-transverse standard MRK combined with `w-wbar -> 0` has a certified three-normal HR for the endpoint-`p4` attachment.  At equal rates its vector is `(-2,-2,-1,-2,-2,-2;1)`, with two cancellation layers `-7,-6 -> -4`; the rapidity-symmetric `p3,p5`-endpoint attachment fails first-sheet positivity |
...
| Central NMRK at zero transverse recoil (`w=z`, `wb=zb` in the input chart) | `06_NMRK_wz_AsymptoticOrderAlignment_Checks.nb` | One-loop and two-loop hidden-region certificates after asymptotic-order alignment |
...
| Two-loop zero-recoil NMRK Part-II/dissection analysis | `09_TwoLoop_NMRK_wz_PartII_Dissection_Audit.nb`--`12_TwoLoop_NMRK_wz_Table8_Comparison_Audit.nb` | Exhaustive source-aware classification of the planar hexagon--box and the two externally labelled hexagon--pentagon cases, including physical-HR deduplication and the relation between interior and codimension-one regions |
| Six-point wide-angle self-crossing example | `examples/six_point_wide_angle_self_crossing/SixPoint_WideAngle_SelfCrossing_HR_Audit.nb` | Exact construction of the wide-angle `K_{4,2}`-type Landshoff HR and its local cancellation structure |
...
The current paper draft and its self-contained JHEP sources are included as
`docs/Hidden_Region_Finder_Principles_JHEP.tex` and
`docs/Hidden_Region_Finder_Principles_JHEP.pdf`.

Two additional exact topology checks are executable directly from the release
root.  `K33_TripleCancellation_Audit.wl` finds cubic cancellation but excludes
the displayed cyclic family from the positive LP domain.
`K43_Landshoff_HRF_Audit.wl` certifies the six-loop wide-angle `K_{4,3}` HR,
including its six normal coordinates and scaleful local polynomial.
`SoftDoubleCollinearResolvedMapCheck.wl` records the exact relation between
the single- and double-collinear cross-ratio paths used in the degeneration
discussion.
...
`"SourceAwareDissectionOptions"` inside
`"AsymptoticOrderAlignmentOptions"` forwards explicit pivot, solve-branch,
sign-sector, point-count, and time budgets to the final dissection.  Reaching
one of these limits is recorded as a truncated, unresolved certification
search; it is not converted into a no-HR result.

The ordinary, non-aligned path now also uses source-aware dissection as its
final certificate.  The coverage equations first propose a scaling; HRF then
dissects the selected cancellation locus, finds exact lower facets of the
complete source-separated LP polynomial and pulls their normals back to the
original LP variables.  The provisional result remains in
`"CoverageScalingData"`, while `"CertifiedScalingData"` is authoritative.
`"OrdinaryDissectionMode" -> "Required"` is the default;
`"OrdinaryDissectionOptions"` forwards the same pivot, sign-sector,
point-count and time budgets.  A complete search with no facet rejects the
candidate, whereas a capped, timed-out or unavailable negative search is
reported through `"CertificationUnresolvedQ"` and is not treated as a
no-HR result.  An exact positive certificate remains valid even if the
remaining chart enumeration was capped; in that case
`"DissectionSearchIncompleteQ"` records that the list of regions is not
known to be exhaustive.  `"OrdinaryDissectionMode" -> "Off"` retains the
former coverage-only behaviour for comparisons.

When a singular-locus analysis supplies a generator ideal that is more
general than the automatic constructor, pass it explicitly without bypassing
the remaining HRF checks:

```wl
findObstructions[
  F0, vars, assumptions, kinVars, Automatic,
  "CancellationFactorOverride" -> {fA, fB, fC},
  "GeneratorSetOverride" -> {{fA fB, fB fC, fC fA}},
  "MaxGenerators" -> 3,
  "U" -> U,
  "FObsForScaling" -> <|
    "DeltaLayers" -> <|2 -> Fperp|>
...
The outer list permits several proposed generator sets. HRF still applies
the positive-domain, degree, exact ideal-membership, obstruction, scaling,
and scalefulness tests to every supplied set. The override is therefore a
semi-automatic discovery interface, not an acceptance override. Its use and
cardinality are recorded in `"GeneratorConstructionAudit"` and
`"EffectiveSearchConfiguration"`.

Saturation of derivative relations is available only after the relevant
leading cancellation sector has been selected. It is not a general fallback
on the full gradient of a starting polynomial that may still contain an
obstruction: obstruction derivatives need not vanish on the HR locus.

For the five-point near-planar example, the expansion on `gram5 == 0` has the
special property that the complete supplied leading polynomial is already the
cancellation sector. Its candidate-specific saturated ideal can therefore be
enabled explicitly:
...
findObstructions[
  ...
  "FactorHarvestKinematicConstraints" -> {gram5 == 0},
  "FactorHarvestNonzeroKinematicFactors" -> {qAB, qBC, qCA},
  "GeneratorMode" -> "PairSectors",
  "MaxGenerators" -> 3,
  "U" -> U,
  "FObsForScaling" -> <|"DeltaLayers" -> <|2 -> Fperp|>|>
]
```

This opt-in route saturates the gradient ideal of the supplied selected
leading sector by the active positive Schwinger parameters and the explicitly
nonzero kinematic factors. Alternatively,
`"FactorHarvestDerivativePolynomials"` can supply selected derivative-sector
relations directly; in that case no claim about the full supplied gradient is
made. The resulting binomial factor classes are added to the ordinary factor
pool; they are not supplied as generators and do not accept a region. The
usual positive-domain, generator, decomposition, scaling and lower-facet tests
remain active. The complete audit is returned as
`"GeneralizedDerivativeHarvestAudit"`. Without either an explicit selected
derivative list or `"FullGradientSaturationJustifiedQ" -> True`, the run is
marked `"ApplicabilityNotEstablished"` and incomplete. The historical names
`"DirectThenSaturated"`, `"SaturatedIdeal"`, and
`"SaturatedDerivativeIdeal"` remain accepted as deprecated aliases, subject
to the same applicability check.
`"FactorHarvestTimeLimit"`, `"FactorHarvestMaxBasisSize"`, and
`"FactorHarvestMaxXDegree"` are explicit user budgets; reaching a time or
basis limit sets `"SearchTruncatedQ" -> True`. The mode is off by default.

In a constrained Mandelstam chart, clearing denominators can make two
legitimate normal factors both look nonlinear in the kinematics. In the
saturated mode HRF treats nonzero kinematic prefactors as units: downstream
polynomial division is performed in the Schwinger variables over the field of
kinematic rational functions, and the supplied kinematic equations certify
which remainder vanishes on the leading surface. Consequently
`"AllowKinematicFactorPairsQ" -> Automatic` enables these pairs only for this
opt-in route and remains false for ordinary runs. No numerical witness or
independent reparametrization is required. `"EffectiveSearchConfiguration"`
records `"ObstructionIdealMode" -> "KinematicCoefficientField"` when this
route is active.

The symbolic five-point near-planar audit is implemented in
`examples/five_point_wide_angle/HRF_FivePointNearPlanarAlgorithmAudit.wl` and
displayed in `examples/five_point_wide_angle/FivePoint_WideAngle_Landshoff_AllLayers.nb`.
It recovers three normal factors, the unique surviving set of three pairwise
generators, and the vector `(-2,-2,-2,-2,-2,-2)` without factor, generator or
numerical-kinematics overrides.
...
ideal, and, where necessary, a source-aware local dissection.  The dissection
keeps `F_SL`, the obstruction, all other `F` layers, and `U` separate until
their coefficients have been recombined in local coordinates.  Exact lower
facets are then enumerated and pulled back to the original LP variables.  In
particular, the resolved weight of `F_SL` is measured rather than imposed.
...
HRF_SourceAwareDissectionCertification.wl    source-resolved dissection, exact facets and pullback
HRF_LayeredDissectionCertification.wl        legacy comparison diagnostic
HRF_SourceAwareDissectionRegressionTests.wl  focused equality/deep-cancellation controls
HRF_SourceAwareSavedAssumptionAudit.wl        reproducible audit of formerly promoted examples
...
examples/five_point_mrk_planar/               simultaneous standard-MRK plus planarity audit
...
examples/five_point_wide_angle/HRF_FivePointNearPlanarAlgorithmAudit.wl
                                                candidate-specific saturated-leading-ideal audit
examples/five_point_wide_angle/FivePoint_WideAngle_Landshoff_AllLayers.nb
                                                near-planar and central-soft comparison notebook
...
docs/                                        current JHEP paper draft and build inputs
```
...
### Five-point central-soft rapidity-ordered limit

`08_Current_5pt_MRK_CentralSoft.nb` records the composite limit in which the
central emitted momentum becomes soft while a rapidity hierarchy is imposed.
It is not standard fixed-transverse MRK: making the emission dimensionfully
soft changes the relation between the transverse and rapidity scalings.  The
full native-layer audit finds one certified HR
for exactly six of the twelve inequivalent attachment labelings: those in
which the central gluon is emitted from the four-valent vertex.  For a
representative labeling the alignment vector is
`{-3,-3,0,-3,-3,-3}`, the relative HRF vector is uniform
`{-1,-1,-1,-1,-1,-1}`, and the composed original-coordinate vector is
`{-4,-4,-1,-4,-4,-4;1}`, with `(W_SL,W_HR)=(-9,-8)`.  The former momentum
widths and power count used a stripped native delta weight and are therefore
not retained; a fresh momentum-space reconstruction is still required.
Reproducible scripts and compact results are in
`examples/five_point_mrk_central_soft/`.

### Five-point simultaneous MRK and near-planarity

`examples/five_point_mrk_planar/FivePoint_MRK_Planar_Composite_Limit.nb`
studies the separate composite path in which both standard fixed-transverse
MRK gaps grow while `w-wbar=i delta^a` approaches the coplanar boundary.  The
positive stationary locus moves as `(rho_A,rho_B,rho_C)~(1,delta^b,1)`, and
the local lower facet pulls back to
`(-2a,-2a,b-2a,-2a,-2a,-2a;1)`.  For equal rates this is
`(-2,-2,-1,-2,-2,-2;1)`.  The exact seven-monomial local polynomial has
affine rank six and exhibits two cancellation layers, `-7,-6 -> -4`.
The twelve-order attachment audit finds only the endpoint-reflected pair
`{1,2,3,5,4}` and `{1,2,4,5,3}` positive.  The rapidity-symmetric order
`{1,2,3,4,5}`, with `p3,p5` at the four-valent endpoints and `p4` at a
trivalent path vertex, has two negative stationary ratios and no first-sheet
pinch on this path.

### NMRK with `w=z`

The one-loop hexagon has one structurally unique certified HR.  At two loops,
the planar hexbox has 7 certified vectors (4 full support and 3 boundary),
...
The release records the following verification outcomes:
...
| Planar hexagon--box Part-II candidates | 13/13 completely dissected |
| Same-path hexagon--pentagon Part-II candidates | 8/8 completely decided |
| Separate-path hexagon--pentagon staged candidates | 0 |
| `K_{3,3}`, `K_{4,3}` and collinear-map exact audits | 3/3 |

The Crown check in the derivative-harvest suite is a negative control for
premature kinematic specialisation, not a failure of the Crown construction.
It substitutes `s12=1, s23=-1` before harvesting and therefore mixes the two
independent kinematic sectors.  The dedicated Crown construction instead
separates the derivative components multiplying `s12` and `s23`; their
factorisation exposes the four cancellation factors and the two product
generators that certify the HR.

The exact commands, scope, and packaging checks for this dated release are
recorded in `RELEASE_CHECKS_20260915.md`.
...
wolframscript -file HRF_GeneratorSetOverrideTests.wl
wolframscript -file HRF_GeneralizedDerivativeHarvestTests.wl
...
wolframscript -file HRF_PartIIDissectionRecordTests.wl
wolframscript -file K33_TripleCancellation_Audit.wl
wolframscript -file K43_Landshoff_HRF_Audit.wl
wolframscript -file SoftDoubleCollinearResolvedMapCheck.wl
```

`HRF_NotebookSmokeTests.wl` is retained as a historical discovery-stage
cross-notebook check.  It predates required source-aware dissection and is not
a release gate.  The current physics certificates are covered by the focused
suites and evaluated notebooks listed above.  In particular, an incomplete
generic chart search is reported as unresolved and must not be interpreted as
the absence of the dedicated Crown or five-point certificates.
