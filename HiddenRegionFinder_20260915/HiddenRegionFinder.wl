...
(* Exact enumeration accelerator: disjoint x-support is a necessary condition
   for every massless pair generator accepted by the physics filter. *)
If[! ValueQ[$HRFUseIndexedDisjointSupportPairEnumerationQ],
  $HRFUseIndexedDisjointSupportPairEnumerationQ = True];
If[! ValueQ[$HRFFindObstructionsStopOnFirstAdmissibleQ], $HRFFindObstructionsStopOnFirstAdmissibleQ = False];
If[! ValueQ[$HRFStoreChannelObstructionAttemptsQ], $HRFStoreChannelObstructionAttemptsQ = True];
...
If[! ValueQ[$HRFPackageDirectory],
  $HRFPackageDirectory = Module[{inp, cwd},
    cwd = Directory[];
    inp = Quiet @ Check[$InputFileName, ""];
    Which[
      FileExistsQ[FileNameJoin[{cwd, "HiddenRegionFinder.wl"}]], cwd,
      StringQ[inp] && inp =!= "" && FileExistsQ[inp], DirectoryName[ExpandFileName[inp]],
      StringQ[inp] && inp =!= "" && DirectoryQ[inp], ExpandFileName[inp],
      True, Directory[]
    ]
  ]
];
hrfPackageDirectory[] := Module[{nb, inp},
  If[StringQ[$HRFPackageDirectory] && $HRFPackageDirectory =!= "" &&
      DirectoryQ[$HRFPackageDirectory],
    Return[$HRFPackageDirectory]
  ];
  If[FileExistsQ[FileNameJoin[{Directory[], "HiddenRegionFinder.wl"}]], Return[Directory[]]];
  inp = Quiet @ Check[$InputFileName, ""];
  If[StringQ[inp] && inp =!= "" && FileExistsQ[inp], Return[DirectoryName[ExpandFileName[inp]]]];
  If[StringQ[inp] && inp =!= "" && DirectoryQ[inp], Return[ExpandFileName[inp]]];
...
ClearAll[monomialQ, binomialQ, derivativeFactors,
  ...
  hrfNormalizePolynomialConstraint,
  hrfCancellationFactorsEquivalentModuloConstraintsQ,
  hrfDeleteCancellationFactorsEquivalentModuloConstraints,
  hrfSaturatedDerivativeFactorHarvest,
...
(* Generalized derivative-factor harvesting.

   Factoring each derivative separately is basis dependent: a collection of
   derivatives may generate an interior Landau ideal whose primitive normal
   factors occur only after polynomial combinations of the derivatives.  This
   opt-in harvester saturates the derivative ideal by the product of the
   active Schwinger variables, optionally together with explicitly nonzero
   kinematic factors, and extracts mixed-x factors from the resulting
   elimination basis.  Kinematic constraints (for example a Gram equation)
   are included as polynomial generators.

   The routine is deliberately separate from acceptance.  Its output still
   passes through the usual positive-domain, generator, obstruction, scaling
   and lower-facet tests. *)
hrfNormalizePolynomialConstraint[c_] := Which[
  Head[Unevaluated[c]] === Equal, Expand[c[[1]] - c[[2]]],
  TrueQ[c === True], 0,
  TrueQ[c === False], 1,
  True, Expand[c]
];

(* Two binomial factors can be distinct as polynomials in an overcomplete
   invariant chart but define the same cancellation hypersurface on the
   kinematic constraint variety.  Compare their two x-monomial coefficient
   ratios without dividing: a1 b2-a2 b1 must lie in the kinematic ideal. *)
hrfCancellationFactorsEquivalentModuloConstraintsQ[
    f_, g_, vars_List, kinVars_List, constraintPolys_List] := Module[
  {rf, rg, cross, remainder},
  rf = Quiet @ Check[SortBy[CoefficientRules[Expand[f], vars], First], {}];
  rg = Quiet @ Check[SortBy[CoefficientRules[Expand[g], vars], First], {}];
  If[Length[rf] =!= 2 || Length[rg] =!= 2 ||
      (First /@ rf) =!= (First /@ rg), Return[False]];
  cross = Expand[Last[rf[[1]]] Last[rg[[2]]] -
    Last[rf[[2]]] Last[rg[[1]]]];
  If[cross === 0, Return[True]];
  If[constraintPolys === {} || kinVars === {}, Return[False]];
  remainder = Quiet @ Check[
    Last @ PolynomialReduce[cross, constraintPolys, kinVars],
    cross
  ];
  TrueQ[Expand[remainder] === 0]
];

hrfDeleteCancellationFactorsEquivalentModuloConstraints[
    factors_List, vars_List, kinVars_List, constraintPolys_List] :=
  Fold[
    Function[{kept, candidate},
      If[AnyTrue[kept,
          Function[existing,
            hrfCancellationFactorsEquivalentModuloConstraintsQ[
              candidate, existing, vars, kinVars, constraintPolys]]],
        kept, Append[kept, candidate]]],
    {}, factors
  ];

hrfSaturatedDerivativeFactorHarvest[
    F_, vars_List, kinVars_List, constraints_: {},
    nonzeroFactors_: {}, timeLimit_: 30, maxBasisSize_: Automatic,
    derivativePolynomials_: Automatic] := Module[
  {gradient, constraintPolys, saturationProduct, tau, allVars, basis,
   eliminated, xPolys, rawFactors, candidates,
   truncatedQ = False,
   status = "Done", canonical},
  gradient = If[derivativePolynomials === Automatic,
    DeleteCases[Expand[D[F, #]] & /@ vars, 0],
    DeleteCases[Expand /@ Flatten[{derivativePolynomials}], 0]
  ];
  constraintPolys = DeleteCases[
    hrfNormalizePolynomialConstraint /@ Flatten[{constraints}], 0
  ];
  saturationProduct = Expand[
    Times @@ vars Times @@ DeleteCases[Flatten[{nonzeroFactors}], 0 | 1 | -1]
  ];
  tau = Unique["hrfSaturation$"];
  allVars = DeleteDuplicates @ Join[{tau}, vars, kinVars];
  basis = TimeConstrained[
    Quiet @ Check[
      GroebnerBasis[
        Join[gradient, constraintPolys, {1 - tau saturationProduct}],
        allVars, MonomialOrder -> EliminationOrder
      ],
      $Failed
    ],
    timeLimit,
  ...
  If[basis === $Aborted,
    Return[<|
      "Status" -> "TimeLimit", "Factors" -> {}, "BasisSize" -> Missing["TimedOut"],
      "SearchTruncatedQ" -> True, "TimeLimit" -> timeLimit
    |>]
  ];
  If[basis === $Failed || ! ListQ[basis],
    Return[<|
      "Status" -> "Failed", "Factors" -> {}, "BasisSize" -> Missing["Failed"],
      "SearchTruncatedQ" -> True, "TimeLimit" -> timeLimit
    |>]
  ];
  If[IntegerQ[maxBasisSize] && maxBasisSize >= 0 && Length[basis] > maxBasisSize,
    basis = Take[basis, maxBasisSize];
    truncatedQ = True;
    status = "BasisLimit"
  ];
  eliminated = Select[basis, FreeQ[#, tau] &];
  xPolys = Select[eliminated,
    ! FreeQ[#, Alternatives @@ vars] && ! monomialQ[#, vars] &
  ];
  rawFactors = Flatten[
    Quiet @ Check[FactorList[Factor[#]][[All, 1]], {}] & /@ xPolys
  ];
  rawFactors = Select[rawFactors,
    # =!= 1 && # =!= -1 && ! monomialQ[#, vars] && binomialQ[#, vars] &
  ];
  rawFactors = DeleteDuplicatesBy[rawFactors, Expand];
  canonical = If[Length[DownValues[hrfCanonicalCancellationFactor]] > 0,
    hrfCanonicalCancellationFactor[#, vars, kinVars] & /@ rawFactors,
    Factor /@ rawFactors
  ];
  candidates = DeleteDuplicatesBy[
    Select[canonical, # =!= 0 && binomialQ[#, vars] &],
    Expand
  ];
  candidates = hrfDeleteCancellationFactorsEquivalentModuloConstraints[
    candidates, vars, kinVars, constraintPolys
  ];
  <|
    "Status" -> status,
    "Factors" -> candidates,
    "BasisSize" -> Length[basis],
    "EliminatedPolynomialCount" -> Length[eliminated],
    "XPolynomialCount" -> Length[xPolys],
    "RawFactorCount" -> Length[rawFactors],
    "AcceptedStructuralFactorCount" -> Length[candidates],
    "DerivativeRelationSource" -> If[derivativePolynomials === Automatic,
      "FullGradientOfSuppliedPolynomial", "SelectedDerivativeRelations"],
    "DerivativeRelationCount" -> Length[gradient],
    "SearchTruncatedQ" -> truncatedQ,
    "TimeLimit" -> timeLimit,
    "MaxBasisSize" -> maxBasisSize
  |>
];

(* In a candidate-specific saturated derivative calculation the kinematic
   variables are parameters,
   not additional coordinates of the cancellation ideal.  Clearing their
   denominators can make a primitive normal factor appear polynomially
   nonlinear in the Mandelstams.  For the subsequent ideal test we therefore
   work over the coefficient field of kinematic rational functions and use
   the supplied kinematic equations only to decide which remainder vanishes
   on the leading kinematic surface. *)
hrfKinematicConstraintGroebnerBasis[constraints_, kinVars_List] := Module[
  {polys},
  polys = DeleteCases[
    hrfNormalizePolynomialConstraint /@ Flatten[{constraints}], 0
  ];
  If[polys === {} || kinVars === {}, {},
    Quiet @ Check[GroebnerBasis[polys, kinVars], polys]
  ]
];

hrfReduceKinematicCoefficientModuloConstraints[
    coefficient_, constraintBasis_List, kinVars_List] := Module[
  {rat, num, den, rem},
  If[constraintBasis === {} || kinVars === {}, Return[Together[coefficient]]];
  rat = Together[coefficient];
  num = Numerator[rat];
  den = Denominator[rat];
  rem = Quiet @ Check[
    Last @ PolynomialReduce[num, constraintBasis, kinVars],
    num
  ];
  Together[rem/den]
];

hrfReducePolynomialKinematicCoefficients[
    polynomial_, vars_List, constraintBasis_List, kinVars_List] := Module[
  {rules},
  rules = Quiet @ Check[CoefficientRules[Expand[polynomial], vars], {}];
  If[rules === {}, Return[Expand[polynomial]]];
  Together @ Total[
    Map[
      Function[rule,
        hrfReduceKinematicCoefficientModuloConstraints[
          Last[rule], constraintBasis, kinVars
        ] Times @@ MapThread[Power, {vars, First[rule]}]
      ],
      rules
    ]
  ]
...
hrfCoefficientFieldPolynomialReduce[
    polynomial_, generators_List, vars_List,
    constraintBasis_List : {}, kinVars_List : {}] := Module[
  {quotients, rawRemainder, constrainedRemainder},
  {quotients, rawRemainder} = PolynomialReduce[
    Together[polynomial], Together /@ generators, vars
  ];
  constrainedRemainder = hrfReducePolynomialKinematicCoefficients[
    rawRemainder, vars, constraintBasis, kinVars
  ];
  <|
    "Quotients" -> quotients,
    "RawRemainder" -> Together[rawRemainder],
    "ConstraintReducedRemainder" -> Together[constrainedRemainder],
    "ConstraintVanishingRemainderQ" ->
      TrueQ[Together[constrainedRemainder] === 0]
  |>
];
...
hrfKinDomainCompatibleQ[C_, vars_, kinAssumptions_, kinVars_] := Module[{inst, allVars,
  ...
  If[inst === $Aborted,
    If[! ValueQ[$HRFKinDomainUnresolvedCount], $HRFKinDomainUnresolvedCount = 0];
    $HRFKinDomainUnresolvedCount++;
    Return[False]
  ];
...
simultaneouslyAdmissibleSubsetQ[factors_List, vars_, kinAssumptions_, kinVars_] := Module[
  ...
  Which[
    ...
    True,
    allVars = DeleteDuplicates @ Join[kinVars, vars];
    With[{timeLimit = If[ValueQ[$HRFKinDomainFindInstanceTimeLimit],
        $HRFKinDomainFindInstanceTimeLimit, 5]},
      inst = If[timeLimit === Infinity,
        Quiet @ FindInstance[
          kinAssumptions && And @@ Thread[vars > 0] &&
            And @@ (Expand[#] == 0 & /@ ff), allVars, Reals],
        TimeConstrained[
          Quiet @ FindInstance[
            kinAssumptions && And @@ Thread[vars > 0] &&
              And @@ (Expand[#] == 0 & /@ ff), allVars, Reals],
...
(* Exact fast certificate used repeatedly during generator scans.  Kin-free,
   square-free homogeneous mixed-sign factors with pairwise-disjoint x support
   can be set to zero independently in the positive orthant; no FindInstance
   call is needed. *)
hrfSimultaneouslyAdmissibleSubsetQFast[factors_List, vars_, kinAssumptions_, kinVars_] := Module[
  {ff = DeleteDuplicates[Flatten[{factors}]], supports},
  If[ff === {}, Return[True]];
  If[Length[DownValues[hrfSquareFreeHomogeneousPositiveZeroQ]] > 0 &&
      (kinVars === {} || And @@ (FreeQ[#, Alternatives @@ kinVars] & /@ ff)) &&
      And @@ (hrfSquareFreeHomogeneousPositiveZeroQ[#, vars, kinVars] & /@ ff),
    supports = hrfFactorXSupport[#, vars] & /@ ff;
    If[DuplicateFreeQ[Flatten[supports]], Return[True]]
  ];
  simultaneouslyAdmissibleSubsetQ[ff, vars, kinAssumptions, kinVars]
];
...
ClearAll[hrfObstructionRemainderCoefficients, hrfObstructionTermCount,
  ...
  hrfObstructionAlgebraicSearch, hrfObstructionAlgebraicSearchBinaryLinearIdeal,
  hrfObstructionAlgebraicSearchPrincipalBinaryLinear,
  ...
  hrfSignedPolynomialTermSubsetQ, hrfWideAngleGeneratorChannelTags,
  hrfWideAngleTwoGeneratorChannelCoverQ,
  ...
  hrfEnsureSourceAwareDissectionLoaded, hrfNormalizeOrdinaryDissectionMode,
  hrfOrdinaryDissectionFactors, hrfOrdinaryDissectionComponents,
  hrfOrdinaryDissectionOutcome, hrfCertifiedScalingDataFromDissection,
  hrfApplyOrdinaryDissectionCertification, hrfHiddenRegionScanFromTrial,
...
(* Exact subset-in-ideal search.  Polynomial reduction is linear, so reduce a
   single generic binary-weighted sum of the original F terms and set every
   coefficient of its remainder to zero.  The resulting problem is mixed
   integer linear feasibility, not exponential subset enumeration. *)
hrfObstructionAlgebraicSearchBinaryLinearIdeal[F_, generators_List, vars_, kinVars_,
   maxSize_: Automatic] := Module[
  {terms, n, allVars, maxSizeEff, maxK, gs, z, genericFSL, rem, coeffs,
   constraints, timeLimit, sol, slIdx, obsIdx},
  terms = List @@ Expand[F];
  If[terms === {} || terms === {0},
    Return[{Missing["ZeroOrUndefinedPolynomial"], 0}]];
  gs = DeleteCases[Expand /@ generators, 0];
  If[gs === {}, Return[{Missing["NoGenerators"], 0}]];
  n = Length[terms];
  If[n <= 1, Return[{Missing["NoObstructionFound", 0], 0}]];
  allVars = DeleteDuplicates @ Join[vars, kinVars];
  maxSizeEff = hrfResolveMaxObstructionSize[F, gs, vars, maxSize];
  maxK = Min[maxSizeEff, n - 1];
  z = Array[Unique["hrfSLSelect$"] &, n];
  genericFSL = Expand[Total[MapThread[Times, {z, terms}]]];
  rem = Expand[PolynomialReduce[genericFSL, gs, allVars][[2]]];
  coeffs = If[rem === 0, {}, DeleteDuplicates[Values[CoefficientRules[rem, allVars]]]];
  constraints = Join[
    Thread[coeffs == 0],
    Thread[0 <= z], Thread[z <= 1],
    {Total[z] >= n - maxK, Total[z] >= 1, Total[z] <= n - 1}
  ];
  timeLimit = If[ValueQ[$HRFObstructionFindInstanceTimeLimit],
    $HRFObstructionFindInstanceTimeLimit, 20];
  sol = If[timeLimit === Infinity,
    Quiet @ FindInstance[constraints, z, Integers],
    TimeConstrained[
      Quiet @ FindInstance[constraints, z, Integers],
      timeLimit,
      $Aborted
  ...
  If[sol === $Aborted,
    Return[{Missing["ObstructionSearchTimeout", timeLimit], 1}]];
  If[! ListQ[sol] || sol === {},
    Return[{Missing["NoObstructionFound", maxK], 1}]];
  slIdx = Flatten @ Position[z /. First[sol], 1];
  obsIdx = Complement[Range[n], slIdx];
  If[slIdx === {} || obsIdx === {} || Length[obsIdx] > maxK,
    Return[{Missing["NoObstructionFound", maxK], 1}]];
  {obsIdx, 1}
];
...
hrfObstructionFromGeneratorVanishing[F_, generators_, vars_, kinVars_] := Module[
  ...
  If[TrueQ[Expand[obs] === 0],
    Return[<|
      ...
      "DerivativeConsistentQ" -> True,
      "SearchMethod" -> "AllInGeneratorIdeal"
    |>]
  ];
  If[TrueQ[Expand[fsl] === 0], Return[Missing["ZeroSuperleadingSector"]]];
...
(* Exact principal-ideal subset search as a 0--1 linear feasibility problem.
   For square-free pair generators g=Product[f_k], F_SL is divisible by g iff
   it vanishes modulo every coprime factor f_k.  Reducing each original term
   separately turns that condition into linear equations for binary term
   selectors.  This avoids the exponential meet-in-the-middle table, whose
   polynomial string keys can exhaust memory even for a 32-term F. *)
hrfObstructionAlgebraicSearchPrincipalBinaryLinear[F_, g_, vars_, kinVars_,
   maxSize_: Automatic] := Module[
  {terms, n, allVars, maxSizeEff, maxK, factors, polyFactors, z, remSums,
   coeffs, constraints, timeLimit, sol, slIdx, obsIdx, fsl},
  terms = List @@ Expand[F];
  If[terms === {} || terms === {0},
    Return[{Missing["ZeroOrUndefinedPolynomial"], 0}]];
  n = Length[terms];
  ...
  If[polyFactors === {}, Return[{Missing["NotApplicable"], 0}]];
  z = Array[Unique["hrfSLSelect$"] &, n];
  remSums = Table[
    Expand @ Total @ Table[
      z[[j]] PolynomialReduce[terms[[j]], {polyFactors[[k]]}, allVars][[2]],
      {j, n}
    ],
    {k, Length[polyFactors]}
  ];
  coeffs = DeleteDuplicates @ Flatten[
    If[Expand[#] === 0, {}, Values[CoefficientRules[Expand[#], allVars]]] & /@
      remSums
  ];
  constraints = Join[
    Thread[coeffs == 0],
  ...
  If[hrfObstructionSuperleadingInIdealQ[fsl, {g}, allVars],
    {obsIdx, 1},
    {Missing["NoObstructionFound", maxK], 1}
  ]
];
...
hrfObstructionAlgebraicSearch[F_, generators_, vars_, kinVars_, maxSize_: Automatic,
  ...
  exactLinear = hrfObstructionAlgebraicSearchBinaryLinearIdeal[
    F, generators, vars, kinVars, maxSize];
  If[! MatchQ[First[exactLinear], Missing["NotApplicable"]], Return[exactLinear]];

  If[Length[generators] === 1,
    Module[{fast},
      fast = hrfObstructionAlgebraicSearchPrincipalBinaryLinear[
        F, First[generators], vars, kinVars, maxSize];
...
(* Constraint-aware version used after saturated derivative harvesting.  The
   division is performed in K(x_kin)[x], so nonzero kinematic prefactors are
   units.  A nonzero raw remainder is accepted as the next kinematic layer
   only when every one of its x-coefficients vanishes modulo the supplied
   leading-surface equations. *)
hrfObstructionByKinematicCoefficientFieldDiagnostic[
    F_, generators_, vars_List, kinVars_List, constraints_] := Module[
  {constraintBasis, reduction, rawObs, reducedObs, fsl, usedCount,
   acceptedQ, result, reason},
  If[generators === {},
    Return[<|
      "Result" -> Missing["NoGenerators"],
      "AttemptData" -> <|
        "GeneratorSet" -> {}, "GeneratorCount" -> 0,
        "AcceptedQ" -> False, "RejectedReason" -> "NoGenerators",
        "SearchMethod" -> "KinematicCoefficientFieldQuotient"
      |>
    |>]
  ];
  constraintBasis = hrfKinematicConstraintGroebnerBasis[constraints, kinVars];
  reduction = Quiet @ Check[
    hrfCoefficientFieldPolynomialReduce[
      F, generators, vars, constraintBasis, kinVars
    ],
    $Failed
  ];
  If[reduction === $Failed || ! AssociationQ[reduction],
    Return[<|
      "Result" -> Missing["CoefficientFieldReductionFailed"],
      "AttemptData" -> <|
        "GeneratorSet" -> Factor /@ generators,
        "GeneratorCount" -> Length[generators],
        "AcceptedQ" -> False,
        "RejectedReason" -> "Coefficient-field polynomial reduction failed",
        "SearchMethod" -> "KinematicCoefficientFieldQuotient"
      |>
    |>]
  ];
  rawObs = Together[reduction["RawRemainder"]];
  reducedObs = Together[reduction["ConstraintReducedRemainder"]];
  acceptedQ = TrueQ[reduction["ConstraintVanishingRemainderQ"]];
  fsl = Together[F - rawObs];
  usedCount = Count[
    Lookup[reduction, "Quotients", {}],
    q_ /; ! TrueQ[Together[q] === 0]
  ];
  reason = If[acceptedQ, "--",
    "Coefficient-field remainder does not vanish on the leading kinematic surface"
  ];
  result = If[acceptedQ && ! TrueQ[Together[fsl] === 0],
    <|
      "Indices" -> {},
      "ObstructionTerms" -> If[TrueQ[rawObs === 0], {}, List @@ Expand[rawObs]],
      "Obstruction" -> Factor[rawObs],
      "Superleading" -> Factor[fsl],
      "Complement" -> Factor[fsl],
      "Generators" -> Factor /@ generators,
      "InIdealQ" -> True,
      "IdealMembershipRemainder" -> 0,
      "KinematicConstraintReducedRemainder" -> reducedObs,
      "KinematicConstraintVanishingObstructionQ" -> True,
      "SearchMethod" -> "KinematicCoefficientFieldQuotient"
    |>,
    Missing[If[TrueQ[Together[fsl] === 0],
      "ZeroSuperleadingSector", "NoConstraintVanishingObstruction"]]
  ];
  <|
    "Result" -> result,
    "AttemptData" -> <|
      "GeneratorSet" -> Factor /@ generators,
      "GeneratorCount" -> Length[generators],
      "UsedGeneratorCount" -> usedCount,
      "SearchMethod" -> "KinematicCoefficientFieldQuotient",
      "ConstraintCount" -> Length[constraintBasis],
      "ConstraintVanishingRemainderQ" -> acceptedQ,
      "RawRemainder" -> Factor[rawObs],
      "ConstraintReducedRemainder" -> Factor[reducedObs],
      "AcceptedQ" -> AssociationQ[result],
...
(* Exact all-in-ideal decomposition: F_SL = F and Obs = 0.  This is the
   Regge case where the leading polynomial is already generated by g(x). *)
...
obstructionByOriginalTermsGeneralDiagnostic[F_, generators_, vars_, kinVars_, maxSize_ : Automatic,
 ...
 Module[{attempts, eta, k1, k2, best, accepted},
  ...
  If[Length[attempts] > 1 && TrueQ[$HRFStoreChannelObstructionAttemptsQ],
...
(* Stricter Crown-form diagnostic:

     F_SL = +/- s12 g12 +/- s23 g23

   with no x-dependent quotient.  In this restricted presentation each
   kin-free generator is, up to sign, an exact term-subpolynomial of one
   channel coefficient of F0, and a two-generator set covers both independent
   channels.  This is useful structural evidence but not a general rejection
   rule: Eq. (15) also permits polynomial multipliers M_k(x,s). *)
hrfSignedPolynomialTermSubsetQ[g_, coefficientPoly_, vars_List] := Module[
  {gt, ct},
  gt = MonomialList[Expand[g], vars];
  ct = MonomialList[Expand[coefficientPoly], vars];
  gt =!= {} && ct =!= {} &&
    (And @@ (MemberQ[ct, #] & /@ gt) ||
      And @@ (MemberQ[ct, -#] & /@ gt))
];

hrfWideAngleGeneratorChannelTags[g_, F0_, vars_List, kinVars_List] :=
  If[
    Length[kinVars] =!= 2 ||
      hrfFactorContainsKinVarsQ[g, kinVars],
    {},
    Select[
      kinVars,
      hrfSignedPolynomialTermSubsetQ[
        g, Expand[Coefficient[F0, #]], vars
      ] &
    ]
  ];

hrfWideAngleTwoGeneratorChannelCoverQ[
    {g1_, g2_}, F0_, vars_List, kinVars_List] := Module[{t1, t2},
  If[Length[kinVars] =!= 2, Return[True]];
  t1 = hrfWideAngleGeneratorChannelTags[g1, F0, vars, kinVars];
  t2 = hrfWideAngleGeneratorChannelTags[g2, F0, vars, kinVars];
  (MemberQ[t1, kinVars[[1]]] && MemberQ[t2, kinVars[[2]]]) ||
    (MemberQ[t1, kinVars[[2]]] && MemberQ[t2, kinVars[[1]]])
];
...
factorsDividingGenerator[gen_, ff_, allVars_, vars_: {}, kinVars_: {}] := Module[
  {key, provenance},
  key = ToString[InputForm[Expand[gen]]];
  provenance = If[ValueQ[$HRFGeneratorFactorProvenance] &&
      AssociationQ[$HRFGeneratorFactorProvenance],
    Lookup[$HRFGeneratorFactorProvenance, key, Missing["NotRecorded"]],
    Missing["NotRecorded"]
  ];
  If[ListQ[provenance] && provenance =!= {},
    Return[DeleteDuplicates[provenance]]
  ];
...
ClearAll[candidateGeneratorSets, candidateGeneratorSetsDiagnostic, candidateGeneratorSetsAdaptive,
  hrfResolveSingleProductGeneratorFactors, hrfCompletedGeneratorSets,
  hrfCompletionFactorsFromQuotient, hrfIndexedDisjointSupportFactorPairIndices,
  hrfIndexedDisjointSupportFactorPairs, hrfPairProductExponentSupportData,
  hrfPairProductDegreeF0SupportAdmissibleQ,
  hrfFirstDistinctExpandedPairProducts, hrfGeneratorSectorSupportTags,
  hrfFirstSectorCompatibleIndexPairs];

(* Precompute the x-exponent support of each kinematic sector once, then tag
   every generator whose monomials are dominated by that support.  This is
   exactly the predicate hrfGeneratorMonomialsInF0Q uses, but avoids rescanning
   the complete F polynomial for every generator and every sector. *)
hrfGeneratorSectorSupportTags[gens_List, F_, vars_List, kinVars_List] := Module[
  {fTerms, sectorSupports, generatorSupports},
  fTerms = If[Head[Expand[F]] === Plus, List @@ Expand[F], {Expand[F]}];
  sectorSupports = Association @ Table[
    sector -> (hrfExponentListInVars[#, vars] & /@
      Select[fTerms,
        Function[term,
          ! FreeQ[term, sector] &&
            And @@ (Function[kv,
              If[kv === sector, True, FreeQ[term, kv]]
            ] /@ kinVars)
        ]]),
    {sector, kinVars}
  ];
  generatorSupports = (hrfExponentListInVars[#, vars] & /@
      If[Head[Expand[#]] === Plus, List @@ Expand[#], {Expand[#]}]) & /@ gens;
  Table[
    Select[kinVars, Function[sector,
      And @@ (Function[gex,
        AnyTrue[Lookup[sectorSupports, sector, {}],
          Function[fex, hrfXExponentDominatedByQ[gex, fex]]]
      ] /@ generatorSupports[[i]])
    ]],
    {i, Length[gens]}
  ]
...
(* Enumerate only sector-compatible pairs.  A finite limit is applied while
   enumerating, rather than after materialising all Binomial[n,2] pairs.  One
   extra compatible pair is sought so LimitReachedQ is an exact statement. *)
hrfFirstSectorCompatibleIndexPairs[tags_List, kinVars_List, limit_: Infinity] := Module[
  {out = {}, finiteQ, target, i, j, compatibleQ, truncatedQ = False},
  finiteQ = IntegerQ[limit] && limit >= 0;
  target = If[finiteQ, limit, Infinity];
  compatibleQ[a_, b_] :=
    (MemberQ[a, kinVars[[1]]] && MemberQ[b, kinVars[[2]]]) ||
    (MemberQ[a, kinVars[[2]]] && MemberQ[b, kinVars[[1]]]);
  Catch[
    Do[
      If[compatibleQ[tags[[i]], tags[[j]]],
        If[finiteQ && Length[out] >= target,
          truncatedQ = True; Throw[Null, "HRFSectorPairLimit"],
          AppendTo[out, {i, j}]
        ]
      ],
      {i, 1, Max[0, Length[tags] - 1]}, {j, i + 1, Length[tags]}
    ],
    "HRFSectorPairLimit"
  ];
  <|"IndexPairs" -> out, "LimitReachedQ" -> truncatedQ|>
];
...
(* Generate exactly the factor pairs that can pass the cheap necessary part
   of the massless physics filter, without first materialising all n(n-1)/2
   pairs.  Factors are grouped by an integer bitmask of their x-support.
   Every omitted mask pair has nonempty support intersection and is therefore
   rejected by hrfGeneratorPairPhysicsAdmissibleQ.  The later degree, F0 and
   positive-domain tests are still applied unchanged. *)
hrfIndexedDisjointSupportFactorPairIndices[
    ff_List, vars_List, kinVars_List, bounds_Association] := Module[
  {n, exponents, masks, degrees, kinQ, groups, keys, keyPairs,
   indexPairs, zeroPairs, dF},
  n = Length[ff];
  If[n < 2, Return[{}]];
  exponents = hrfExponentListInVars[#, vars] & /@ ff;
  masks = Map[
    Function[ex, Total @ MapIndexed[
      If[#1 > 0, 2^(First[#2] - 1), 0] &, ex]],
    exponents
  ];
  degrees = hrfPolynomialTotalDegreeInVars[#, vars] & /@ ff;
  kinQ = hrfFactorContainsKinVarsQ[#, kinVars] & /@ ff;
  dF = Lookup[bounds, "MaxGeneratorTotalDegree", Infinity];
  groups = GroupBy[Range[n], masks[[#]] &];
  keys = Keys[groups];
  keyPairs = Select[Subsets[keys, {2}], BitAnd @@ # == 0 &];
  indexPairs = Flatten[
    (Tuples[{groups[#[[1]]], groups[#[[2]]]}] &) /@ keyPairs,
    1
  ];
  zeroPairs = If[KeyExistsQ[groups, 0], Subsets[groups[0], {2}], {}];
  indexPairs = Sort /@ Join[indexPairs, zeroPairs];
  indexPairs = Select[DeleteDuplicates[indexPairs],
    (TrueQ[$HRFAllowKinematicFactorPairsQ] ||
      ! (kinQ[[#[[1]]]] && kinQ[[#[[2]]]])) &&
      degrees[[#[[1]]]] + degrees[[#[[2]]]] <= dF &
  ];
  indexPairs
];

hrfIndexedDisjointSupportFactorPairs[
    ff_List, vars_List, kinVars_List, bounds_Association] :=
  (ff[[#]] &) /@
    hrfIndexedDisjointSupportFactorPairIndices[ff, vars, kinVars, bounds];

(* For support-disjoint factors the x-monomials of f_i f_j are in one-to-one
   correspondence with Cartesian sums of the exponent vectors of f_i and f_j.
   Hence the massless exponent and F0-support tests can be performed exactly
   without expanding tens of thousands of symbolic pair products. *)
hrfPairProductExponentSupportData[ff_List, F0_, vars_List, kinVars_List] := Module[
  {factorExponents, terms, sectors, sectorTerms, sectorExponents, binaryQ,
   exponentMask, subMasks, factorMasks, allowedMaskSets},
  factorExponents = (First /@ CoefficientRules[Expand[#], vars]) & /@ ff;
  terms = If[F0 === None || F0 === Automatic, {},
    If[Head[Expand[F0]] === Plus, List @@ Expand[F0], {Expand[F0]}]
  ];
  sectors = If[kinVars === {}, {None}, Prepend[kinVars, None]];
  sectorTerms[sector_] := Select[terms, Function[t,
    Which[
      sector === None, kinVars === {} || FreeQ[t, Alternatives @@ kinVars],
      True, ! FreeQ[t, sector] &&
        And @@ (If[k === sector, True, FreeQ[t, k]] & /@ kinVars)
    ]
  ]];
  sectorExponents = Association @ Table[
    sector -> (hrfExponentListInVars[#, vars] & /@ sectorTerms[sector]),
    {sector, sectors}
  ];
  binaryQ = And @@ Flatten[
    Map[MemberQ[{0, 1}, #] &, Join[factorExponents, Values[sectorExponents]], {3}]
  ];
  exponentMask[ex_List] := Total @ MapIndexed[If[#1 > 0, 2^(First[#2] - 1), 0] &, ex];
  subMasks[ex_List] := Total[2^(# - 1) & /@ #] & /@
    Subsets[Flatten @ Position[ex, 1]];
  factorMasks = If[binaryQ, (exponentMask /@ #) & /@ factorExponents, {}];
  allowedMaskSets = If[binaryQ,
    Association @ KeyValueMap[
      #1 -> AssociationThread[
        DeleteDuplicates[Flatten[subMasks /@ #2]],
        ConstantArray[True, Length[DeleteDuplicates[Flatten[subMasks /@ #2]]]]
      ] &,
      sectorExponents
    ],
    <||>
  ...
  <|"FactorExponents" -> factorExponents,
    "SectorExponents" -> sectorExponents,
    "BinaryMaskModeQ" -> binaryQ,
    "FactorMasks" -> factorMasks,
    "AllowedMaskSets" -> allowedMaskSets,
    "RequireF0SupportQ" -> hrfResolveRequireGeneratorMonomialsInF0Q[kinVars],
    "F0AvailableQ" -> !(F0 === None || F0 === Automatic || TrueQ[Expand[F0] === 0])|>
];

hrfPairProductDegreeF0SupportAdmissibleQ[
    pairIndex_List, data_Association, bounds_Association] := Module[
  {e1, e2, productExponents, maxVE, sectorExponents, dominatedQ,
   m1, m2, productMasks, allowedMaskSets},
  maxVE = Lookup[bounds, "MaxGeneratorVarExponent", Infinity];
  If[TrueQ[data["BinaryMaskModeQ"]] && maxVE >= 1,
    {m1, m2} = data["FactorMasks"][[pairIndex]];
    productMasks = DeleteDuplicates @ Flatten[Table[BitOr[a, b], {a, m1}, {b, m2}]];
    If[! TrueQ[data["RequireF0SupportQ"]] || ! TrueQ[data["F0AvailableQ"]],
      Return[True]
    ];
    allowedMaskSets = Values[data["AllowedMaskSets"]];
    Return[AnyTrue[allowedMaskSets,
      Function[allowed, And @@ (TrueQ[Lookup[allowed, #, False]] & /@ productMasks)]
    ]]
  ];
  {e1, e2} = data["FactorExponents"][[pairIndex]];
  productExponents = Flatten[Table[a + b, {a, e1}, {b, e2}], 1];
  If[productExponents === {}, Return[True]];
  If[Max[Flatten[productExponents]] > maxVE, Return[False]];
  If[! TrueQ[data["RequireF0SupportQ"]] || ! TrueQ[data["F0AvailableQ"]],
    Return[True]
  ];
  sectorExponents = Values[data["SectorExponents"]];
  dominatedQ[ex_, available_] := AnyTrue[available, hrfXExponentDominatedByQ[ex, #] &];
  AnyTrue[sectorExponents,
    Function[available,
      available =!= {} && And @@ (dominatedQ[#, available] & /@ productExponents)
    ]
  ]
];

(* Exact lazy form of Take[DeleteDuplicates[Expand /@ (Times @@@ pairs)], n].
   This matters when the caller has explicitly requested a finite exploratory
   candidate cap: products beyond that cap must not be expanded merely to be
   discarded.  Infinity retains the complete historical enumeration. *)
hrfFirstDistinctExpandedPairProducts[pairs_List, n_, vars_: {}] := Module[
  {out = {}, g, key, coeffRules, leadCoeff, finiteQ = IntegerQ[n] && n >= 0},
  If[finiteQ && n === 0, Return[{}]];
  Do[
    g = Expand[Times @@ pairs[[i]]];
    If[vars =!= {},
      coeffRules = CoefficientRules[g, vars];
      leadCoeff = If[coeffRules === {}, 1, First[coeffRules][[2]]];
      If[NumericQ[leadCoeff] && TrueQ[leadCoeff < 0], g = -g]
    ];
    key = ToString[InputForm[g]];
    If[! MemberQ[out, g],
      AppendTo[out, g];
      If[! AssociationQ[$HRFGeneratorFactorProvenance],
        $HRFGeneratorFactorProvenance = <||>];
      $HRFGeneratorFactorProvenance[key] = pairs[[i]]
    ];
    If[finiteQ && Length[out] >= n, Break[]],
    {i, Length[pairs]}
  ];
  out
];
...
candidateGeneratorSetsDiagnostic[factors_, maxGenerators_ : 2, vars_, kinAssumptions_, kinVars_, F_,
   opts_: <||>] := Module[
  {pack, ff, bounds, factorPairIndices, factorPairs, admissiblePairIndices,
   pairSupportData, positiveFactorQ, xSupports, admissiblePairs, pairGens,
   fullGen, fullSet, pairSingles,
   pairSubsets, sets, limit, skipPDFQ, fullDegree, useSectorQuotientQ,
   unionLimit, unionLimitReachedQ = False, pairGenCountBeforeLimit,
   pairGenLimitReachedQ = False, finalLimitReachedQ = False},
  ...
  skipPDFQ = TrueQ[Lookup[opts, "SkipPDFFindInstanceQ", False]] ||
    TrueQ[Lookup[bounds, "SkipPDFFindInstanceQ", False]];
  (* A linear basis of pair-generator polynomials need not preserve the
     individual principal ideals or their hypersurfaces.  Keep the historical
     sector quotient available for exploratory runs, but require an explicit
     opt-in; completeness audits enumerate every distinct generator. *)
  useSectorQuotientQ = TrueQ[Lookup[opts, "UseGeneratorSectorQuotientQ", False]];
  $HRFGeneratorFactorProvenance = <||>;
  factorPairIndices = If[TrueQ[$HRFUseGeneratorPhysicsFilterQ] &&
      TrueQ[$HRFUseIndexedDisjointSupportPairEnumerationQ],
    hrfIndexedDisjointSupportFactorPairIndices[ff, vars, kinVars, bounds],
    Subsets[Range[Length[ff]], {2}]
  ];
  factorPairs = (ff[[#]] &) /@ factorPairIndices;
  admissiblePairIndices = Pick[factorPairIndices,
    (If[TrueQ[$HRFUseGeneratorPhysicsFilterQ],
  ...
  pairSupportData = hrfPairProductExponentSupportData[ff, F, vars, kinVars];
  admissiblePairIndices = Select[admissiblePairIndices,
    hrfPairProductDegreeF0SupportAdmissibleQ[#, pairSupportData, bounds] &
  ];
  positiveFactorQ = If[Length[DownValues[hrfSquareFreeHomogeneousPositiveZeroQ]] > 0,
    hrfSquareFreeHomogeneousPositiveZeroQ[#, vars, kinVars] & /@ ff,
    ConstantArray[False, Length[ff]]
  ];
  xSupports = hrfFactorXSupport[#, vars] & /@ ff;
  admissiblePairIndices = Select[admissiblePairIndices,
    (* Support/kinematic-sector, degree and F0-support tests have already been
       applied.  Cache the exact square-free positive-zero certificate once
       per factor; retain the original simultaneous-domain fallback for every
       pair outside that certified class. *)
    skipPDFQ ||
      (positiveFactorQ[[#[[1]]]] && positiveFactorQ[[#[[2]]]] &&
        Intersection[xSupports[[#[[1]]]], xSupports[[#[[2]]]]] === {}) ||
      simultaneouslyAdmissibleSubsetQ[ff[[#]], vars, kinAssumptions, kinVars] &
  ];
  admissiblePairs = (ff[[#]] &) /@ admissiblePairIndices;
  If[useSectorQuotientQ && TrueQ[$HRFUseGeneratorPhysicsFilterQ] &&
      Length[DownValues[hrfFilterAdmissiblePairsModuleDedup]] > 0,
  ...
  (* Different factor-pair presentations can produce exactly the same
     polynomial generator.  Equality here preserves the principal ideal
     itself (unlike the optional linear/module quotient), so exact polynomial
     deduplication is completeness-safe before building generator unions. *)
  pairGens = hrfFirstDistinctExpandedPairProducts[
    admissiblePairs,
    If[IntegerQ[limit] && limit > 0, limit, Infinity],
    vars
  ];
  pairGenCountBeforeLimit = Length[pairGens];
  If[useSectorQuotientQ && TrueQ[$HRFUseGeneratorPhysicsFilterQ] &&
      Length[DownValues[hrfCanonicalizeGeneratorsModKinSectors]] > 0,
  ...
  fullDegree = Total[hrfPolynomialTotalDegreeInVars[#, vars] & /@ ff];
  fullSet = If[
    fullDegree <= Lookup[bounds, "MaxGeneratorTotalDegree", Infinity],
    fullGen = Times @@ ff;
    If[hrfGeneratorDegreeAdmissibleQ[fullGen, bounds, vars, F, kinVars] &&
        (skipPDFQ || simultaneouslyAdmissibleSubsetQ[
          ff, vars, kinAssumptions, kinVars
        ]),
      {{fullGen}}, {}
    ],
    {}
  ...
  pairSubsets = If[maxGenerators >= 2,
    Module[{kinFreeGens, base, subs, tags, pairEnumeration},
      kinFreeGens = Select[pairGens, ! hrfFactorContainsKinVarsQ[#, kinVars] &];
      base = If[kinFreeGens === {}, pairGens, kinFreeGens];
      unionLimit = Which[
        KeyExistsQ[opts, "MaxTwoGeneratorUnionTrials"], opts["MaxTwoGeneratorUnionTrials"],
        ValueQ[$HRFMaxTwoGeneratorUnionTrials], $HRFMaxTwoGeneratorUnionTrials,
        True, Infinity
      ];
      subs = If[maxGenerators === 2 && Length[kinVars] === 2 &&
          hrfResolveRequireGeneratorMonomialsInF0Q[kinVars],
        tags = hrfGeneratorSectorSupportTags[base, F, vars, kinVars];
        pairEnumeration = hrfFirstSectorCompatibleIndexPairs[tags, kinVars, unionLimit];
        unionLimitReachedQ = TrueQ[pairEnumeration["LimitReachedQ"]];
        base[[#]] & /@ pairEnumeration["IndexPairs"],
        Module[{all = Select[Subsets[base, {2, maxGenerators}],
            hrfGeneratorSetDegreeAdmissibleQ[#, bounds, vars, F, kinVars] &]},
          unionLimitReachedQ = IntegerQ[unionLimit] && unionLimit >= 0 &&
            Length[all] > unionLimit;
          If[unionLimitReachedQ, Take[all, unionLimit], all]
        ]
      ];
      subs
    ],
  ...
  If[IntegerQ[limit] && limit > 0 && Length[sets] > limit,
    Module[{multi = Select[sets, Length[#] > 1 &], single = Select[sets, Length[#] === 1 &]},
      finalLimitReachedQ = True;
      sets = Take[Join[multi, single], limit]
    ],
    Null
  ];
  $HRFLastCandidateGeneratorConstructionAudit = <|
    "PairGeneratorCountBeforeLimit" -> pairGenCountBeforeLimit,
    "PairGeneratorCountUsed" -> Length[pairGens],
    "PairGeneratorLimitReachedQ" -> pairGenLimitReachedQ,
    "TwoGeneratorUnionCountUsed" -> Length[pairSubsets],
    "MaxTwoGeneratorUnionTrials" -> unionLimit,
    "TwoGeneratorUnionLimitReachedQ" -> unionLimitReachedQ,
    "CandidateGeneratorSetLimit" -> limit,
    "CandidateGeneratorSetLimitReachedQ" -> finalLimitReachedQ,
    "SearchTruncatedQ" -> Or[pairGenLimitReachedQ, unionLimitReachedQ, finalLimitReachedQ]
  |>;
  sets
];
...
hrfCompletionFactorsFromQuotient[q_, vars_, kinVars_] := Module[
  {parts, fac},
  parts = FactorList[Factor[Expand[q]]][[All, 1]];
  parts = DeleteCases[parts, _?NumericQ];
  parts = Select[parts, ! FreeQ[#, Alternatives @@ vars] &];
  fac = If[ValueQ[hrfCanonicalCancellationFactor],
    hrfCanonicalCancellationFactor[#, vars, kinVars] & /@ parts,
    Factor /@ parts
  ];
  DeleteDuplicates @ Select[fac,
    ! TrueQ[Expand[#] === 0] && ! monomialQ[#, vars] &
  ]
];

(* Completion step for well-hidden generators.
   A known cancellation factor f_a is used only as a probe.  If the quotient
   decomposition F_SL in <f_a> exposes another admissible factor f_b in
   F_SL/f_a, then only the completed product g = f_a f_b is promoted to the
   ordinary generator list.  The one-factor probe itself is never accepted as
   a generator. *)
hrfCompletedGeneratorSets[F_, factors_, vars_, kinAssumptions_, kinVars_, bounds_: <||>] := Module[
  {ff, allVars, rows = {}, genSets = {}, completed = {}, probe, obsDiag, obs,
   fsl, quot, rem, q, qFactors, h, pair, gen, admissiblePairQ, degreeQ,
   pairPrefilterQ},
  ...
  Do[
    probe = ff[[i]];
    obsDiag = obstructionByOriginalTermsGeneralDiagnostic[
      F, {probe}, vars, kinVars, Automatic, kinAssumptions
    ];
    obs = Lookup[obsDiag, "Result", Missing["NoResult"]];
    If[AssociationQ[obs] && hrfValidObstructionResultQ[obs, {probe}, vars, kinVars],
      fsl = Expand[Lookup[obs, "Superleading", 0]];
      If[! TrueQ[fsl === 0],
        {quot, rem} = PolynomialReduce[fsl, {probe}, allVars];
        If[ListQ[quot] && Length[quot] >= 1 && TrueQ[Expand[rem] === 0],
          q = First[quot];
          qFactors = hrfCompletionFactorsFromQuotient[q, vars, kinVars];
          Do[
            h = qFactors[[j]];
            pair = DeleteDuplicates[Factor /@ {probe, h}];
            If[Length[pair] >= 2,
              gen = Expand[Times @@ pair];
              pairPrefilterQ =
                Length[DownValues[hrfGeneratorPairKinPrefilterQ]] === 0 ||
                  hrfGeneratorPairKinPrefilterQ[pair[[1]], pair[[2]], kinVars];
              admissiblePairQ = pairPrefilterQ &&
                And @@ (positiveCompatibleQ[#, vars, kinAssumptions, kinVars] & /@ pair) &&
                simultaneouslyAdmissibleSubsetQ[pair, vars, kinAssumptions, kinVars];
              degreeQ = If[AssociationQ[bounds],
                hrfGeneratorDegreeAdmissibleQ[gen, bounds, vars, F, kinVars],
                True
              ];
              AppendTo[rows, <|
                "ProbeFactor" -> probe,
                "CompletedFactor" -> h,
                "CompletedGenerator" -> Factor[gen],
                "PairPrefilterQ" -> pairPrefilterQ,
                "SimultaneouslyAdmissibleSubsetQ" -> admissiblePairQ,
                "DegreeAdmissibleQ" -> degreeQ,
                "AcceptedCompletionQ" -> TrueQ[admissiblePairQ && degreeQ],
                "ProbeSuperleading" -> Factor[fsl],
                "ProbeObstruction" -> Lookup[obs, "Obstruction", Missing["NoObstruction"]]
              |>];
              If[TrueQ[admissiblePairQ && degreeQ],
                AppendTo[genSets, {gen}];
                completed = Join[completed, pair];
              ];
            ],
            {j, Length[qFactors]}
          ];
  ...
  <|
    "GeneratorSets" -> DeleteDuplicates[genSets],
    "CompletedFactors" -> DeleteDuplicates[completed],
    "Rows" -> rows
  |>
];
...
generatorFactorData[generators_, factors_, vars_, kinVars_, kinAssumptions_: True, bounds_: Automatic, F0_: None] := Module[{allVars, ff},
  ...
  Table[
    Module[{fac = factorsDividingGenerator[gen, ff, allVars, vars, kinVars], simQ, degQ, f0Q, relaxQ, skipPDFQ},
      simQ = hrfSimultaneouslyAdmissibleSubsetQFast[fac, vars, kinAssumptions, kinVars];
...
generatorSetAdmissibilityData[generators_, factors_, vars_, kinVars_, kinAssumptions_: True, bounds_: Automatic, F0_: None] := Module[
  ...
  setQ = skipPDFQ || hrfSimultaneouslyAdmissibleSubsetQFast[
    unionFac, vars, kinAssumptions, kinVars];
  <|
...
slSectorAdmissibilityData[FSL_, generators_, factors_, vars_, kinVars_, kinAssumptions_: True,
   bounds_: Automatic, F0_: None, idealContext_: <||>] := Module[
  {use, usedGens, usedData, unionFac, perGenQ, setQ, idealQ, allVars = DeleteDuplicates@Join[vars, kinVars],
   skipPDFQ, constraintBasis, coefficientReduction},
  skipPDFQ = hrfSkipPDFFindInstanceQ[bounds] || hrfCollinearSingleProductLegacyQ[bounds];
  use = generatorUseData[FSL, generators, vars, kinVars, idealContext];
  If[TrueQ[Expand[FSL] === 0],
    Return[<|
      "GeneratorUseData" -> use,
      "SLSectorGenerators" -> {},
      "SLSectorGeneratorCount" -> 0,
      "SLSectorGeneratorFactorData" -> {},
      "SLSectorFactorUnion" -> {},
      "SLSectorFactorCount" -> 0,
      "PerGeneratorSLSectorAdmissibleQ" -> False,
      "SimultaneouslyAdmissibleSLSectorQ" -> False,
      "SuperleadingInIdealQ" -> False,
      "AdmissibleSLSectorQ" -> False,
      "SLSectorAdmissibilityRule" -> "Rejected: zero superleading sector (F_SL=0)"
    |>]
  ];
  ...
  setQ = skipPDFQ || (Length[usedGens] >= 1 &&
    hrfSimultaneouslyAdmissibleSubsetQFast[unionFac, vars, kinAssumptions, kinVars]);
  idealQ = If[Lookup[idealContext, "Mode", "PolynomialRing"] ===
      "KinematicCoefficientField",
    constraintBasis = hrfKinematicConstraintGroebnerBasis[
      Lookup[idealContext, "Constraints", {}], kinVars
    ];
    coefficientReduction = Quiet @ Check[
      hrfCoefficientFieldPolynomialReduce[
        FSL, generators, vars, constraintBasis, kinVars
      ],
      <|"ConstraintVanishingRemainderQ" -> False|>
    ];
    TrueQ[Lookup[coefficientReduction,
      "ConstraintVanishingRemainderQ", False]],
    hrfObstructionSuperleadingInIdealQ[FSL, generators, allVars]
  ];
  <|
    "GeneratorUseData" -> use,
...
generatorUseData[FSL_, generators_, vars_, kinVars_, idealContext_: <||>] :=
 Module[{allVars, quotients, remainder, used, constraintBasis, reduction},
  allVars = DeleteDuplicates@Join[vars, kinVars];
  If[Lookup[idealContext, "Mode", "PolynomialRing"] ===
      "KinematicCoefficientField",
    constraintBasis = hrfKinematicConstraintGroebnerBasis[
      Lookup[idealContext, "Constraints", {}], kinVars
    ];
    reduction = hrfCoefficientFieldPolynomialReduce[
      FSL, generators, vars, constraintBasis, kinVars
    ];
    quotients = reduction["Quotients"];
    remainder = reduction["ConstraintReducedRemainder"],
...
(* Rank admissible trials.  Wide-angle: prefer more generators (Crown F_SL = s12 g1 + s23 g2).
   Regge / single kin var: prefer all-in-ideal decompositions, then fewer
   generators, then smaller obstruction. *)
...
(* Wide-angle 4pt Crown-form diagnostic: test whether F_SL has the restricted
   form s_a*g_a with kin-free sector generators g_a.  This identifies familiar
   Crown presentations and flags decompositions such as {s23,s12*x10}, but it
   is not used by validTrials: the general Eq. (15) form permits polynomial
   multipliers M_k(x,s). *)
...
ClearAll[hrfTrialObstructionForScaling, hrfTrialPostScalingInput,
  hrfTrialEffectiveSuperleading, hrfMiniScanFromTrial,
  hrfTrialCoverageScalingData, hrfTrialCoverageFoundQ, hrfEvaluateValidTrialScaling,
  hrfEnsureSourceAwareDissectionLoaded, hrfNormalizeOrdinaryDissectionMode,
  hrfOrdinaryDissectionFactors, hrfOrdinaryDissectionComponents,
  hrfOrdinaryDissectionOutcome, hrfCertifiedScalingDataFromDissection,
  hrfApplyOrdinaryDissectionCertification, hrfHiddenRegionScanFromTrial,
  hrfResolveRequireValidScalingForHiddenRegionQ,
...
hrfTrialObstructionForScaling[trial_] := Module[{od, obst},
  ...
  (* A remainder proportional to a defining equation of the leading
     kinematic surface is not a native layer-zero obstruction.  Its actual
     asymptotic order must be supplied through FObsForScaling/DeltaLayers by
     the chosen kinematic expansion. *)
  If[TrueQ[Lookup[od, "KinematicConstraintVanishingObstructionQ", False]],
    Return[None]
  ];
...
hrfTrialPostScalingInput[trial_, fObs_] := Module[{obst = hrfTrialObstructionForScaling[trial]},
  Which[
    fObs === Automatic, obst,
    TrueQ[fObs === None], None,
    AssociationQ[fObs] && ! KeyExistsQ[fObs, "FObs"] && ! KeyExistsQ[fObs, "Obstruction"],
      Join[<|"FObs" -> obst|>, fObs],
    AssociationQ[fObs] && KeyExistsQ[fObs, "FObs"] && MatchQ[fObs["FObs"], Automatic],
      Join[fObs, <|"FObs" -> obst|>],
    AssociationQ[fObs] && KeyExistsQ[fObs, "Obstruction"] && MatchQ[fObs["Obstruction"], Automatic],
      Join[fObs, <|"Obstruction" -> obst|>],
    True, fObs
  ]
];
...
hrfTrialCoverageScalingData[trial_, U_, vars_, maxAbs_, fObs_:Automatic,
   method_:Automatic] := Module[{fsl, fObsVal, meth},
  ...
  If[MatchQ[fsl, _Missing] || TrueQ[Expand[fsl] === 0],
    Return[<|
      "Scaling" -> Missing["ZeroSuperleadingSector"],
      "ScalingStatus" -> "ZeroSuperleadingSector",
      "ScalingStatusMessage" -> "No scaling: zero superleading sector (F_SL=0)"
    |>]
  ];
  fObsVal = hrfTrialPostScalingInput[trial, fObs];
  meth = Replace[method, Automatic -> "ExactCoverage"];
  Which[
    MemberQ[{"ExactCoverage", "Exact"}, meth] &&
      Length[DownValues[findExactCoverageLPScaling]] > 0,
      findExactCoverageLPScaling[fsl, Expand[U], vars, fObsVal,
        "Generators" -> Lookup[trial, "Generators", {}]],
    MemberQ[{"ExactCoverage", "Exact"}, meth],
      <|"Scaling" -> Missing["ExactCoverageScalingUnavailable"],
        "ScalingStatus" -> "Unavailable",
        "ScalingStatusMessage" -> "Exact coverage scaling helper was not loaded"|>,
    True,
      findCoverageLPScalingBounded[fsl, Expand[U], vars, maxAbs, fObsVal]
  ]
];
...
hrfEvaluateValidTrialScaling[trial_, U_, vars_, maxAbs_, fObs_:Automatic,
   method_:Automatic] := Module[{cov, sl, exactQ, foundQ},
  cov = hrfTrialCoverageScalingData[trial, U, vars, maxAbs, fObs, method];
  sl = hrfTrialEffectiveSuperleading[trial];
  exactQ = False;
  foundQ = Which[
    MatchQ[cov, _Missing], False,
    True, hrfTrialCoverageFoundQ[cov]
  ];
  <|
    "Trial" -> trial,
...
(* The ordinary (non-aligned) search uses the coverage equations only as a
   candidate filter.  Exact source-aware dissection is the final certificate:
   it resolves the cancellation in local coordinates, finds a genuine lower
   facet of the complete LP polynomial, and pulls its scaling back to the
   original variables. *)
hrfEnsureSourceAwareDissectionLoaded[] := Module[{path, loaded},
  If[Length[DownValues[hrfSourceAwareDissectionCertificate]] > 0, Return[True]];
  path = FileNameJoin[{hrfPackageDirectory[],
    "HRF_SourceAwareDissectionCertification.wl"}];
  loaded = Quiet @ Check[Get[path], $Failed];
  loaded =!= $Failed &&
    Length[DownValues[hrfSourceAwareDissectionCertificate]] > 0
];

hrfNormalizeOrdinaryDissectionMode[mode_] := Switch[mode,
  Automatic | "Required" | "On", "Required",
  "Diagnostic", "Diagnostic",
  "Off" | None | False, "Off",
  _, "Required"
];

hrfOrdinaryDissectionFactors[trial_] := Module[{factors},
  factors = Lookup[trial, "SLSectorFactorUnion",
    Lookup[trial, "GeneratorSetFactorUnion", {}]];
  factors = DeleteDuplicates[Factor /@ Flatten[{factors}]];
  DeleteCases[factors, 0 | 1 | -1]
];

hrfOrdinaryDissectionComponents[trial_, U_, fObs_, eta_] := Module[
  {fsl, postInput, layers, weighted},
  fsl = hrfTrialEffectiveSuperleading[trial];
  postInput = hrfTrialPostScalingInput[trial, fObs];
  layers = hrfWeightedPostLayerData[postInput, "FObs"];
  layers = Select[layers,
    ! MatchQ[Lookup[#, "Polynomial", None], None | Automatic | Missing[__]] &
  ];
  weighted[chosen_] := Expand @ Total @ Map[
    Lookup[#, "Polynomial", 0] eta^Lookup[#, "Power", 0] &,
    Select[layers, chosen[Lookup[#, "Label", "FObs"]] &]
  ];
  <|
    "FSL" -> If[MatchQ[fsl, _Missing], 0, Expand[fsl]],
    "FObs" -> weighted[SameQ[#, "FObs"] &],
    "FOutside" -> weighted[! SameQ[#, "FObs"] &],
    "U" -> Expand[U]
  |>
];

hrfOrdinaryDissectionOutcome[cert_Association] := Which[
  TrueQ[Lookup[cert, "CertifiedQ", False]], "Certified",
  TrueQ[Lookup[cert, "SearchCompleteQ", False]], "Rejected",
  True, "Unresolved"
];
hrfOrdinaryDissectionOutcome[_] := "Unresolved";

hrfCertifiedScalingDataFromDissection[
    certification_Association, vars_List, coverage_] := Module[
  {certificates, selected, pullbacks, vectors, selectedPullback,
   selectedVector, wsl, whr, diag},
  certificates = Lookup[certification, "Certificates", {}];
  If[certificates === {},
    Return[<|
      "Scaling" -> Missing["NoDissectionCertificate"],
      "ScalingStatus" -> "NoDissectionCertificate",
      "ScalingStatusMessage" -> "No exact source-aware dissection certificate",
      "ScalingSearchCompleteQ" ->
        TrueQ[Lookup[certification, "SearchCompleteQ", False]],
      "ProvisionalCoverageScalingData" -> coverage
    |>]
  ];
  selected = First[certificates];
  pullbacks = Lookup[certificates, "PullbackScaling", <||>];
  vectors = (Lookup[#, vars, Missing["IncompletePullbackScaling"]] &) /@ pullbacks;
  selectedPullback = Lookup[selected, "PullbackScaling", <||>];
  selectedVector = Lookup[selectedPullback, vars,
    Missing["IncompletePullbackScaling"]];
  wsl = Lookup[selected, "WSL", Missing["NotAvailable"]];
  whr = Lookup[selected, "ResolvedLeadingWeight", Missing["NotAvailable"]];
  diag = <|
    "ScalingVector" -> selectedVector,
    "VariableScaling" -> selectedPullback,
    "WSL" -> wsl,
    "WHR" -> whr,
    "FSLWeight" -> wsl,
    "PostCancellationLeadingWeight" -> whr,
    "HierarchyGapPostLPminusFSL" ->
      Lookup[selected, "CancellationDepth", Missing["NotAvailable"]],
    "HiddenDominatesPostCancellationLPQ" -> True,
    "LeadingRegionCoverageQ" -> True,
    "FSLOnLeadingFacetQ" ->
      Lookup[selected, "FSLOnLeadingFacetQ", Missing["NotAvailable"]]
  |>;
  <|
    "Scaling" -> selectedVector,
    "VariableScaling" -> selectedPullback,
    "AcceptedCount" -> Length[certificates],
    "AcceptedScalingVectors" -> vectors,
    "UniqueAcceptedScalingQ" -> (Length[DeleteDuplicates[vectors]] == 1),
    "ScalingStatus" -> "CertifiedByOrdinaryDissection",
    "ScalingStatusMessage" -> "Certified by exact source-aware dissection",
    "ScalingSearchCompleteQ" ->
      TrueQ[Lookup[certification, "SearchCompleteQ", False]],
    "SelectedCandidateDiagnostic" -> diag,
    "DissectionCertificate" -> selected,
    "ProvisionalCoverageScalingData" -> coverage
  |>
];

hrfApplyOrdinaryDissectionCertification[
    eval_Association, U_, vars_List, fObs_, mode_, dissectionOptions_] := Module[
  {normalizedMode, coverage, coverageQ, eta, components, factors, cert,
   outcome, certifiedScaling, finalScaling, finalHiddenQ, rules},
  normalizedMode = hrfNormalizeOrdinaryDissectionMode[mode];
  coverage = Lookup[eval, "CoverageScalingData",
    Lookup[eval, "ScalingData", <||>]];
  coverageQ = TrueQ[Lookup[eval, "ValidScalingQ", False]];
  If[normalizedMode === "Off",
    Return @ Join[eval, <|
      "CoverageScalingQ" -> coverageQ,
      "DissectionCertificationStatus" -> "Off",
      "CertificationUnresolvedQ" -> False,
      "DissectionSearchIncompleteQ" -> False,
      "CertifiedRegionCount" -> If[coverageQ, 1, 0]
    |>]
  ];
  If[! coverageQ,
    Return @ Join[eval, <|
      "CoverageScalingQ" -> False,
      "DissectionCertificationStatus" -> "NotRunCoverageRejected",
      "CertificationUnresolvedQ" -> False,
      "DissectionSearchIncompleteQ" -> False,
      "CertifiedRegionCount" -> 0
    |>]
  ];
  eta = Unique["hrfOrdinaryDissectionEta$"];
  components = hrfOrdinaryDissectionComponents[eval["Trial"], U, fObs, eta];
  factors = hrfOrdinaryDissectionFactors[eval["Trial"]];
  cert = If[hrfEnsureSourceAwareDissectionLoaded[],
    rules = FilterRules[Flatten[{dissectionOptions}],
      Options[hrfSourceAwareDissectionCertificate]];
    hrfSourceAwareDissectionCertificate[
      components, factors, vars, eta, Sequence @@ rules
    ],
    <|
      "CertificationStatus" -> "SourceAwareDissectionUnavailable",
      "CertifiedQ" -> False,
      "SearchCompleteQ" -> False,
      "Certificates" -> {}
    |>
  ];
  outcome = hrfOrdinaryDissectionOutcome[cert];
  certifiedScaling = hrfCertifiedScalingDataFromDissection[cert, vars, coverage];
  finalHiddenQ = Which[
    normalizedMode === "Diagnostic", coverageQ,
    True, outcome === "Certified"
  ];
  finalScaling = Which[
    outcome === "Certified", certifiedScaling,
    normalizedMode === "Diagnostic", coverage,
    True, certifiedScaling
  ];
  Join[eval, <|
    "ScalingData" -> finalScaling,
    "CoverageScalingData" -> coverage,
    "CertifiedScalingData" -> If[outcome === "Certified", certifiedScaling,
      Missing["NoDissectionCertificate"]],
    "OrdinaryDissectionCertification" -> cert,
    "DissectionCertificationStatus" -> outcome,
    "CertificationUnresolvedQ" -> (outcome === "Unresolved"),
    "DissectionSearchIncompleteQ" ->
      ! TrueQ[Lookup[cert, "SearchCompleteQ", False]],
    "CertificationSearchCompleteQ" ->
      TrueQ[Lookup[cert, "SearchCompleteQ", False]],
    "CertifiedRegionCount" ->
      Lookup[cert, "DistinctCertifiedRegionCount",
        If[outcome === "Certified", 1, 0]],
    "CoverageScalingQ" -> coverageQ,
    "ValidScalingQ" -> finalHiddenQ,
    "HiddenRegionQ" -> finalHiddenQ
  |>]
];

hrfHiddenRegionScanFromTrial[trial_, scalingData_, baseMeta_Association,
    hiddenQ_:Automatic, certification_:<||>, coverageData_:Automatic] := Join[
  baseMeta,
  ...
  <|
    ...
    "CoverageScalingData" -> hrfScalingDataAssoc @ Replace[coverageData,
      Automatic -> scalingData],
    "CertifiedScalingData" -> hrfScalingDataAssoc[scalingData],
    "OrdinaryDissectionCertification" -> certification,
    "HiddenRegionQ" -> If[hiddenQ === Automatic,
      hrfTrialCoverageFoundQ[scalingData], TrueQ[hiddenQ]],
    "GeneratorUseData" -> If[AssociationQ[Lookup[trial, "ObstructionData", Missing[]]] &&
      ...
      generatorUseData[trial["ObstructionData", "Superleading"], trial["Generators"],
        ...
        Lookup[baseMeta, "ObstructionIdealContext", <||>]],
      Missing["NoUseData"]
    ]
  |>
];
    ...
    Module[{diag = If[AssociationQ[cov] && Length[DownValues[hrfSelectedDiagnostic]] > 0,
      ...
      <|
        ...
        "ScalingStatus" -> Which[
          ...
          MatchQ[ev, _Missing], "Not evaluated (no U or scaling skipped)",          ! AssociationQ[cov] || cov === <||>, "Not evaluated (no U or scaling skipped)",
...
ClearAll[
  hrfNormalizeAsymptoticOrderAlignmentMode, hrfFindObstructionsAsymptoticOrderAlignmentOptionNames,
  hrfFindObstructionsLegacyFaceliftOptionNames, hrfResolveAlignmentOption,
  hrfAddLegacyFaceliftResultKeys,
  hrfFindObstructionsBaseOptionRules, hrfRunAsymptoticOrderAlignmentFromFindObstructions,
  hrfMergeAsymptoticOrderAlignmentIntoFindObstructions
];

hrfNormalizeAsymptoticOrderAlignmentMode[mode_] := Which[
  TrueQ[mode === True], "Fallback",
  TrueQ[mode === False] || MemberQ[{None, "Off", Off}, mode], "Off",
  MemberQ[{"Fallback", "Always", "Only"}, mode], mode,
  True, "Off"
];

hrfFindObstructionsAsymptoticOrderAlignmentOptionNames[] := {
  "AsymptoticOrderAlignmentMode", "AsymptoticOrderAlignmentPolynomial", "AsymptoticOrderAlignmentOptions"
};

(* Deprecated compatibility names.  New code should use the
   AsymptoticOrderAlignment option family. *)
hrfFindObstructionsLegacyFaceliftOptionNames[] := {
  "FaceliftMode", "FaceliftPolynomial", "FaceliftOptions"
};

hrfResolveAlignmentOption[optionRules_List, canonical_String, legacy_String, default_] :=
 Module[{assoc = Association[optionRules]},
  Which[
   KeyExistsQ[assoc, canonical], assoc[canonical],
   KeyExistsQ[assoc, legacy], assoc[legacy],
   True, default
  ]
 ];

hrfAddLegacyFaceliftResultKeys[result_Association] := Join[
  result,
  <|
    "FaceliftHiddenRegionQ" -> Lookup[
      result, "AsymptoticOrderAlignmentHiddenRegionQ", False
    ],
    "FaceliftAcceptedPresentationCount" -> Lookup[
      result, "AsymptoticOrderAlignmentAcceptedPresentationCount", 0
    ],
    "FaceliftUniqueHiddenRegionCount" -> Lookup[
      result, "AsymptoticOrderAlignmentUniqueHiddenRegionCount", 0
    ],
    "FaceliftScan" -> Lookup[
      result, "AsymptoticOrderAlignmentScan", Missing["NotAvailable"]
    ],
    "FaceliftStatus" -> Lookup[
      result, "AsymptoticOrderAlignmentStatus", Missing["NotAvailable"]
    ]
  |>
];

hrfFindObstructionsBaseOptionRules[optionRules_List] := Select[
  FilterRules[optionRules, Options[findObstructions]],
  ! MemberQ[
    Join[
      hrfFindObstructionsAsymptoticOrderAlignmentOptionNames[],
      hrfFindObstructionsLegacyFaceliftOptionNames[],
...
hrfRunAsymptoticOrderAlignmentFromFindObstructions[
    F_, vars_, kinAssumptions_, kinVars_, UOpt_, optionRules_List] := Module[
  {assoc, poly, userOptions, userAssoc, userHRFOptions, passOptions,
   hrfOptions, alignmentU, scan, summary},
  If[Length[DownValues[hrfAsymptoticOrderAlignmentSearch]] == 0,
    Quiet @ Check[Get[FileNameJoin[{hrfPackageDirectory[], "HRF_AsymptoticOrderAlignment.wl"}]], Null]
  ];
  If[Length[DownValues[hrfAsymptoticOrderAlignmentSearch]] == 0,
    Return[hrfAddLegacyFaceliftResultKeys @ <|
      "AsymptoticOrderAlignmentHiddenRegionQ" -> False,
      "AsymptoticOrderAlignmentScan" -> Missing["AsymptoticOrderAlignmentLayerUnavailable"],
      "AsymptoticOrderAlignmentStatus" -> "Unavailable"
    |>]
  ];
  assoc = Association[optionRules];
  poly = Replace[
    hrfResolveAlignmentOption[
      optionRules, "AsymptoticOrderAlignmentPolynomial", "FaceliftPolynomial", Automatic
    ],
    Automatic -> F
  ];
  userOptions = Flatten @ {
    hrfResolveAlignmentOption[
      optionRules, "AsymptoticOrderAlignmentOptions", "FaceliftOptions", {}
    ]
  };
  userAssoc = Association[userOptions];
  userHRFOptions = Flatten[{Lookup[userAssoc, "HRFOptions", {}]}];
  alignmentU = Lookup[userAssoc, "U", UOpt];
  passOptions = DeleteCases[userOptions,
    ("HRFOptions" -> _) | ("HRFOptions" :> _) | ("U" -> _) | ("U" :> _)
  ];
  hrfOptions = Join[userHRFOptions, hrfFindObstructionsBaseOptionRules[optionRules]];
  scan = Quiet @ Check[
    hrfAsymptoticOrderAlignmentSearch[
      poly,
      vars,
      "KinematicAssumptions" -> kinAssumptions,
      "KinematicVariables" -> kinVars,
      "U" -> alignmentU,
      "HRFOptions" -> hrfOptions,
      Sequence @@ passOptions
    ],
    $Failed
  ];
  If[! AssociationQ[scan],
    Return[hrfAddLegacyFaceliftResultKeys @ <|
      "AsymptoticOrderAlignmentHiddenRegionQ" -> False,
      "AsymptoticOrderAlignmentScan" -> scan,
      "AsymptoticOrderAlignmentStatus" -> "Failed"
    |>]
  ];
  summary = Lookup[scan, "Summary", <||>];
  hrfAddLegacyFaceliftResultKeys @ <|
    "AsymptoticOrderAlignmentHiddenRegionQ" -> TrueQ[Lookup[summary, "HiddenRegionCount", 0] > 0],
    "AsymptoticOrderAlignmentAcceptedPresentationCount" -> Lookup[summary, "HiddenRegionCount", 0],
    "AsymptoticOrderAlignmentUniqueHiddenRegionCount" -> Lookup[summary, "UniqueHiddenRegionCount", 0],
    "AsymptoticOrderAlignmentScan" -> scan,
    "AsymptoticOrderAlignmentStatus" -> "Done"
  |>
];

hrfMergeAsymptoticOrderAlignmentIntoFindObstructions[standard_Association, mode_, alignment_Association] := Module[
  {stdQ, alignmentQ, stdCount, alignmentUnique},
  stdQ = TrueQ[Lookup[standard, "HiddenRegionQ", False]];
  alignmentQ = TrueQ[Lookup[alignment, "AsymptoticOrderAlignmentHiddenRegionQ", False]];
  stdCount = Lookup[standard, "HiddenRegionCount", 0];
  alignmentUnique = Lookup[alignment, "AsymptoticOrderAlignmentUniqueHiddenRegionCount", 0];
  Join[
    standard,
    <|
      "AsymptoticOrderAlignmentMode" -> mode,
      "FaceliftMode" -> mode,
      "StandardHiddenRegionQ" -> stdQ,
      "StandardHiddenRegionCount" -> stdCount,
      "CombinedHiddenRegionQ" -> (stdQ || alignmentQ)
    |>,
    alignment,
    <|
      "HiddenRegionQ" -> (stdQ || alignmentQ),
      "HiddenRegionCount" -> stdCount + alignmentUnique
    |>
  ]
];

(* Deprecated internal aliases retained for notebooks that called the former
   wrapper helpers directly. *)
hrfNormalizeFaceliftMode[args___] :=
  hrfNormalizeAsymptoticOrderAlignmentMode[args];
hrfFindObstructionsFaceliftOptionNames[args___] :=
  hrfFindObstructionsLegacyFaceliftOptionNames[args];
hrfRunFaceliftFromFindObstructions[args___] :=
  hrfRunAsymptoticOrderAlignmentFromFindObstructions[args];
hrfMergeFaceliftIntoFindObstructions[args___] :=
  hrfMergeAsymptoticOrderAlignmentIntoFindObstructions[args];

findObstructions::badgensetoverride =
  "GeneratorSetOverride must be a list of nonempty generator lists; received `1`.";

Options[findObstructions] = {
   "GeneratorMode" -> "PairSectors",
   "MaxGenerators" -> 2,
   "MaxProductSubsetSize" -> Automatic,
   "UseGeneratorCompletionQ" -> Automatic,
   "UseExtendedFactors" -> False,
   "DerivativeFactorHarvestMode" -> "Off",
   "FullGradientSaturationJustifiedQ" -> False,
   "FactorHarvestDerivativePolynomials" -> Automatic,
   "FactorHarvestKinematicConstraints" -> {},
   "FactorHarvestNonzeroKinematicFactors" -> {},
   "FactorHarvestTimeLimit" -> 30,
   "FactorHarvestMaxBasisSize" -> Automatic,
   "FactorHarvestMaxXDegree" -> Automatic,
   "AllowKinematicFactorPairsQ" -> Automatic,
   "CancellationFactorOverride" -> Automatic,
   "GeneratorSetOverride" -> Automatic,
   ...
   "MaxTwoGeneratorUnionTrials" -> Inherited,
   "PolynomialMaxMonomials" -> Inherited,
   "EnableSignedMonomialPairs" -> Inherited,
   ...
   "OrdinaryDissectionMode" -> "Required",
   "OrdinaryDissectionOptions" -> {},
   "RequireValidScalingForHiddenRegionQ" -> Automatic,
   "EnumerateHiddenRegionsQ" -> Automatic,
   "AsymptoticOrderAlignmentMode" -> "Off",
   "AsymptoticOrderAlignmentPolynomial" -> Automatic,
   "AsymptoticOrderAlignmentOptions" -> {},
   "FaceliftMode" -> Automatic,
   "FaceliftPolynomial" -> Automatic,
   "FaceliftOptions" -> Automatic
   };
...
findObstructionsDebug[F_, vars_, kinAssumptions_, kinVars_, maxSize_ : Automatic,
 ...
 <|
  ...
  "DerivativeFactorHarvestMode" -> OptionValue["DerivativeFactorHarvestMode"],
  "FullGradientSaturationJustifiedQ" ->
    OptionValue["FullGradientSaturationJustifiedQ"],
...
findObstructions[F_, vars_, kinAssumptions_, kinVars_, maxSize_ : Automatic,
 ...
 Module[{factorData, safeFactors, factorsByDerivative, appearsIn,
   ...
   unionLimit, polynomialMaxMonomials, factorHarvestAudit, generatorConstructionAudit,
   effectiveSearchConfiguration, searchTruncatedQ, polynomialLimitReachedQ,
   hiddenRegionSearchCompleteQ, enableSignedMonomialPairs,
   UOpt, maxScalingAbs, fObsScaling, coverageScalingMethod,
   ordinaryDissectionMode, ordinaryDissectionOptions, enumerateQ, requireScalingQ,
   scalingEvaluations, hiddenRegionTrials, hiddenRegionScans, baseScanMeta,
   generatorSetScalingSummary, completionData, completionGeneratorSets, completionFactors,
   optionRules, alignmentMode, result, alignment,
   factorDomainUnresolvedCount, kinDomainUnresolvedCount,
   generatorSetOverride, generalizedHarvestMode, generalizedHarvest,
   generalizedHarvestFactors, generalizedHarvestSafeFactors,
   generalizedHarvestTruncatedQ, allowKinematicFactorPairsQ,
   factorHarvestMaxXDegree, factorHarvestDerivativePolynomials,
   fullGradientSaturationJustifiedQ, generalizedHarvestApplicabilityQ,
   idealContext, coefficientFieldIdealQ},

  (* A finite FindInstance timeout is an unresolved decision, never evidence
     that a positive-domain solution is absent.  The low-level predicates
     retain Boolean compatibility for existing callers but increment this
     audit counter; completion below is denied whenever it is nonzero. *)
  ...
  unionLimit = Which[
    OptionValue["MaxTwoGeneratorUnionTrials"] === Inherited,
      If[ValueQ[$HRFMaxTwoGeneratorUnionTrials], $HRFMaxTwoGeneratorUnionTrials, Infinity],
    True, OptionValue["MaxTwoGeneratorUnionTrials"]
  ];
  polynomialMaxMonomials = Which[
    OptionValue["PolynomialMaxMonomials"] === Inherited,
      If[ValueQ[$HRFPolynomialMaxMonomials], $HRFPolynomialMaxMonomials, Automatic],
    True, OptionValue["PolynomialMaxMonomials"]
  ];
  enableSignedMonomialPairs = Which[
    OptionValue["EnableSignedMonomialPairs"] === Inherited,
      If[ValueQ[$HRFPolynomialEnableSignedMonomialPairs],
        TrueQ[$HRFPolynomialEnableSignedMonomialPairs], False],
    True, TrueQ[OptionValue["EnableSignedMonomialPairs"]]
  ];
  ...
  coverageScalingMethod = OptionValue["CoverageScalingMethod"];
  ordinaryDissectionMode = hrfNormalizeOrdinaryDissectionMode[
    OptionValue["OrdinaryDissectionMode"]
  ];
  ordinaryDissectionOptions = OptionValue["OrdinaryDissectionOptions"];
  optionRules = Flatten[{opts}];
  alignmentMode = hrfNormalizeAsymptoticOrderAlignmentMode @
    hrfResolveAlignmentOption[
      optionRules, "AsymptoticOrderAlignmentMode", "FaceliftMode", "Off"
    ];
  If[alignmentMode === "Only",
    alignment = hrfRunAsymptoticOrderAlignmentFromFindObstructions[
      F, vars, kinAssumptions, kinVars, UOpt, optionRules
    ];
    Return @ hrfMergeAsymptoticOrderAlignmentIntoFindObstructions[
      <|
        "HiddenRegionQ" -> False,
        "HiddenRegionCount" -> 0,
        "StandardScanSkippedQ" -> True,
        "StandardScanSkipReason" -> "AsymptoticOrderAlignmentMode=Only",
        "ActiveVars" -> vars,
        "KinVars" -> kinVars,
        "U" -> UOpt
      |>,
  ...
  degreeOpts = <|
    ...
    "MaxTwoGeneratorUnionTrials" -> unionLimit,
    "UseGeneratorSectorQuotientQ" ->
      TrueQ[OptionValue["UseGeneratorSectorQuotientQ"]],
    "MaxGenerators" -> OptionValue["MaxGenerators"],
    ...
    "UseGeneratorCompletionQ" -> Which[
      OptionValue["UseGeneratorCompletionQ"] === Automatic,
        If[ValueQ[$HRFUseGeneratorCompletionQ], TrueQ[$HRFUseGeneratorCompletionQ], False],
      True, TrueQ[OptionValue["UseGeneratorCompletionQ"]]
    ],
  ...
  generatorSetOverride = OptionValue["GeneratorSetOverride"];
  generalizedHarvestMode = OptionValue["DerivativeFactorHarvestMode"];
  factorHarvestDerivativePolynomials =
    OptionValue["FactorHarvestDerivativePolynomials"];
  fullGradientSaturationJustifiedQ =
    TrueQ[OptionValue["FullGradientSaturationJustifiedQ"]];
  coefficientFieldIdealQ = MemberQ[
    {"SaturatedLeadingIdeal", "CandidateLeadingIdealSaturation",
     "SaturatedIdeal", "SaturatedDerivativeIdeal", "DirectThenSaturated"},
    generalizedHarvestMode
  ];
  generalizedHarvestApplicabilityQ = coefficientFieldIdealQ && Or[
    fullGradientSaturationJustifiedQ,
    factorHarvestDerivativePolynomials =!= Automatic
  ];
  idealContext = If[coefficientFieldIdealQ,
    <|
      "Mode" -> "KinematicCoefficientField",
      "Constraints" -> OptionValue["FactorHarvestKinematicConstraints"],
      "NonzeroKinematicFactors" ->
        OptionValue["FactorHarvestNonzeroKinematicFactors"]
    |>,
    <|"Mode" -> "PolynomialRing"|>
  ];
  factorHarvestMaxXDegree = OptionValue["FactorHarvestMaxXDegree"];
  allowKinematicFactorPairsQ = Which[
    OptionValue["AllowKinematicFactorPairsQ"] === Automatic,
      coefficientFieldIdealQ,
    True, TrueQ[OptionValue["AllowKinematicFactorPairsQ"]]
  ];
  ...
  factorData = hrfDebugTimed["findObstructions/factor finder",
    Block[{$HRFPolynomialMaxMonomials = polynomialMaxMonomials,
        $HRFPolynomialEnableSignedMonomialPairs = enableSignedMonomialPairs},
     Module[{override = OptionValue["CancellationFactorOverride"], dimSpec, data},
      ...
      factorHarvestAudit = If[override =!= Automatic && ListQ[override],
        <|"CancellationFactorOverrideQ" -> True,
          "AcceptedCount" -> Length[DeleteDuplicates[override]]|>,
  ...
  (* Optional, candidate-specific algebraic completion.  It is valid only
     when F is already the selected leading cancellation sector, or when the
     caller supplies the selected derivative-sector relations explicitly.
     It must not be used as a full-gradient fallback on a starting polynomial
     that may still contain an obstruction. *)
  generalizedHarvest = Which[
    coefficientFieldIdealQ && ! generalizedHarvestApplicabilityQ,
      <|
        "Status" -> "ApplicabilityNotEstablished", "Factors" -> {},
        "BasisSize" -> 0, "SearchTruncatedQ" -> True,
        "RequiredCondition" ->
          "Set FullGradientSaturationJustifiedQ->True only when the supplied polynomial is the selected leading cancellation sector, or supply FactorHarvestDerivativePolynomials."
      |>,
    coefficientFieldIdealQ,
      hrfDebugTimed["findObstructions/saturated derivative-factor harvest",
        hrfSaturatedDerivativeFactorHarvest[
          F, vars, kinVars,
          OptionValue["FactorHarvestKinematicConstraints"],
          OptionValue["FactorHarvestNonzeroKinematicFactors"],
          OptionValue["FactorHarvestTimeLimit"],
          OptionValue["FactorHarvestMaxBasisSize"],
          factorHarvestDerivativePolynomials
        ]
      ],
    MemberQ[{"Off", None, Automatic}, generalizedHarvestMode],
      <|
        "Status" -> "Off", "Factors" -> {}, "BasisSize" -> 0,
        "SearchTruncatedQ" -> False
      |>,
    True,
      <|
        "Status" -> "UnsupportedMode", "RequestedMode" -> generalizedHarvestMode,
        "Factors" -> {}, "BasisSize" -> 0, "SearchTruncatedQ" -> True
      |>
  ];
  generalizedHarvestFactors = Lookup[generalizedHarvest, "Factors", {}];
  If[IntegerQ[factorHarvestMaxXDegree] && factorHarvestMaxXDegree >= 0,
    generalizedHarvestFactors = Select[
      generalizedHarvestFactors,
      hrfPolynomialTotalDegreeInVars[#, vars] <= factorHarvestMaxXDegree &
    ]
  ];
  generalizedHarvestSafeFactors = Select[
    generalizedHarvestFactors,
    positiveCompatibleQ[#, vars, kinAssumptions, kinVars] &
  ];
  generalizedHarvest = Join[generalizedHarvest, <|
    "DomainCompatibleFactors" -> generalizedHarvestSafeFactors,
    "DomainCompatibleFactorCount" -> Length[generalizedHarvestSafeFactors],
    "MaxXDegreeFilter" -> factorHarvestMaxXDegree,
    "AfterXDegreeFilterCount" -> Length[generalizedHarvestFactors],
    "RejectedByDomainCount" ->
      Length[generalizedHarvestFactors] - Length[generalizedHarvestSafeFactors]
  |>];
  If[generalizedHarvestSafeFactors =!= {},
    safeFactors = DeleteDuplicates @ Join[safeFactors, generalizedHarvestSafeFactors];
    factorsByDerivative = Join[
      factorsByDerivative,
      <|"SaturatedDerivativeIdeal" -> generalizedHarvestSafeFactors|>
    ];
  ];
  generalizedHarvestTruncatedQ =
    TrueQ[Lookup[generalizedHarvest, "SearchTruncatedQ", False]];
  If[AssociationQ[factorHarvestAudit],
    factorHarvestAudit = Append[
      factorHarvestAudit, "GeneralizedDerivativeHarvest" -> generalizedHarvest
    ]
  ];
  factorDomainUnresolvedCount = If[ValueQ[$HRFKinDomainUnresolvedCount],
    $HRFKinDomainUnresolvedCount, 0];
  If[AssociationQ[factorHarvestAudit],
    factorHarvestAudit = Append[factorHarvestAudit,
      "UnresolvedKinematicDomainDecisionCount" -> factorDomainUnresolvedCount]
  ];
  ...
  $HRFLastCandidateGeneratorConstructionAudit = <||>;
  generatorSets = If[generatorSetOverride =!= Automatic,
    If[ListQ[generatorSetOverride] && generatorSetOverride =!= {} &&
        And @@ (ListQ[#] && # =!= {} & /@ generatorSetOverride),
      DeleteDuplicates[Expand /@ # & /@ generatorSetOverride],
      Message[findObstructions::badgensetoverride, generatorSetOverride]; {}
    ],
    Block[{$HRFAllowKinematicFactorPairsQ = allowKinematicFactorPairsQ},
      hrfDebugTimed["findObstructions/generator-set construction",
  ...
  generatorConstructionAudit = If[generatorSetOverride =!= Automatic,
    <|"GeneratorSetOverrideQ" -> True,
      "GeneratorSetOverrideCount" -> Length[generatorSets],
      "SearchTruncatedQ" -> False|>,
    If[ValueQ[$HRFLastCandidateGeneratorConstructionAudit],
      $HRFLastCandidateGeneratorConstructionAudit, <||>]
  ];
  completionData = If[TrueQ[Lookup[degreeOpts, "UseGeneratorCompletionQ", False]],
    hrfDebugTimed["findObstructions/generator completion",
      hrfCompletedGeneratorSets[F, safeFactors, vars, kinAssumptions, kinVars, degreeBounds]
    ],
    <|"GeneratorSets" -> {}, "CompletedFactors" -> {}, "Rows" -> {}|>
  ];
  completionGeneratorSets = Lookup[completionData, "GeneratorSets", {}];
  completionFactors = Lookup[completionData, "CompletedFactors", {}];
  If[completionGeneratorSets =!= {},
    safeFactors = DeleteDuplicates @ Join[safeFactors, completionFactors];
    generatorSets = DeleteDuplicates @ Join[generatorSets, completionGeneratorSets];
  ];
   ...
   Do[
     ...
     Module[{obsDiag, obs, setData, slData, perGenQ, setUnionQ, admissibleQ, attemptData, trial},
      ...
      obsDiag = If[perGenQ,
        hrfDebugTimed["findObstructions/trial " <> ToString[i] <> " obstruction test",
          If[coefficientFieldIdealQ,
            hrfObstructionByKinematicCoefficientFieldDiagnostic[
              F, generatorSets[[i]], vars, kinVars,
              OptionValue["FactorHarvestKinematicConstraints"]
            ],
            obstructionByOriginalTermsGeneralDiagnostic[
              F, generatorSets[[i]], vars, kinVars, maxSize, kinAssumptions
            ]
          ]
        ],
        <|
      ...
      slData = If[hrfValidObstructionResultQ[obs, generatorSets[[i]], vars, kinVars],
        slSectorAdmissibilityData[
          obs["Superleading"], generatorSets[[i]], safeFactors, vars,
          kinVars, kinAssumptions, degreeBounds, F, idealContext
        ],
        <|
         "SLSectorGenerators" -> {},
         "SLSectorFactorUnion" -> {},
      ...
      If[! storeAllTrialsQ, ClearSystemCache[]];
     ],
     {j, Length[trialOrder]}
    ];
    If[stopOnFirstQ && validAtLen =!= {},
     ...
     hrfDebugSay["findObstructions: stop after |gens|=" <> ToString[len] <>
       ...
       If[preferFewerQ, "all-in-ideal, min |gens|, min obstruction", "max |gens|, min obstruction"]];
     Break[]
    ];
    ,
  ...
  scalingEvaluations = If[enumerateQ && validTrials =!= {} && ! MatchQ[UOpt, None | Automatic | Missing],
    hrfApplyOrdinaryDissectionCertification[
      hrfEvaluateValidTrialScaling[
        #, UOpt, vars, maxScalingAbs, fObsScaling, coverageScalingMethod
      ],
      UOpt, vars, fObsScaling, ordinaryDissectionMode,
      ordinaryDissectionOptions
    ] & /@ validTrials,
    {}
  ];
  ...
  baseScanMeta = <|
    ...
    "ActiveVars" -> vars,
    "KinVars" -> kinVars,
    "ObstructionIdealContext" -> idealContext
  |>;
  hiddenRegionScans = If[hiddenRegionTrials =!= {},
    Flatten[Table[
      Module[{certification, certificates, singleCertification,
         singleScaling, coverage},
        certification = Lookup[eval,
          "OrdinaryDissectionCertification", <||>];
        certificates = Lookup[certification, "Certificates", {}];
        coverage = Lookup[eval, "CoverageScalingData", eval["ScalingData"]];
        If[ordinaryDissectionMode === "Required" && certificates =!= {},
          Table[
            singleCertification = Join[certification, <|
              "Certificates" -> {certificate},
              "DistinctCertifiedRegionCount" -> 1,
              "MultipleCertifiedRegionsQ" -> False
            |>];
            singleScaling = hrfCertifiedScalingDataFromDissection[
              singleCertification, vars, coverage
            ];
            Join[
              hrfHiddenRegionScanFromTrial[
                eval["Trial"], singleScaling, baseScanMeta,
                eval["ValidScalingQ"], singleCertification, coverage
              ],
              <|"SelectedDissectionCertificate" -> certificate|>
            ],
            {certificate, certificates}
          ],
          {hrfHiddenRegionScanFromTrial[
            eval["Trial"], eval["ScalingData"], baseScanMeta,
            eval["ValidScalingQ"], certification, coverage
          ]}
        ]
      ],
  ...
  best = Which[
    requireScalingQ && hiddenRegionTrials =!= {},
      Module[{ranked, ev, trial},
        ...
        trial = ev["Trial"];
        Join[trial, <|
          "CoverageScalingData" -> hrfScalingDataAssoc @
            Lookup[ev, "CoverageScalingData", ev["ScalingData"]],
          "CertifiedScalingData" -> hrfScalingDataAssoc @ ev["ScalingData"],
          "OrdinaryDissectionCertification" ->
            Lookup[ev, "OrdinaryDissectionCertification", <||>],
          "DissectionCertificationStatus" ->
            Lookup[ev, "DissectionCertificationStatus", "Off"],
          "CertificationUnresolvedQ" ->
            TrueQ[Lookup[ev, "CertificationUnresolvedQ", False]],
    ...
    requireScalingQ && ! MatchQ[UOpt, None | Automatic | Missing] && validTrials =!= {} && hiddenRegionTrials === {},
      <|
        ...
        "ObstructionData" -> Missing["NoHiddenRegionWithValidScaling",
          "Valid obstruction(s) found but none obtained the required scaling and dissection certificate"],
        "ObstructionAttemptData" -> <||>,
        "CoverageScalingData" -> Missing["NoHiddenRegionWithValidScaling"],
        "CertifiedScalingData" -> Missing["NoDissectionCertificate"],
        "OrdinaryDissectionCertification" -> FirstCase[
          scalingEvaluations,
          e_Association /; KeyExistsQ[e, "OrdinaryDissectionCertification"] :>
            e["OrdinaryDissectionCertification"],
          <||>
        ],
        "DissectionCertificationStatus" -> If[
          AnyTrue[scalingEvaluations,
            TrueQ[Lookup[#, "CertificationUnresolvedQ", False]] &],
          "Unresolved",
          If[MemberQ[Lookup[scalingEvaluations,
              "DissectionCertificationStatus", {}], "Rejected"],
            "Rejected", "NotRunCoverageRejected"]
        ],
        "CertificationUnresolvedQ" -> AnyTrue[scalingEvaluations,
          TrueQ[Lookup[#, "CertificationUnresolvedQ", False]] &]
      |>,
    validTrials =!= {},
      Module[{ranked, eval},
        ...
        If[AssociationQ[eval] && KeyExistsQ[eval, "ScalingData"] &&
          ...
          Join[ranked, <|
            "CoverageScalingData" -> hrfScalingDataAssoc @
              Lookup[eval, "CoverageScalingData", eval["ScalingData"]],
            "CertifiedScalingData" -> hrfScalingDataAssoc @ eval["ScalingData"],
            "OrdinaryDissectionCertification" ->
              Lookup[eval, "OrdinaryDissectionCertification", <||>],
            "DissectionCertificationStatus" ->
              Lookup[eval, "DissectionCertificationStatus", "NotEvaluated"],
            "CertificationUnresolvedQ" ->
              TrueQ[Lookup[eval, "CertificationUnresolvedQ", False]],
  ...
  use = If[AssociationQ[best["ObstructionData"]] &&
    ...
    generatorUseData[
      best["ObstructionData", "Superleading"], best["Generators"],
      vars, kinVars, idealContext
    ],
    Missing["NoUseData"]
    ];

  polynomialLimitReachedQ = IntegerQ[polynomialMaxMonomials] &&
    AnyTrue[Lookup[factorHarvestAudit, "AuditRows", {}],
      Lookup[#, "MonomialCount", 0] > polynomialMaxMonomials &];
  limitReachedQ = generatorSetOverride === Automatic && (
    (IntegerQ[genLimit] && genLimit < Infinity &&
      Length[generatorSets] >= genLimit) ||
    TrueQ[Lookup[generatorConstructionAudit,
      "CandidateGeneratorSetLimitReachedQ", False]]);
  searchTruncatedQ = Or[
    limitReachedQ,
    polynomialLimitReachedQ,
    generalizedHarvestTruncatedQ,
    TrueQ[Lookup[generatorConstructionAudit, "SearchTruncatedQ", False]],
    (kinDomainUnresolvedCount = If[ValueQ[$HRFKinDomainUnresolvedCount],
      $HRFKinDomainUnresolvedCount, 0]) > 0
  ];
  effectiveSearchConfiguration = <|
    "CandidateGeneratorSetLimit" -> genLimit,
    "MaxTwoGeneratorUnionTrials" -> unionLimit,
    "PolynomialMaxMonomials" -> polynomialMaxMonomials,
    "EnableSignedMonomialPairs" -> enableSignedMonomialPairs,
    "KinDomainFindInstanceTimeLimit" ->
      If[ValueQ[$HRFKinDomainFindInstanceTimeLimit],
        $HRFKinDomainFindInstanceTimeLimit, 5],
    "ObstructionFindInstanceTimeLimit" ->
      If[ValueQ[$HRFObstructionFindInstanceTimeLimit],
        $HRFObstructionFindInstanceTimeLimit, 20],
    "MaxGenerators" -> OptionValue["MaxGenerators"],
    "DerivativeFactorHarvestMode" -> generalizedHarvestMode,
    "FullGradientSaturationJustifiedQ" ->
      fullGradientSaturationJustifiedQ,
    "SelectedDerivativeRelationsSuppliedQ" ->
      (factorHarvestDerivativePolynomials =!= Automatic),
    "SaturatedIdealApplicabilityEstablishedQ" ->
      generalizedHarvestApplicabilityQ,
    "ObstructionIdealMode" -> Lookup[idealContext, "Mode", "PolynomialRing"],
    "FactorHarvestKinematicConstraintCount" ->
      Length[Flatten[{OptionValue["FactorHarvestKinematicConstraints"]}]],
    "FactorHarvestNonzeroKinematicFactorCount" ->
      Length[Flatten[{OptionValue["FactorHarvestNonzeroKinematicFactors"]}]],
    "FactorHarvestTimeLimit" -> OptionValue["FactorHarvestTimeLimit"],
    "FactorHarvestMaxBasisSize" -> OptionValue["FactorHarvestMaxBasisSize"],
    "FactorHarvestMaxXDegree" -> factorHarvestMaxXDegree,
    "AllowKinematicFactorPairsQ" -> allowKinematicFactorPairsQ,
    "GeneratorSetOverrideQ" -> (generatorSetOverride =!= Automatic),
    "GeneratorSetOverrideCount" -> If[generatorSetOverride =!= Automatic,
      Length[generatorSets], 0],
    "MaxProductSubsetSize" -> degreeOpts["MaxProductSubsetSize"],
    "MaxScalingAbs" -> maxScalingAbs,
    "CoverageScalingMethod" -> coverageScalingMethod,
    "OrdinaryDissectionMode" -> ordinaryDissectionMode,
    "OrdinaryDissectionOptions" -> ordinaryDissectionOptions,
    "StopOnFirstAdmissible" -> stopOnFirstQ
  |>;
  (* An exhaustive empty generator set is a completed algebraic exclusion,
     not an unresolved search: there are then no F_SL+F_obs decompositions
     to test.  The truncation audit is what distinguishes that case from an
     empty result caused by a search cap. *)
  allCandidatesTriedQ = Length[trials] === Length[generatorSets] &&
    ! searchTruncatedQ;
  hiddenRegionSearchCompleteQ = allCandidatesTriedQ &&
    If[validTrials === {}, True,
      requireScalingQ && Length[scalingEvaluations] === Length[validTrials] &&
        And @@ (TrueQ[Lookup[
          hrfScalingDataAssoc @ Lookup[#, "ScalingData", <||>],
  ...
  result = <|
   ...
   "CandidateGeneratorCount" -> Length[generatorSets],
   "CompletedGeneratorCount" -> Length[completionGeneratorSets],
   "CompletedGeneratorSets" -> completionGeneratorSets,
   "CompletedCancellationFactors" -> completionFactors,
   "CompletedGeneratorRows" -> Lookup[completionData, "Rows", {}],
   "CandidateGeneratorSetLimit" -> genLimit,
   "CandidateGeneratorSetLimitReachedQ" -> limitReachedQ,
   "EffectiveSearchConfiguration" -> effectiveSearchConfiguration,
   "GeneratorConstructionAudit" -> generatorConstructionAudit,
   "PolynomialFactorHarvestAudit" -> factorHarvestAudit,
   "GeneralizedDerivativeHarvestAudit" -> generalizedHarvest,
   "PolynomialMaxMonomialsLimitReachedQ" -> polynomialLimitReachedQ,
   "UnresolvedPositivityCount" -> kinDomainUnresolvedCount,
   "UnresolvedKinematicDomainDecisionCount" -> kinDomainUnresolvedCount,
   ...
   "HiddenRegionSearchCompleteQ" -> hiddenRegionSearchCompleteQ,
   ...
   "NoHiddenRegionWithinSearchBoundsQ" ->
     (hiddenRegionScans === {} && hiddenRegionSearchCompleteQ),
   ...
   "CertifiedScalingData" -> Lookup[best, "CertifiedScalingData", Missing["NotEvaluated"]],
   "OrdinaryDissectionCertification" ->
     Lookup[best, "OrdinaryDissectionCertification", <||>],
   "OrdinaryDissectionCertifications" ->
     Lookup[scalingEvaluations, "OrdinaryDissectionCertification", {}],
   "DissectionCertificationStatus" ->
     Lookup[best, "DissectionCertificationStatus", "NotEvaluated"],
   "CertificationUnresolvedQ" ->
     TrueQ[Lookup[best, "CertificationUnresolvedQ", False]],
   "DissectionSearchIncompleteQ" -> AnyTrue[scalingEvaluations,
     TrueQ[Lookup[#, "DissectionSearchIncompleteQ", False]] &],
   ...
   |>;
  If[alignmentMode === "Always" ||
      (alignmentMode === "Fallback" && ! TrueQ[Lookup[result, "HiddenRegionQ", False]]),
    alignment = hrfRunAsymptoticOrderAlignmentFromFindObstructions[
      F, vars, kinAssumptions, kinVars, UOpt, optionRules
    ];
    hrfMergeAsymptoticOrderAlignmentIntoFindObstructions[result, alignmentMode, alignment],
    result
  ]
  ];
...
ClearAll[hrfPolynomialTerms, monomialWeight, leadingWeight, allTermsSameWeightQ,
  leadingTerms, leadingVariables, scalingDiagnostic,
  coverageScalingQ, findCoverageLPScaling, findCoverageLPScalingBounded, findMinimalLPScaling,
  polynomialExponentRows, leadingRowsFromExponentRows, variablesFromExponentRows,
  hrfWeightedPostLayerData, hrfWeightedRowsFromPolynomial, hrfWeightedRowsFromLayers,
  hrfAsWeightedRows, hrfWeightedRow, hrfWeightedRowWeight, hrfWeightedRowWeightExpr,
  hrfRowsAtWeightedWeight, hrfWeightedRowsSupportPositions,
  hrfWeightedRowsCoverVariables, hrfPostCancellationWeightedRows,
  hrfDeltaLayerAssociation, hrfPostCancellationLayerInput,
  coverageConstraintDiagnosticFromRows, coverageFailureList];

hrfPolynomialTerms[p_] := Module[{expanded = Expand[p]},
  Which[
    TrueQ[expanded === 0], {},
...
scalingDiagnostic[fSL_, U_, vars_, v_, fObs_ : None] := Module[
  {fSLList, fObsLayers, wSL, wU, wObs, wLP, lpTerms,
   lpVars, fSLVars, fSLTerms, UTerms, fObsTermData, postRows,
   postWeights, postRowsAtLP},
  fSLList = DeleteCases[Flatten[{fSL}], 0];
  fObsLayers = hrfWeightedPostLayerData[fObs, "FObs"];
  fSLTerms = If[fSLList === {}, {}, hrfPolynomialTerms[Total[fSLList]]];
  UTerms = hrfPolynomialTerms[U];
  fObsTermData = Flatten[
    Table[
      (<|"Power" -> layer["Power"], "Term" -> #, "Source" -> layer["Label"]|> & /@
        hrfPolynomialTerms[layer["Polynomial"]]),
      {layer, fObsLayers}
    ],
    1
  ];
  postRows = hrfPostCancellationWeightedRows[U, fObs, vars];
  postWeights = hrfWeightedRowWeight[#, v] & /@ postRows;
  wSL = If[fSLTerms === {} || fSLTerms === {0}, Missing["NoFSL"],
    Sort@DeleteDuplicates[monomialWeight[#, vars, v] & /@ fSLTerms]];
  wU = leadingWeight[U, vars, v];
  wObs = If[fObsTermData === {}, {},
    Sort @ DeleteDuplicates[(#["Power"] + monomialWeight[#["Term"], vars, v]) & /@ fObsTermData]
  ];
  wLP = If[postRows === {}, Infinity, Min[postWeights]];
  postRowsAtLP = Pick[postRows, postWeights, wLP];
  lpTerms = Join[
    Select[UTerms, monomialWeight[#, vars, v] == wLP &],
    Lookup[
      Select[fObsTermData, (#["Power"] + monomialWeight[#["Term"], vars, v]) == wLP &],
      "Term",
      {}
    ]
  ];
  lpVars = hrfWeightedRowsCoverVariables[postRowsAtLP, vars];
  ...
  <|
   ...
   "FObsTermsAtPostCancellationOrder" ->
     Lookup[
       Select[fObsTermData, (#["Power"] + monomialWeight[#["Term"], vars, v]) == wLP &],
       "Term",
       {}
   ...
   "PostCancellationLeadingRows" -> postRowsAtLP,
   "PostCancellationLeadingPowers" -> Sort @ DeleteDuplicates[Lookup[#, "Power", 0] & /@ postRowsAtLP],
   "PostCancellationLeadingSources" -> Tally[Lookup[#, "Source", "Post"] & /@ postRowsAtLP],
   "ExplicitDeltaWeightsRestoredQ" -> AnyTrue[postRows, ! TrueQ[Lookup[#, "Power", 0] === 0] &],
...
(* Augmented exponent rows for restored delta-suppressed terms.  A weighted
   row stores (r_i, a_i), and its region weight is r_i.v + a_i, equivalent
   to dotting (r_i; a_i) into the region vector (v; 1).  The old API is kept:
   passing a plain F_obs polynomial is interpreted as the layer 0 -> F_obs. *)
hrfWeightedPostLayerData[input_, defaultLabel_ : "FObs"] := Module[
  {items = {}, add, parse},
  add[power_, poly_, label_] := Module[{polys},
    polys = DeleteCases[Flatten[{poly}], 0];
    Scan[
      If[! TrueQ[Expand[#] === 0],
        AppendTo[items, <|"Power" -> power, "Polynomial" -> Expand[#], "Label" -> label|>]
      ] &,
      polys
    ]
  ];
  parse[obj_, label_] := Which[
    MatchQ[obj, None | Automatic | Missing[__]], Null,
    AssociationQ[obj] && AllTrue[Keys[obj], NumericQ],
      Scan[add[#[[1]], #[[2]], label] &, Normal[obj]],
    AssociationQ[obj],
      (
        If[KeyExistsQ[obj, "FObs"], add[0, obj["FObs"], "FObs"]];
        If[KeyExistsQ[obj, "Obstruction"], add[0, obj["Obstruction"], "FObs"]];
        If[KeyExistsQ[obj, "Polynomial"],
          add[Lookup[obj, "Power", 0], obj["Polynomial"], Lookup[obj, "Label", label]]
        ];
        If[KeyExistsQ[obj, "WeightedPolynomials"], parse[obj["WeightedPolynomials"], label]];
        If[KeyExistsQ[obj, "Layers"], parse[obj["Layers"], label]];
        If[KeyExistsQ[obj, "DeltaLayers"], parse[obj["DeltaLayers"], "FDelta"]];
        If[KeyExistsQ[obj, "FDelta"], parse[obj["FDelta"], "FDelta"]];
      ),
    ListQ[obj] && AllTrue[obj, MatchQ[#, _Rule | _RuleDelayed] &],
      Scan[add[#[[1]], #[[2]], label] &, obj],
    ListQ[obj] && AllTrue[obj, AssociationQ[#] && KeyExistsQ[#, "Polynomial"] &],
      Scan[add[Lookup[#, "Power", 0], #["Polynomial"], Lookup[#, "Label", label]] &, obj],
    ListQ[obj],
      Scan[add[0, #, label] &, obj],
    True,
      add[0, obj, label]
  ];
  parse[input, defaultLabel];
  items
];

hrfWeightedRow[row_, power_ : 0, source_ : "Post"] := <|
  "Row" -> row,
  "Power" -> power,
  "Source" -> source
|>;

hrfWeightedRowsFromPolynomial[p_, vars_, power_ : 0, source_ : "Post"] :=
  hrfWeightedRow[#, power, source] & /@ polynomialExponentRows[p, vars];

hrfWeightedRowsFromLayers[layers_List, vars_] :=
  DeleteDuplicatesBy[
    Join @@ (hrfWeightedRowsFromPolynomial[#["Polynomial"], vars, #["Power"], #["Label"]] & /@ layers),
    {Lookup[#, "Power", 0], Lookup[#, "Row", {}], Lookup[#, "Source", "Post"]} &
  ];

hrfAsWeightedRows[rows_, power_ : 0, source_ : "Post"] := Which[
  rows === {} || MatchQ[rows, None | Automatic | Missing[__]], {},
  ListQ[rows] && AllTrue[rows, AssociationQ[#] && KeyExistsQ[#, "Row"] &], rows,
  MatrixQ[rows], hrfWeightedRow[#, power, source] & /@ rows,
  ListQ[rows] && AllTrue[rows, MatrixQ],
    Join @@ MapIndexed[(hrfWeightedRow[#, power, source <> ToString[First[#2]]] & /@ #1) &, rows],
  True, {}
];

hrfWeightedRowWeight[row_Association, v_] := Lookup[row, "Power", 0] + Lookup[row, "Row", {}].v;
hrfWeightedRowWeight[row_List, v_] := row.v;
hrfWeightedRowWeightExpr[row_Association, r_] := Lookup[row, "Power", 0] + Lookup[row, "Row", {}].r;
hrfWeightedRowWeightExpr[row_List, r_] := row.r;

hrfRowsAtWeightedWeight[rows_List, v_, w_] := Module[{weights},
  If[rows === {}, Return[{}]];
  weights = hrfWeightedRowWeight[#, v] & /@ rows;
  Pick[rows, weights, w]
];

hrfWeightedRowsSupportPositions[rows_List] := Module[{raw},
  If[rows === {}, Return[{}]];
  raw = Lookup[#, "Row", #] & /@ rows;
  If[raw === {}, {}, Flatten @ Position[Total[Unitize[raw]], _?(# > 0 &)]]
];

hrfWeightedRowsCoverVariables[rows_List, vars_List] := vars[[hrfWeightedRowsSupportPositions[rows]]];

hrfPostCancellationWeightedRows[U_, fObs_, vars_] := Join[
  hrfWeightedRowsFromPolynomial[U, vars, 0, "U"],
  hrfWeightedRowsFromLayers[hrfWeightedPostLayerData[fObs, "FObs"], vars]
];

hrfDeltaLayerAssociation[poly_, delta_ : \[Delta], leadingPower_ : Automatic] := Module[
  {expanded, terms, powers, p0, pmax, coeff},
  If[MatchQ[poly, None | Automatic | Missing[__]], Return[<||>]];
  expanded = Expand[poly];
  terms = hrfPolynomialTerms[expanded];
  If[terms === {}, Return[<||>]];
  powers = Exponent[#, delta] & /@ terms;
  p0 = Replace[leadingPower, Automatic :> Min[powers]];
  pmax = Max[powers];
  Association @ DeleteCases[
    Table[
      coeff = Expand[Coefficient[expanded, delta, p]];
      If[TrueQ[coeff === 0] || p == p0, Nothing, (p - p0) -> coeff],
      {p, p0, pmax}
    ],
    Nothing
  ]
...
hrfPostCancellationLayerInput[fObs_, fullPoly_ : Automatic, delta_ : \[Delta],
   leadingPower_ : Automatic] := <|
  "FObs" -> fObs,
  "DeltaLayers" -> hrfDeltaLayerAssociation[fullPoly, delta, leadingPower]
|>;
...
coverageConstraintDiagnosticFromRows[v_, fSLRows_, URows_, fObsRows_, vars_] := Module[
  {fSLRowsFlat, fSLWeights, fSLUniform, wSL, uRowsW, obsRowsW, postRowsW,
   postWeights, uWeights, wU, obsWeights, wObs, wLP, postRows, postRowsU,
   postRowsObs, postRowsDelta,
  ...
  uRowsW = hrfAsWeightedRows[URows, 0, "U"];
  obsRowsW = hrfAsWeightedRows[fObsRows, 0, "FObs"];
  postRowsW = Join[uRowsW, obsRowsW];
  postWeights = hrfWeightedRowWeight[#, v] & /@ postRowsW;
  uWeights = hrfWeightedRowWeight[#, v] & /@ uRowsW;
  wU = If[uWeights === {}, Infinity, Min[uWeights]];
  obsWeights = hrfWeightedRowWeight[#, v] & /@ obsRowsW;
  wObs = If[obsWeights === {}, {}, Sort@DeleteDuplicates[obsWeights]];
  wLP = If[postWeights === {}, Infinity, Min[postWeights]];
  postRows = Pick[postRowsW, postWeights, wLP];
  postRowsU = Select[postRows, Lookup[#, "Source", "Post"] === "U" &];
  postRowsObs = Select[postRows,
    Lookup[#, "Source", "Post"] =!= "U" && TrueQ[Lookup[#, "Power", 0] === 0] &
  ];
  postRowsDelta = Select[postRows, ! TrueQ[Lookup[#, "Power", 0] === 0] &];
  postVars = hrfWeightedRowsCoverVariables[postRows, vars];
  ...
  diag = <|
    ...
    "PostCancellationLeadingUSupport" -> hrfWeightedRowsCoverVariables[postRowsU, vars],
    "PostCancellationLeadingFObsSupport" -> hrfWeightedRowsCoverVariables[postRowsObs, vars],
    "PostCancellationLeadingFDeltaSupport" -> hrfWeightedRowsCoverVariables[postRowsDelta, vars],
    "PostCancellationLeadingPowers" -> Sort @ DeleteDuplicates[Lookup[#, "Power", 0] & /@ postRows],
    "PostCancellationLeadingSources" -> Tally[Lookup[#, "Source", "Post"] & /@ postRows],
    "ExplicitDeltaWeightsRestoredQ" -> AnyTrue[postRowsW, ! TrueQ[Lookup[#, "Power", 0] === 0] &],
...
coverageScalingQ[fSL_, U_, vars_, v_, fObs_ : None] := Module[
  {fSLList, fSLRows, URows, fObsRows, diag},
  fSLList = DeleteCases[Flatten[{fSL}], 0];
...
Quiet @ Check[
  If[Length[DownValues[findExactCoverageLPScaling]] == 0,
    Quiet @ Get[FileNameJoin[{hrfPackageDirectory[], "HRF_ExactCoverageScaling.wl"}]]
  ],
  Null
];
...
ClearAll[hrfNormalizeScalingVector, hrfFSLUniformScalingQ, hrfScalingVectorInBoundsQ,
  ...
  hrfHomogeneousScalingCandidates, hrfCoordinateUniformScalingSearch,
  hrfScalingSearchMeta, hrfAssocMerge];
...
hrfCoordinateUniformScalingSearch[fSLRows_, URows_, fObsRows_, vars_, maxAbs_Integer,
   maxReport_: 20] := Module[
  {nvars, bruteCount, limit, rows, candidates, accepted = {}, nearMisses = {},
   diag, sortKey, uniformTest, status, candidateCount = 0},
  nvars = Length[vars];
  bruteCount = (maxAbs + 1)^nvars - 1;
  limit = If[ValueQ[$HRFScalingCoordinateUniformScanLimit],
    $HRFScalingCoordinateUniformScanLimit,
    2000000
  ];
  If[bruteCount > limit,
    Return[hrfAssocMerge[<|
      "Scaling" -> Missing["CoordinateUniformScanTooLarge"],
      "CandidateGenerationMethod" -> "CoordinateUniformFSLScan",
      "CandidateCount" -> 0,
      "CoordinateUniformScanBoxSize" -> bruteCount,
      "CoordinateUniformScanLimit" -> limit,
      "AcceptedCount" -> 0,
  ...
  rows = DeleteDuplicates[Join @@ fSLRows];
  uniformTest[v_] := hrfFSLUniformScalingQ[rows, v];
  Print[DateString[{"Hour", ":", "Minute", ":", "Second"}] <>
    "  [coverage coordinate-uniform scan start] box=" <> ToString[bruteCount] <>
  ...
  Do[
    ...
    {v, Tuples[Range[-maxAbs, 0], nvars]}
  ];
  Print[DateString[{"Hour", ":", "Minute", ":", "Second"}] <>
    "  [coverage coordinate-uniform scan end] uniformCandidates=" <>
    ToString[candidateCount] <> " accepted=" <> ToString[Length[accepted]]];
  accepted = SortBy[accepted, sortKey];
  nearMisses = SortBy[nearMisses,
    {Length[Lookup[#, "FailedConditions", {}]] &, Total[Abs[Lookup[#, "ScalingVector", {}]]] &, Max[Abs[Lookup[#, "ScalingVector", {0}]]] &}
  ];
  If[accepted === {},
    hrfAssocMerge[<|
      "Scaling" -> Missing["NoCoordinateUniformCoverageScalingFoundUpTo", maxAbs],
      "CandidateGenerationMethod" -> "CoordinateUniformFSLScan",
      ...
      "Diagnostics" -> {}
    |>, hrfScalingSearchMeta[0, True, candidateCount]],
    Module[{best = First[accepted]},
      hrfAssocMerge[<|
        ...
        "Diagnostics" -> Join[best, <|"WSL" -> best["FSLWeight"], "WHR" -> best["PostCancellationLeadingWeight"]|>]
      |>, hrfScalingSearchMeta[Length[accepted], True, candidateCount]]
    ]
  ]
];
...
findCoverageLPScaling[fSL_, U_, vars_, maxAbs_ : 5, fObs_ : None,
   method_ : Automatic] := Module[{meth = Replace[method, Automatic -> "ExactCoverage"]},
  Which[
    MemberQ[{"ExactCoverage", "Exact"}, meth] &&
      Length[DownValues[findExactCoverageLPScaling]] > 0,
      findExactCoverageLPScaling[fSL, Expand[U], vars, fObs],
    MemberQ[{"ExactCoverage", "Exact"}, meth],
      <|"Scaling" -> Missing["ExactCoverageScalingUnavailable"],
        "ScalingStatus" -> "Unavailable",
        "ScalingStatusMessage" -> "Exact coverage scaling helper was not loaded"|>,
    MemberQ[{"Bounded", "Legacy", "NullspaceBox"}, meth],
      findCoverageLPScalingBounded[fSL, U, vars, maxAbs, fObs],
    True,
      findCoverageLPScalingBounded[fSL, U, vars, maxAbs, fObs]
  ]
];

findCoverageLPScalingBounded[fSL_, U_, vars_, maxAbs_ : 5, fObs_ : None] := Module[
  {fSLList, fSLRows, URows, fObsRows, candidateInfo, candidates, exhaustiveQ,
   genMethod, bruteCount, accepted, nearMisses, best, nvars, diag, sortKey, maxReport = 20, v,
   searchMeta, coordinateUniform},
  fSLList = DeleteCases[Flatten[{fSL}], 0];
  If[fSLList === {} || U === 0,
  ...
  If[accepted === {},
   coordinateUniform = hrfCoordinateUniformScalingSearch[fSLRows, URows, fObsRows, vars, maxAbs, maxReport];
   If[AssociationQ[coordinateUniform] && TrueQ[Lookup[coordinateUniform, "AcceptedCount", 0] > 0],
    Return[Join[
      coordinateUniform,
      <|
        "FastNullspaceCandidateCount" -> Length[candidates],
        "FastNullspaceMethod" -> genMethod,
        "FastNearMissDiagnostics" -> Take[nearMisses, UpTo[maxReport]]
      |>
    ]]
   ];
   If[AssociationQ[coordinateUniform] && TrueQ[Lookup[coordinateUniform, "ScalingSearchCompleteQ", False]],
    Return[Join[
      coordinateUniform,
      <|
        "FastNullspaceCandidateCount" -> Length[candidates],
   ...
   If[bruteCount > If[ValueQ[$HRFScalingBruteForceLimit], $HRFScalingBruteForceLimit, 1000000],
    ...
    Return[hrfAssocMerge[<|
      ...
      "CoordinateUniformScalingData" -> coordinateUniform,
      "FastNearMissDiagnostics" -> Take[nearMisses, UpTo[maxReport]],
      "Diagnostics" -> {}
...
findCoverageLPScalingScan[fSL_, U_, vars_, maxAbs_ : 5, fObs_ : None] := Module[
  {fSLList, fSLRows, URows, fObsRows, candidates, accepted,
