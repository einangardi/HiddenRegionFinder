...
hrfPreselectedScanObstruction[F_, vars_, U_, postLayers_:Automatic] :=
  findObstructions[
    F, vars, KinAssump, KinVars, Automatic,
    Sequence @@ hrfPreselectedFindOptions[],
    "U" -> U,
    Sequence @@ If[MatchQ[postLayers, Automatic | None | Missing[__]], {}, {"FObsForScaling" -> postLayers}],
    "MaxScalingAbs" -> $HRFPreselectedMaxScalingAbs,
    "RequireValidScalingForHiddenRegionQ" -> True,
    "EnumerateHiddenRegionsQ" -> True
  ];

hrfPreselectedScalingForScan[scan_, U_, vars_, postLayers_:Automatic] := Module[
  {builtInScaling, obsData, obstruction, fsl, postInput},
  ...
  postInput = Which[
    MatchQ[postLayers, Automatic | Missing[__]], obstruction,
    AssociationQ[postLayers] && ! KeyExistsQ[postLayers, "FObs"] && ! KeyExistsQ[postLayers, "Obstruction"],
      Join[<|"FObs" -> obstruction|>, postLayers],
    True, postLayers
  ];
  fsl = hrfFPEffectiveSuperleadingSector[scan];
  If[MatchQ[fsl, _Missing] || TrueQ[Expand[fsl] === 0],
    Missing["NoNonzeroSuperleadingSector"],
    findCoverageLPScaling[fsl, U, vars, $HRFPreselectedMaxScalingAbs, postInput]
  ]
];
...
hrfPreselectedScanInteriorRow[data_Association] := Module[{scan, scaling, postLayers},
  postLayers = <|"DeltaLayers" -> hrfDeltaLayerAssociation[data["FCollinear"], \[Delta]]|>;
  scan = hrfPreselectedScanObstruction[data["Flead"], data["Variables"], data["U"], postLayers];
  scaling = hrfPreselectedScalingForScan[scan, data["U"], data["Variables"], postLayers];
  Join[data, <|
    "RegionType" -> "Interior",
    "ZeroVars" -> {},
    "RemainingVars" -> data["Variables"],
    "PostCancellationLayers" -> postLayers,
...
hrfPreselectedScanBoundaryRows[data_Association] := Module[
  {strata, rows = {}, zeroVars, remainingVars, fRestricted, fColRestricted,
   uRestricted, postLayers, scan, scaling},
  ...
  Do[
    ...
    fColRestricted = Expand[data["FCollinear"] /. Thread[zeroVars -> 0]];
    uRestricted = Expand[data["U"] /. Thread[zeroVars -> 0]];
    postLayers = <|"DeltaLayers" -> hrfDeltaLayerAssociation[fColRestricted, \[Delta]]|>;
    scan = If[fRestricted === 0 || remainingVars === {},
      <||>,
      hrfPreselectedScanObstruction[fRestricted, remainingVars, uRestricted, postLayers]
    ];
    scaling = hrfPreselectedScalingForScan[scan, uRestricted, remainingVars, postLayers];
    AppendTo[rows, Join[data, <|
      "RegionType" -> "Boundary",
      "ZeroVars" -> zeroVars,
      "RemainingVars" -> remainingVars,
      "FRestricted" -> fRestricted,
      "FCollinearRestricted" -> fColRestricted,
