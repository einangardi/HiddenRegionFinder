...
hrfEx02ChannelMeta[channel_, Finput_, vars_, kinAssump_, kinVars_, scan_, Ffull_:Automatic] := Module[
  ...
  <|
    ...
    "AllInIdealDecompositionQ" -> If[ValueQ[hrfExactReductionQ], hrfExactReductionQ[scan], False],
...
hrfEx02RunObstructionScan[F_, vars_, kinAssump_, kinVars_, maxSize_:Automatic,
   Ffull_:Automatic, U_:Automatic, kinLimit_:Automatic, reggeChannel_:Automatic] := Module[
  {reggeQ = Ffull =!= Automatic || (Length[kinVars] === 1 && MemberQ[{s12, s23}, First[kinVars]]),
   limit, postLayers, scan},
  limit = Which[
    kinLimit =!= Automatic, kinLimit,
    reggeQ, "Regge4pt",
    True, "WideAngle4ptBoundary"
  ];
  postLayers = If[StringQ[reggeChannel] && ! MatchQ[Ffull, Automatic | Missing],
    <|"DeltaLayers" -> hrfReggeDeltaLayers[Ffull, reggeChannel]|>,
    Automatic
  ];
  scan = findObstructions[
    ...
    Sequence @@ If[MatchQ[postLayers, Automatic], {}, {"FObsForScaling" -> postLayers}],
  ...
  If[! MatchQ[U, Automatic | Missing] && AssociationQ[scan] && ! hrfScanCoverageScalingReadyQ[scan],
    hrfAttachCoverageScalingData[scan, U, vars, 5, postLayers],
    scan
  ]
];
...
hrfEx02InteriorStudy[diagram_, data_Association, maxSize_:Automatic] := Module[
  ...
  Do[
    Module[{fRegge, kv, ka, scan, cov},
      ...
      scan = hrfEx02TrimScanForNotebook @ hrfEx02RunObstructionScan[fRegge, vars, ka, kv, maxSize, F, U, Automatic, regCh];
...
hrfEx02BoundaryStudy[diagram_, data_Association, zeroVars_List, regionLabel_, maxSize_:Automatic] := Module[
  ...
  Do[
    Module[{fRegge, kv, ka, scan, cov, t1},
      ...
      kv = hrfReggeKinVars[regCh];
      ka = hrfReggeKinAssumptions[regCh];
      scan = hrfEx02TrimScanForNotebook @ hrfEx02RunObstructionScan[fRegge, varsB, ka, kv, maxSize, fB, uB, Automatic, regCh];
      channelScans = Join[channelScans, <|regCh -> scan|>];
...
hrfEx02RunCrownSanityCheck[] := Module[{data, F, vars, U, rows = {}, row, t0, regCh, fRegge, kv, ka, scan, channels, sanityScans = <||>},
  ...
  Do[
    ...
    kv = hrfReggeKinVars[regCh];
    ka = hrfReggeKinAssumptions[regCh];
    scan = hrfEx02TrimScanForNotebook @ hrfEx02RunObstructionScan[fRegge, vars, ka, kv, Automatic, F, U, Automatic, regCh];
