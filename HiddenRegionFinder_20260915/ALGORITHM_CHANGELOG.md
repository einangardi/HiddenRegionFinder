...
## 2026-09-15 Part-II/dissection consolidation

The source-aware local dissection introduced in August has now been applied
exhaustively to the staged two-loop zero-recoil NMRK candidates.  The
classification distinguishes a staged candidate presentation, an exact local
facet certificate and a physical HR.  The latter is identified by the
boundary stratum, exact cancellation ideal, normalised pullback vector and
`W_HR`; different `W_SL` values or cancellation depths do not by themselves
split one physical region.

For the planar hexagon--box all 13 staged candidates produce certificates:
41 raw local certificates reduce to 25 physical HRs, comprising 13 interior
and 12 codimension-one regions.  For the same-path hexagon--pentagon, four of
eight staged candidates produce nine physical HRs, comprising three interior
and six codimension-one regions; the other four candidates are decided by an
exact positive-pinch obstruction or a complete no-facet dissection.  The
separate-path hexagon--pentagon has no staged candidate.  Notebooks 09--12 and
the compact records in `results/nmrk_wz/` preserve the complete audit.

The consolidation also includes the wide-angle six-point self-crossing
notebook and exact standalone `K_{3,3}` and `K_{4,3}` audits.  The former
exhibits cubic cancellation only outside the positive LP domain on the cyclic
family tested; the latter supplies a positive, scaleful six-loop Landshoff HR
with six independent normal coordinates.

## 2026-08-22 ordinary source-aware certification

The ordinary, non-aligned HRF path now separates the provisional
original-coordinate coverage solution from an exact source-aware local
certificate.  It retains `F_SL`, the obstruction, other restored `F` layers
and `U` as distinct sources until the local polynomial is formed.  A complete
chart search with no lower facet rejects a candidate, whereas a capped,
timed-out or unsupported chart is reported as unresolved.  The historical
coverage data remain available separately for audit.

## 2026-08-21 source-aware aligned dissection

Synthetic promotion of the `F_SL` support to `W_HR` is no longer an
acceptance test.  The final aligned audit maps the cancellation factors to
local endpoint variables, builds the full source-resolved LP polynomial,
enumerates its exact lower facets with `cddlib`, and pulls their normals back
to the original variables.  The saved-assumption audit recertifies all six
formerly promoted accepted representatives and finds 17 local facets, with
the resolved `F_SL` jet leading in every case.
...
`HRF_GeneralizedDerivativeHarvestTests.wl` includes 13 checks.  It verifies
...
five-point near-planar example, and a Crown negative control.  The latter
specialises `s12=1, s23=-1` before harvesting, so the independent kinematic
sectors are mixed and the resulting incomplete relation set is correctly not
promoted to a Crown HR.  The dedicated Crown construction instead separates
the derivative components multiplying `s12` and `s23` before factorisation.
...
- **Common-pinch requirement:** every transverse coordinate is suppressed by
  ...
  certificate.
- **Ultimate dissection certificate:** as of 21 August,
  `HRF_SourceAwareDissectionCertification.wl` constructs local sectors and
  applies the exact lower-facet test to every staged candidate.  A unique
  original-coordinate ideal-jet solution determines the candidate vector and
  remains an independent check, but acceptance requires the dissected local
  facet.  The older `HRF_LayeredDissectionCertification.wl` is retained as a
  diagnostic and no longer accepts a region.
...
| one-loop NMRK with `w=z` | `{-2,-5,-6,-2,-6,-5}` | -10 | -6 | 4 |
...
and the source-aware NMRK `w=z` result.  The standard Seed5pt spacelike
collinear case remains covered by `HRF_PolynomialFactorRegressionTests.wl`
and `HRF_NotebookSmokeTests.wl`.

## 3e. Source-aware dissection and exact pullback (2026-08-21)

The earlier provisional construction completed the rank of the putative resolved facet by
inserting copies of the `F_SL` exponent rows at `W_HR`.  That construction
encoded the expectation that the first nonzero `F_SL` jet contributes, but
did not test it.  Synthetic promotion is now retained only as the diagnostic
`SyntheticFSLPromotionFacetQ`; the companion field
`SyntheticFSLPromotionUsedForAcceptanceQ` is always false.

The new certification step:

1. reconstructs `F_SL`, `F_Obs`, the remaining `F` layers, and `U` in the
   aligned coordinates while retaining source labels;
2. solves independent cancellation factors for pivot LP variables and maps
   them to signed endpoint coordinates;
3. recombines equal exponent rows exactly, so cancellations between sources
   occur before the Newton support is formed;
4. enumerates exact lower facets of the dissected polynomial with `cddlib`;
5. requires positive transverse suppressions and a strict hidden hierarchy;
6. pulls each local normal back to the original LP variables and records
   inverse-chart, Jacobian, facet-rank, valuation, and hierarchy checks; and
7. measures `ResolvedFSLWeight` independently of
   `ResolvedLeadingWeight`.

Consequently `FSLOnLeadingFacetQ` and
`FSLSubleadingAfterCancellationQ` are outputs rather than assumptions.  The
synthetic regression in `HRF_SourceAwareDissectionRegressionTests.wl`
contains two exact facets at one cancellation endpoint: one has `F_SL` on the
leading layer, while the other has
`ResolvedFSLWeight=2 > ResolvedLeadingWeight=1`.  This demonstrates that the
implementation can detect the deeper-cancellation possibility.

`HRF_SourceAwareSavedAssumptionAudit.wl` rechecks the six saved accepted
representatives that formerly needed synthetic promotion.  All six receive
source-aware certificates (17 distinct local facets in total); every facet
has `F_SL` on the resolved leading layer, and none displays deeper
cancellation.  This is evidence from the current examples, not a general
proof.

As of 2026-08-22 the same source-aware certificate is the default final stage
of the ordinary, non-aligned `findObstructions` path.  Exact coverage scaling
is retained as a provisional candidate filter, not an acceptance condition.
Each certified local facet supplies the authoritative pulled-back scaling and
one entry in `HiddenRegionScans`.  A complete negative dissection rejects the
candidate; a negative search stopped by a pivot, sign-sector, point-count or
time budget remains unresolved.  Focused integration coverage is in
`HRF_OrdinaryDissectionIntegrationTests.wl`.

## 3f. Terminology and API migration (2026-07-24)
