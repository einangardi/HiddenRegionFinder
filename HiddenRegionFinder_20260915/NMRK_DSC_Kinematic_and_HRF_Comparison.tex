...
The ideal-layer equations determine a candidate vector whenever they have a
unique positive solution.  In every case the ultimate HRF certificate is a
source-resolved local dissection: an exact lower-facet normal in coordinates
adapted to the pinch is pulled back to fix \(v\).  Agreement with a unique
ideal-layer solution is an independent check.  No space-time dimensional
information enters any of these decisions.
...
For this case the alignment and provisional face-only data are
\begin{align*}
 f&=(-1,-1,-2,-1,-2,-1),\\
 h^{(0)}&=(-1,-1,-1,-1,-1,-1).
\end{align*}
The direct sum is not a certificate: after cancellation its genuine leading
support has affine rank four rather than six.  The previous test completed
that rank by promoting the \(\F_{\SL}\) exponent rows to the anticipated
\(W_{\HR}\) layer, thereby assuming that their first nonzero jet contributes.

The source-resolved dissection makes the correction explicit.  In the
physical sign sector set \(t_1=L_1^{\rm NMRK}\),
\(t_2=L_2^{\rm NMRK}\), with \(t_1,t_2\geq0\), and use
\begin{equation}
 x_4=\frac{\hX_{34}X_{45}x_5-t_1}{1+X_{45}},
 \qquad
 x_1=\frac{(1+X_{45})x_2-t_2}{K_zX_{45}\hX_{56}}.
 \label{eq:nmrk-local-map}
\end{equation}
In the local order \((x_0,x_2,x_3,x_5,t_1,t_2;1)\), the exact lower-facet
normal is
\begin{equation}
 (-1,-4,-1,-4,-2,-2;1).
...
\begin{align}
 h&=(-1,-4,-4,-1,-4,-4),\nonumber\\
 v_{\rm NMRK}&=f+h=(-2,-5,-6,-2,-6,-5),
 \qquad (v_{\rm NMRK};1),
 \label{eq:nmrk-vector}
\end{align}
with
\begin{equation}
 W_{\SL}=-10,\qquad W_{\HR}=-6,\qquad
 W_{\HR}-W_{\SL}=4.
 \label{eq:nmrk-gap}
\end{equation}
Each normal factor is suppressed by two powers.  The first nonzero
\(\F_{\SL}\) jet therefore occurs four powers above its individual
monomials, exactly at \(W_{\HR}\), together with two \(\U\) monomials and
seven monomials from the remaining \(\F\) polynomial.  The ten leading
exponent points have affine rank six.  Thus the participation of
\(\F_{\SL}\) is observed in the dissected polynomial rather than imposed.
...
next occupied layers differ.  In NMRK with \(w_M=z_M\), each of the two
normal factors is suppressed by two powers, giving a depth-four hierarchy.
Generic DSC has an additional first-order cancellation layer and a depth-two
hierarchy.  This is why the two final vectors are related but not equal.
...
\midrule
NMRK with \(w_M=z_M\) & \((-2,-5,-6,-2,-6,-5)\) & \((-10,-6)\) & 4\\
