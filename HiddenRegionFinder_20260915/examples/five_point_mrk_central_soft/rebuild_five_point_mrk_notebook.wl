...
endpointMomentumComparison = Get[FileNameJoin[{base,
...
oppositeEndpointTable = Dataset @ {
  <|"quantity" -> "external order at vertices", "value" ->
    "{1,2,4,5,3}"|>,
  <|"quantity" -> "factorised aligned polynomial (up to overall coefficient)",
    "value" -> -(x2 - X34hat x3) (x1 x4 - x0 x5)|>,
  <|"quantity" -> "alignment scaling", "value" ->
    <|x0 -> -3, x1 -> -3, x2 -> -3, x3 -> 0, x4 -> -3, x5 -> -3|>|>,
  <|"quantity" -> "HRF scaling", "value" ->
    <|x0 -> -1, x1 -> -1, x2 -> -1, x3 -> -1, x4 -> -1, x5 -> -1|>|>,
  <|"quantity" -> "total scaling", "value" ->
    <|x0 -> -4, x1 -> -4, x2 -> -4, x3 -> -1, x4 -> -4, x5 -> -4|>|>,
  <|"quantity" -> "(W_SL,W_HR)", "value" -> {-9, -8}|>,
  <|"quantity" -> "resolved facet rank / required", "value" -> {6, 6}|>,
  <|"quantity" -> "final audit", "value" -> "AcceptedLowerFacet"|>
};
...
repMomentumCase = endpointMomentumComparison["Representative"];
oppMomentumCase = endpointMomentumComparison["OppositeEndpoint"];
endpointRoutingTable = readableTable @ {
  <|"attachment" -> "p4 at endpoint 5",
    "external order" -> "{1,2,3,5,4}",
    "routing (q0,...,q5)" -> Values[repMomentumCase["ExactRouting"]]|>,
  <|"attachment" -> "p4 at endpoint 3",
    "external order" -> "{1,2,4,5,3}",
    "routing (q0,...,q5)" -> Values[oppMomentumCase["ExactRouting"]]|>
};
endpointCentralTable = readableTable @ Map[
  Function[edge, <|
    "edge" -> edge,
    "endpoint 5 central (q+,q-,q_perp)" ->
      repMomentumCase["CentralEdgeComponentPowers", edge],
    "endpoint 5 q^2" ->
      repMomentumCase["PropagatorVirtualityPowers", edge],
    "endpoint 3 central (q+,q-,q_perp)" ->
      oppMomentumCase["CentralEdgeComponentPowers", edge],
    "endpoint 3 q^2" ->
      oppMomentumCase["PropagatorVirtualityPowers", edge]
  |>],
  {x0, x1, x2, x3, x4, x5}
];
endpointLocalEdgeTable = readableTable @ Map[
  Function[edge, <|
    "edge" -> edge,
    "endpoint 5 mode-adapted powers" ->
      repMomentumCase["ModeAdaptedEdgeComponentPowers", edge],
    "endpoint 3 mode-adapted powers" ->
      oppMomentumCase["ModeAdaptedEdgeComponentPowers", edge]
  |>],
  {x0, x1, x2, x3, x4, x5}
];
endpointModeTable = readableTable @ {
  <|"attachment" -> "p4 at endpoint 5",
    "ordinary loop" -> "r=q0", "ordinary widths" -> {4, 0, 2},
    "Glauber loop" -> "ell=q0-q4", "Glauber widths" -> {4, 1, 2},
    "soft-collinear edge" -> "q3:(1,3,2), q3^2~delta^4",
    "longitudinal-dominated edge (not Glauber)" ->
      "q2:(-2,3,2), q2^2~delta"|>,
  <|"attachment" -> "p4 at endpoint 3",
    "ordinary loop" -> "r=q1", "ordinary widths" -> {4, 0, 2},
    "Glauber loop" -> "ell=q1-q5", "Glauber widths" -> {4, 1, 2},
    "soft-collinear edge" -> "q2:(1,3,2), q2^2~delta^4",
    "longitudinal-dominated edge (not Glauber)" ->
      "q3:(-2,3,2), q3^2~delta"|>
};
endpointGlauberRoutingTable = readableTable @ {
  <|"attachment" -> "p4 at endpoint 5",
    "first t-channel routing" -> "ell3=q0-q4=p3-q2",
    "first central powers" -> {2, 2, 0},
    "second t-channel routing" -> "ell5=q1-q5=p4-q3",
    "second central powers" -> {4, 1, 1},
    "local relation" -> "Delta ell5=-Delta ell3",
    "independent Glauber loops" -> 1|>,
  <|"attachment" -> "p4 at endpoint 3",
    "first t-channel routing" -> "ell5=q1-q5=p3-q3",
    "first central powers" -> {2, 2, 0},
    "second t-channel routing" -> "ell3=q0-q4=p4-q2",
    "second central powers" -> {4, 1, 1},
    "local relation" -> "Delta ell3=-Delta ell5",
    "independent Glauber loops" -> 1|>
};
endpointPowerTable = readableTable @ {
  <|"attachment" -> "endpoint 5", "parameter measure" -> -20,
    "momentum measure" -> "4D+1", "sum q_e^2 powers" -> 21,
    "scalar power" -> "4D-20=-4-8 epsilon"|>,
  <|"attachment" -> "endpoint 3", "parameter measure" -> -20,
    "momentum measure" -> "4D+1", "sum q_e^2 powers" -> 21,
    "scalar power" -> "4D-20=-4-8 epsilon"|>
};
...
cells = {
  ...
  txt["Generic five-point MRK and the composite limit p4_perp -> 0 are not the same expansion. In the scan recorded here, all inequivalent one-loop pentagon attachment labelings have no hidden region. The two-loop six-propagator seed has no certified hidden region in generic MRK, but has one in the composite limit for six of the twelve inequivalent graph attachment labelings. Every accepted labeling attaches the central gluon p4 to internal vertex 3 or 5, the two four-valent vertices of the seed graph, so this is a necessary condition within the scan but not sufficient: {1,3,2,5,4} also has p4 at vertex 5 and is negative. The direct endpoint-reflected partner {1,2,4,5,3} is positive. These graph labelings are not alternative rapidity orderings: every row uses y3 >> y4 >> y5."],
  ...
  outCell[graphPanel],
  subsec["Why a four-valent attachment is necessary"],
  ...
  subsec["Opposite four-valent endpoint"],
  txt["Moving p4 from endpoint 5 to the directly reflected endpoint 3, while reflecting the endpoint attachment of p3, gives the ordering {1,2,4,5,3}. A separate full-native-layer run again has three staged candidates; two are rejected as scaleless and one is a rank-six lower facet. Its fixed-edge vector differs from the representative vector:"],
  outCell[oppositeEndpointTable],
  txt["The graph reflection (x0,x1,x2,x3,x4,x5)->(x1,x0,x3,x2,x5,x4) maps the representative certificate into this one. They are therefore the same graph-symmetry orbit, although the vectors are not componentwise equal in the fixed edge labeling."],

  sec["6. Complete momentum-space comparison and scalar power counting"],
  txt["The opposite certificate was not inferred only from its region vector. The exact graph reflection x0<->x1, x2<->x3, x4<->x5 maps the representative factorised polynomial, total vector and momentum routing into the opposite attachment. The transported central edge assignment independently satisfies plus, minus and transverse momentum conservation at all five vertices. The two exact routings are:"],
  outCell[endpointRoutingTable],
  txt["The next table separates central edge-momentum valuations from propagator virtualities. Entries (a,b,c) denote (q+,q-,|q_perp|)~(delta^a,delta^b,delta^c). The x4 and x5 propagators are near-on-shell enhanced: cancellation between longitudinal and transverse terms raises their virtuality to delta^4. Under endpoint reflection the longitudinal-dominated virtuality delta moves from x2 to x3, while the on-shell-balanced soft-collinear edge moves from x3 to x2. Neither edge is a Glauber propagator."],
  outCell[endpointCentralTable],
  txt["Central values are not integration widths. Expanding about the pinched solution gives the following mode-adapted edge powers and loop variables:"],
  outCell[endpointLocalEdgeTable],
  outCell[endpointModeTable],
  txt["Both four-valent endpoints carry a t-channel Glauber transfer. They do not define two loop modes: the second transfer is an affine rerouting of the first, ell_second=p1-p5-ell_first, so their local fluctuations obey Delta ell_second=-Delta ell_first."],
  outCell[endpointGlauberRoutingTable],
  txt["The mode tests are: 1+3=2*2 for the on-shell-balanced soft-collinear edge; -2+3<2*2 for the longitudinal-dominated edge; and 4+1>2*2 for the local widths of the Glauber loop ell. Thus Glauber character belongs to the loop combination ell, not to an individual propagator."],
  txt["For both attachments q0/q1 pinch the ordinary plus component at width delta^4, q4/q5 pinch the Glauber plus component at delta^4, and q2/q3 restrict its minus component to delta. Both transverse widths are delta^2. Since 4+1>2*2, there is precisely one Glauber loop, presented in two t-channel affine routings through the two theta endpoints."],
  txt["In parameter space the representative normals are A=P x2-K delta^3 x3 and B=x1 x4-x0 x5. For the opposite endpoint A becomes K delta^3 x2-P x3, while B is unchanged. In each case wt(A)=0, wt(B)=-8, four tangential coordinates contribute -16 and the Jacobian contributes +4, giving measure power -20. In momentum space the ordinary and Glauber measures give 2D and 2D+1; the six propagator powers sum to 21."],
  outCell[endpointPowerTable],
...
*)
  ...
  txt["All twelve one-loop pentagon labelings fail already at the recursive mixed-derivative pinch preselection for each of the three tested rates. Generic MRK gives no certified HR for any labeling of the two-loop seed: six labelings produce staged near-misses, but the lower-facet audit rejects them. In the exact central-soft chart, all six accepted labelings attach p4 to a four-valent seed vertex, but the endpoint condition alone is not sufficient: {1,3,2,5,4} is an endpoint attachment with no surviving face."],
