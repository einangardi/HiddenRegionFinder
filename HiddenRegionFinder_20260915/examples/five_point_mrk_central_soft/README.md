...
- Six of the twelve labelings have an HR. Every accepted case attaches the
  central gluon `p4` to internal vertex 3 or 5, the two four-valent vertices
  of the seed graph, so this is necessary within the scan but not sufficient:
  the ordering `{1,3,2,5,4}` has `p4` at vertex 5 and is nevertheless
  negative. The direct endpoint-reflected partner `{1,2,4,5,3}` is positive.
