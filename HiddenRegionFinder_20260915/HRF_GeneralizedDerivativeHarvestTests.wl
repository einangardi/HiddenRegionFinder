...
(* Crown negative control for premature kinematic specialization.  Substituting
   the physical witness before separating the s12 and s23 derivative
   components mixes the two kinematic sectors.  This route harvests two
   positive relations, but it must not promote that incomplete relation set
   to the dedicated Crown generator presentation. *)
...
tests = {
  ...
  <|"Test" -> "CrownMixedRelationsHarvestedQ",
    "PassQ" -> TrueQ[
      Length[crownScan["GeneralizedDerivativeHarvestAudit",
        "DomainCompatibleFactors"]] === 2
    ]|>,
  <|"Test" -> "CrownPrematureSpecializationNotPromotedQ",
    "PassQ" -> TrueQ[
