...
hrfRunPolynomialFactorRegressionTests[] := Module[
  {rows = {}, obs, ff, bin, bounds, phys, rem, genSeed, ffSeed, binSeed,
   ...
   fullPairs, indexedPairs, pairKey,
  ...
  AppendTo[rows, hrfRegressionAssert["Crown.CompleteChannelPolynomialHarvestQ",
    Lookup[factorPack, "FromCompleteChannelPolynomials", {}] =!= {} &&
      Lookup[factorPack, "FromSignedMonomialPairs", {}] === {},
    "Primitive A1, A2 and A1 +/- A2 polynomials are factorised completely; arbitrary monomial sub-pairs are not promoted"]];
  ...
  AppendTo[rows, hrfRegressionAssert["Crown.AllFactorsKinFreeQ",
    And @@ (! hrfFactorContainsKinQ[#, KinVars4pt] & /@ ff), ""]];
  fullPairs = Select[Subsets[phys["Factors"], {2}],
    hrfGeneratorPairPhysicsAdmissibleQ[
      #[[1]], #[[2]], bounds, VarsCrown, KinVars4pt] &];
  indexedPairs = Select[
    hrfIndexedDisjointSupportFactorPairs[
      phys["Factors"], VarsCrown, KinVars4pt, bounds],
    hrfGeneratorPairPhysicsAdmissibleQ[
      #[[1]], #[[2]], bounds, VarsCrown, KinVars4pt] &];
  pairKey[pair_] := Sort[ToString[InputForm[Expand[#]]] & /@ pair];
  AppendTo[rows, hrfRegressionAssert[
    "Crown.IndexedDisjointSupportPairEnumerationExactQ",
    Sort[pairKey /@ indexedPairs] === Sort[pairKey /@ fullPairs],
    "indexed enumeration equals full Subsets enumeration after the exact physics filter"]];
  AppendTo[rows, hrfRegressionAssert[
    "Crown.PairExponentSupportFilterExactQ",
    Module[{pairIndices, supportData, oldDecisions, newDecisions},
      pairIndices = hrfIndexedDisjointSupportFactorPairIndices[
        phys["Factors"], VarsCrown, KinVars4pt, bounds];
      supportData = hrfPairProductExponentSupportData[
        phys["Factors"], F0Crown, VarsCrown, KinVars4pt];
      oldDecisions = hrfGeneratorDegreeAdmissibleQ[
          Expand[Times @@ phys["Factors"][[#]]], bounds,
          VarsCrown, F0Crown, KinVars4pt] & /@ pairIndices;
      newDecisions = hrfPairProductDegreeF0SupportAdmissibleQ[
          #, supportData, bounds] & /@ pairIndices;
      SameQ[oldDecisions, newDecisions]
    ],
    "bit-mask exponent support test equals expanded-polynomial test pair by pair"]];
