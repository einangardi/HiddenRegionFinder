...
(* Coverage-based LP scaling diagnostic for the ordinary Crown.  The
   obstruction search uses the on-shell F0, while the scaling test restores
   the off-shell delta-suppressed layers from FOnShell. *)
If[TrueQ[$HRFRunScalingDiagnostics] && AssociationQ[CrownScan],
  ...
  CrownCoverageScalingData = findCoverageLPScaling[
    ...
    hrfPostCancellationLayerInput[
      CrownScan["ObstructionData", "Obstruction"],
      CrownData["FOnShell"], \[Delta]
    ]
  ];
  ...
  CrownCoverageScalingData = Missing["NotRun", If[TrueQ[$HRFRunScalingDiagnostics],
    "Crown scan was not run before the scaling diagnostic.",
    "Set $HRFRunScalingDiagnostics=True before loading the example"
  ]];
  CrownOldScalingDiagnostic = CrownCoverageScalingData
];
...
If[TrueQ[$HRFRunScalingDiagnostics] && AssociationQ[SuperCrownBoundaryScan],
  ...
  SuperCrownCoverageScalingData = findCoverageLPScaling[
    ...
    hrfPostCancellationLayerInput[
      ...
      Expand[SuperCrownData["FOnShell"] /. Thread[zeroSC -> 0]], \[Delta]
    ]
  ];
  ...
  SuperCrownCoverageScalingData = Missing["NotRun", If[TrueQ[$HRFRunScalingDiagnostics],
    "SuperCrown boundary scan was not run before the scaling diagnostic.",
    "Set $HRFRunScalingDiagnostics=True before loading the example"
  ]];
  SuperCrownOldScalingDiagnostic = SuperCrownCoverageScalingData
];
...
If[TrueQ[$HRFRunHyperCrownBoundaryScansOnLoad],
  ...
  HyperCrownBoundaryCandidateScans = Table[
    Module[{zeroHC, row, scan, vars, f, fOnShellRestricted, genCheck, uRestricted, cov},
      ...
      f = row["FRestricted"];
      fOnShellRestricted = Expand[HyperCrownData["FOnShell"] /. Thread[zeroHC -> 0]];
      ...
      <|
        ...
        "RemainingVars" -> vars,
        "FRestricted" -> f,
        "FOnShellRestricted" -> fOnShellRestricted,
