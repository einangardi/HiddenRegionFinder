(* HRF_FinalLogicPatch.wl
   ...
   - Treat the all-in-generator-ideal case (F_SL = F, Obs = 0) as a valid
     decomposition, but require the usual LP scaling vector before reporting a
     hidden region.  A zero superleading sector is rejected upstream.
*)
...
hrfExactReductionQ[scan_] := Module[{od, sl, obs},
  ...
  sl = od["Superleading"];
  obs = Lookup[od, "Obstruction", Missing["NoObstruction"]];
  TrueQ[Expand[sl] =!= 0] && ! MatchQ[obs, _Missing] && TrueQ[Expand[obs] === 0]
];

(* HRF reporting/scaling patch: use the unreduced Complement as the displayed/scaled
   superleading sector when it is the available valid representative. *)
...
(* Successful obstruction decomposition (PDF): F_0^restricted = Obstruction + F_SL
   with nonzero F_SL and PolynomialReduce[F_SL, generators] remainder == 0 exactly. *)
...
hrfCoverageData[scan_, U_, activeVars_, maxAbs_:5, fObs_:Automatic] := Module[{cached, fsl, obs, scanObs},
  ...
  hrfNotebookTimed["findCoverageLPScaling vars=" <> ToString[Length[activeVars]],
    If[Length[DownValues[findExactCoverageLPScaling]] > 0,
      findExactCoverageLPScaling[fsl, Expand[U], activeVars, obs,
...
hrfHiddenSummaryRow[label_, configuration_, zeroVars_, activeVars_, scan_, scaling_:Automatic] := Module[
  ...
  (* Hidden region requires a successful obstruction decomposition and an accepted
     LP scaling vector.  The all-in-ideal case is a decomposition convention, not
     a substitute for the scaling test. *)
  hiddenQ = decompOK && hrfCoverageFoundQ[sc];
  ...
  <|
    ...
    "Comment" -> Which[
      ...
      hiddenQ && exactQ, "all-in-generator-ideal decomposition with accepted scaling",
