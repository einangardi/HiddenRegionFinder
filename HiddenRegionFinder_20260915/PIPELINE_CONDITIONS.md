...
flowchart TD
  ...
  OBS["5. Obstruction F_SL + candidate scaling"]
  CERT["6. Complete-LP lower-facet certification"]

  F0 --> RAW --> SAFE --> ELIG --> GEN --> OBS --> CERT
```
...
## Stage 6 — Complete-LP lower-facet certification

The scaling returned at Stage 5 is a candidate until it has been tested on
the complete LP polynomial.  In an ordinary non-composite limit the standard
complete-polynomial lower-facet/scalefulness audit applies directly.  After
asymptotic-order alignment, HRF uses the following order:

1. `HRF_IdealLayerCertification.wl` tests the occupied original-coordinate
   layers in the cancellation-ideal filtration.  A unique positive solution
   determines the candidate total vector and provides an independent exact
   check.
2. In every case, `HRF_SourceAwareDissectionCertification.wl` supplies the
   final geometric certificate.  It reconstructs four separate sources:
   `F_SL`, `F_Obs`, all remaining `F` layers, and `U`.
3. Independent cancellation factors are mapped to signed endpoint
   coordinates.  Equal local exponent rows are recombined exactly before the
   Newton support is formed.
4. `cddlib` enumerates exact lower facets.  A certificate requires full facet
   rank, a positive eta-oriented normal, positive suppression transverse to
   every selected cancellation factor, and `W_HR > W_SL`.
5. The local normal is pulled back through the exact dissection map to the
   original LP variables.  The output records inverse-chart, nonzero
   Jacobian, valuation, facet-rank, and hierarchy checks.

When the ideal-layer solution exists, its agreement with the dissected
pullback is recorded by `IdealLayerDissectionAgreementQ`.  Acceptance still
requires the source-aware local lower facet.

`ResolvedFSLWeight` is measured from the source-resolved transformed
polynomial.  HRF separately reports whether it equals the resolved leading
weight (`FSLOnLeadingFacetQ`) or is larger
(`FSLSubleadingAfterCancellationQ`).  No copy of an `F_SL` exponent row is
inserted at `W_HR` for acceptance.  The former promoted construction remains
available only as `SyntheticFSLPromotionFacetQ`, with
`SyntheticFSLPromotionUsedForAcceptanceQ -> False`.

Pivot, solve-branch, sign-sector, point-count, and time limits are explicit.
Reaching any of them sets `SearchTruncatedQ -> True`; failure to find a facet
in such a run is reported as unresolved and is not a scaleless rejection.
Likewise, an unsupported chart denominator or a failed external facet call
sets `SearchIncompleteQ -> True` rather than producing a negative result.

The source-aware certificate can return more than one exact local lower
facet for a single discovery presentation.  All are retained in
`SourceAwareDissectionCertification["Certificates"]`; the singular legacy
fields in `TotalScalingAudit` contain the first deterministic representative.
