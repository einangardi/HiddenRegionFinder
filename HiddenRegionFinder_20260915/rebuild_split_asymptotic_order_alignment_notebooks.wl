...
glauberAudit = Import[
  FileNameJoin[{resultsDirectory,
    "one_loop_NMRK_DSC_glauber_audit.wl"}], "WL"
];
...
nmrkGlauberGrid = Grid[
  Prepend[{
    {"parameter-space scalar power",
      glauberAudit["NMRK", "ParameterIntegralPower"]},
    {"propagator virtuality powers",
      glauberAudit["NMRK", "PropagatorVirtualityPowers"]},
    {"required one-loop measure",
      glauberAudit["NMRK", "RequiredLoopMeasurePower"]},
    {"local-width constraints",
      glauberAudit["NMRK", "LocalWidthConstraints"]},
    {"mode", glauberAudit["NMRK", "Mode"]},
    {"independent Glauber loops",
      glauberAudit["NMRK", "IndependentLoopCount"]}
...
nmrkNotebook = Notebook[{
  ...
  Cell["One-loop Glauber routing and power-counting audit", "Section"],
  Cell["The two positive factor equations are localized at the p1 and p2 vertices on the adjacent edge pairs (x1,x2) and (x4,x5).  They are the two attachment points of one t-channel loop, not two independent loops.  Matching the restricted parameter-space count to the six inverse-LP virtualities fixes c_perp=3/2 and a_plus+a_minus=4.  Since a_plus+a_minus-2 c_perp=1>0, the unique loop is transverse dominated and hence Glauber.  Positivity of the one-loop Landau solution forces opposite signs among the longitudinal edge components, providing the opposite-half-plane pole pinch.", "Text"],
  outputCell[nmrkGlauberGrid],
  inputCell["$HRFRunOneLoopNMRKDSCGlauberAuditOnLoad=True;\nGet[\"HRF_OneLoopNMRKDSCGlauberAudit.wl\"];\nKeyTake[OneLoopNMRKDSCGlauberAudit,{\"Status\",\"Checks\"}]"],
  Cell["Two-loop NMRK results", "Section"],
  Cell["The exhaustive generic-NMRK scans have no staged candidate.  The w=z candidates have now been rerun through the current full-native-F audit: the aligned face proposes the cancellation and scaling, but every occupied eta layer of the original polynomial is then restored and tested under the composed vector.  Full-support and one-edge-boundary regions are counted separately.  This final audit retains seven planar-hexbox regions (four full support and three boundary); four additional staged candidates fail after the omitted layers are restored and are not counted as hidden regions.", "Text"],
  ...
  Cell["A cancellation hypersurface is defined by non-monomial polynomial factors.  No x_i alone is treated as a cancellation factor.  The four full-support planar-hexbox vectors fall into three generator classes.  Rows 2 and 3 have the same product of the same two non-monomial linear factors (up to factor order and overall signs), but have different certified vectors.  Whether these are two genuinely distinct local facets of one singular hypersurface or an overcount remains to be established by a common local-coordinate/dissection analysis.", "Text"],
...
dscGlauberGrid = Grid[
  Prepend[{
    {"parameter-space scalar power",
      glauberAudit["DSC", "ParameterIntegralPower"]},
    {"propagator virtuality powers",
      glauberAudit["DSC", "PropagatorVirtualityPowers"]},
    {"required one-loop measure",
      glauberAudit["DSC", "RequiredLoopMeasurePower"]},
    {"local-width constraints",
      glauberAudit["DSC", "LocalWidthConstraints"]},
    {"mode", glauberAudit["DSC", "Mode"]},
    {"independent Glauber loops",
      glauberAudit["DSC", "IndependentLoopCount"]}
  }, {"Quantity", "Certified value"}],
  Frame -> All, Alignment -> Left
];
...
dscNotebook = Notebook[{
  ...
  Cell["One-loop Glauber routing and power-counting audit", "Section"],
  Cell["The same adjacent edge pairs (x1,x2) and (x4,x5) localize the two positive factor equations at the p1 and p2 vertices.  The loop count is nevertheless one.  The restricted parameter-space measure restores two powers, and matching it to the inverse-LP propagator virtualities fixes c_perp=1 and a_plus+a_minus=4.  Thus a_plus+a_minus-2 c_perp=2>0: the unique t-channel loop is Glauber, with a different transverse width from the NMRK w=z region.  The positive one-loop Landau equations ensure the corresponding plus and minus poles approach from opposite half-planes.", "Text"],
  outputCell[dscGlauberGrid],
  inputCell["$HRFRunOneLoopNMRKDSCGlauberAuditOnLoad=True;\nGet[\"HRF_OneLoopNMRKDSCGlauberAudit.wl\"];\nKeyTake[OneLoopNMRKDSCGlauberAudit,{\"Status\",\"Checks\"}]"],
  Cell["Two-loop structural audit", "Section"],
  Cell["These certificates already test every occupied layer of the native DSC polynomial after composing alignment with the HR scaling.  For the planar hexbox the weight -4 and -3 layers vanish on the common pinch and the weight -2 layer is the first nonzero lower facet.  The restored-layer correction therefore does not change the DSC conclusion.", "Text"],
