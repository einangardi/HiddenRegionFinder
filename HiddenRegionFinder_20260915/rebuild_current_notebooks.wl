...
hrfCurExport[
  ...
  "Current wide-angle 4-point examples",
  {
    ...
    hrfCurText["This focused constructive check restores the SuperCrown example without attempting a full boundary survey. It is an ordinary wide-angle HRF run: no asymptotic-order alignment is performed. On B={x8=0,x9=0}, HRF starts from the native restricted F0, finds the two Crown generators, constructs FSL with zero obstruction, and only then solves for one exact region vector. Here FSL is the Crown superleading polynomial multiplied by x10 x11. Boundary equations x_e=0 and scaling components v_e are reported separately."],
    hrfCurInput[
    ...
    hrfCurText["This is the current positive HyperCrown boundary example. It is an ordinary wide-angle HRF run on the native restricted F0, with no asymptotic-order alignment. The WideAngle4ptBoundary preset first identifies cancellation generators and the FSL+FObs decomposition; exact coverage then determines the scaling."],
    hrfCurInput[
