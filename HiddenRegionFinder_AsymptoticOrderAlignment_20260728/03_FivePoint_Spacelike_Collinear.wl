...
scalingForBoundaryCase[case_, UFCollinearData_] := Module[
  {graphIndex, scan, obsData, obstruction, fsl, fslSource, U, vars,
   fColRestricted, postLayers, newScaling},
  ...
  fColRestricted = UFCollinearData[[graphIndex, "FCollinear"]] /. Thread[case["ZeroVars"] -> 0] // Expand;
  postLayers = hrfPostCancellationLayerInput[obstruction, fColRestricted, \[Delta]];
  vars = case["RemainingVars"];
  newScaling = If[MatchQ[fsl, _Missing] || TrueQ[Expand[fsl] === 0],
    Missing["NoNonzeroSuperleadingSector"],
    (* Restore the full F_delta tower for the scaling test.  The obstruction
       search itself still uses F0, but the accepted region is tested against
       U + F_obs + F_delta with augmented weights (r_i; a_i). *)
    findCoverageLPScaling[fsl, U, vars, 5, postLayers]
  ];
  <|
    "GraphIndex" -> graphIndex,
...
scalingForInteriorCase[data_] := Module[
  {scan, obsData, obstruction, fsl, fslSource, U, vars, postLayers, newScaling},
  ...
  U = data["U"];
  vars = data["Variables"];
  postLayers = hrfPostCancellationLayerInput[obstruction, data["FCollinear"], \[Delta]];
  newScaling = If[MatchQ[fsl, _Missing] || TrueQ[Expand[fsl] === 0],
    Missing["NoNonzeroSuperleadingSector"],
    (* Restore the full F_delta tower for the scaling test.  The obstruction
       search itself still uses F0, but the accepted region is tested against
