...
ClearAll[hrfSmokeAssert, hrfSmokeRow, hrfSmokeScalingFoundStatusQ, hrfRunNotebookSmokeTests];
...
hrfSmokeScalingFoundStatusQ[status_] := Module[{s = ToString[status]},
  MemberQ[{"Scaling vector determined", "Found",
    "Exact face-enumerated rational coverage scaling found",
    "Exact layered lower-face coverage scaling found"}, status] ||
    StringContainsQ[s, "Exact face-enumerated rational coverage scaling found"] ||
    StringContainsQ[s, "Exact layered lower-face coverage scaling found"]
];
...
hrfRunNotebookSmokeTests[] := Module[{rows = {}, row, sc, case},
  ...
  Block[{
    ...
    (* A smoke test needs one certified Crown presentation, not an exhaustive
       enumeration of the uncapped exploratory generator pool.  Completeness
       is covered by HRF_PolynomialFactorRegressionTests and the face audit. *)
    ...
    (* The notebook's on-load switch intentionally runs all four diagrams in
       all channels (16 scans).  A smoke test needs only the Crown in the two
       regimes asserted below. *)
    Ex02InteriorStudies = <|"Crown" -> Block[{hrfReggeChannels},
      hrfReggeChannels[] := {"T23"};
      hrfEx02InteriorStudy["Crown", Ex02Diagrams["Crown"], 20]
    ]|>;
