...
(* The historical signed-monomial-pair harvest is retained only as a legacy
   diagnostic.  An arbitrary two-term sub-sum of a derivative/channel
   polynomial is not itself a polynomial factor and therefore does not define
   a cancellation hypersurface.  The complete factor harvest below instead
   factorises every full primitive channel polynomial. *)
...
If[! ValueQ[$HRFPolynomialEnableSignedMonomialPairs], $HRFPolynomialEnableSignedMonomialPairs = False];
...
ClearAll[
  ...
  hrfCompleteChannelPolynomialFactors,
...
hrfNormalizeCancellationCandidates[cands_List, vars_, kinVars_: {}] := Module[{out, clean},
  out = hrfCanonicalCancellationFactor[#, vars, kinVars] & /@ cands;
  clean = Select[out, ! TrueQ[# === 0] &];
  (* Every element of clean has already had monomial content, non-x shells,
     overall sign and factor form canonicalized.  Re-running that full
     canonicalization for every pair was quadratic and dominated uncapped
     harvesting.  Expanded canonical polynomials are exact SameQ keys. *)
  DeleteDuplicatesBy[clean, Expand]
];
...
hrfSignedMonomialPairFactors[deriv_, vars_] := Module[
  {terms, harvested, i, j, sum, diff},
  terms = MonomialList[Expand[deriv], vars];
  harvested = Reap[
    For[i = 1, i <= Length[terms] - 1, i++,
      For[j = i + 1, j <= Length[terms], j++,
        sum = hrfPrimitiveCancellationFactor[
          Expand[terms[[i]] + terms[[j]]], vars
        ];
        diff = hrfPrimitiveCancellationFactor[
          Expand[terms[[i]] - terms[[j]]], vars
        ];
        If[hrfPolynomialFactorQ[sum, vars] && hrfMixedSignQ[sum, vars], Sow[sum]];
        If[hrfPolynomialFactorQ[diff, vars] && hrfMixedSignQ[diff, vars], Sow[diff]];
      ]
    ]
  ][[2]];
  If[harvested === {}, {}, DeleteDuplicates[First[harvested]]]
];
...
(* Factor the complete coefficient polynomials in every primitive kinematic
   channel direction.  For two channels this includes A1, A2 and A1 +/- A2.
   Unlike signed monomial sub-pairs, every returned object is an actual factor
   of a complete polynomial whose cancellation can be tested consistently. *)
hrfCompleteChannelPolynomialFactors[F_, vars_, kinVars_] := Module[
  {coefficients, directions, polynomials, factors},
  If[kinVars === {}, Return[{}]];
  coefficients = Expand[Coefficient[F, #]] & /@ kinVars;
  directions = primitiveChannelDirections[Length[kinVars]];
  polynomials = DeleteCases[
    DeleteDuplicates[Expand[Total[# coefficients]] & /@ directions], 0];
  factors = Flatten[
    Select[FactorList[Factor[#]][[All, 1]],
      # =!= 1 && # =!= -1 && ! monomialQ[#, vars] &] & /@ polynomials,
    1
  ];
...
hrfCancellationFactorAcceptanceQ[f_, vars_, kinAssumptions_, kinVars_, mode_String] := Module[
  {shapeQ, kinDomainQ, core, beforeDomainUnresolved, exactPositiveZeroQ},
  ...
  (* Apply exact algebraic gates before the potentially expensive semialgebraic
     domain test.  A factor rejected here cannot become a generator in this
     mode, so asking FindInstance about it is unnecessary. *)
  ...
  (* In the massless wide-angle problem, a kin-free homogeneous square-free
     mixed-sign factor has an exact positive zero: every 0/1 exponent is a
     Newton-polytope vertex, so opposite-sign monomials can be made dominant
     along two positive rays.  Use the existing generator-physics certificate
     here, before the semialgebraic fallback. *)
  exactPositiveZeroQ = Length[
      DownValues[hrfSquareFreeHomogeneousPositiveZeroQ]] > 0 &&
    TrueQ[hrfSquareFreeHomogeneousPositiveZeroQ[f, vars, kinVars]];
  beforeDomainUnresolved = If[ValueQ[$HRFKinDomainUnresolvedCount],
    $HRFKinDomainUnresolvedCount, 0];
  kinDomainQ = If[exactPositiveZeroQ, True,
    hrfPolynomialKinDomainQ[f, vars, kinAssumptions, kinVars]];
  If[! TrueQ[kinDomainQ],
    Return[<|"AcceptQ" -> False, "PositiveZeroMethod" -> If[
        ValueQ[$HRFKinDomainUnresolvedCount] &&
          $HRFKinDomainUnresolvedCount > beforeDomainUnresolved,
        "UnresolvedFindInstance", "FindInstanceNoSolution"],
      "RejectReason" -> If[ValueQ[$HRFKinDomainUnresolvedCount] &&
          $HRFKinDomainUnresolvedCount > beforeDomainUnresolved,
        "kinematic-domain decision unresolved within time limit",
        "fails kinematic-domain compatibility (no positive x_i solution in domain)"]|>]
  ];
  <|"AcceptQ" -> True,
    "PositiveZeroMethod" -> If[exactPositiveZeroQ,
      "ExactSquareFreeNewtonVertexCertificate", "FindInstanceWitness"],
    "RejectReason" -> "accepted"|>
];

hrfRawPolynomialCandidates[F_, vars_, kinAssumptions_, kinVars_, dimensionfulSpec_: Automatic] := Module[
  {byDeriv, fromFactors, fromWhole = {}, fromChannel = {}, fromPairs = {}, deriv, v,
   derivOnlyQ, pairDerivativePolynomials, channelCoefficients,
   channelDirections},
  (* Automatic is resolved from the raw pool of the current F0.  Reset the
     cached effective value before harvesting so a preceding stratum with
     shorter factors cannot impose a stateful cap on this one. *)
  $HRFPolynomialEffectiveMaxMonomials = Automatic;
  ...
  If[! derivOnlyQ,
    ...
    fromChannel = hrfCompleteChannelPolynomialFactors[F, vars, kinVars];
    If[TrueQ[$HRFPolynomialEnableSignedMonomialPairs],
      (* Channel derivatives are essential in the wide-angle problem: after a
         boundary restriction they can expose kin-free cancellation factors
         that are not visible in any single x_i derivative. *)
      pairDerivativePolynomials = If[
        hrfResolveKinFreeCancellationFactorsQ[kinVars] && kinVars =!= {},
        (* Wide-angle F = Sum_a s_a A_a(x) is multilinear in x.  Harvest
           signed pairs from every primitive channel direction, not only the
           displayed coefficient polynomials A_a.  For four points this adds
           A_1 +/- A_2 and makes factor discovery consistent with the crossed
           Mandelstam bases already used by the obstruction reconstruction.

           A same-channel kin-free pair from D_xi F is redundant: its parent
           monomials share x_i, which primitive normalization removes, so the
           identical pair occurs in the corresponding channel polynomial.
           Cross-channel D_xi pairs retain Mandelstam variables and fail the
           mandatory kin-free gate. *)
        channelCoefficients = Expand[Coefficient[F, #]] & /@ kinVars;
        channelDirections = primitiveChannelDirections[Length[kinVars]];
        DeleteCases[
          DeleteDuplicates[
            Expand[Total[# channelCoefficients]] & /@ channelDirections
          ],
          0
        ],
        Expand[D[F, #]] & /@ DeleteDuplicates[Join[vars, kinVars]]
      ];
      Do[
        deriv = pairDerivativePolynomials[[v]];
        fromPairs = Join[fromPairs, hrfSignedMonomialPairFactors[deriv, vars]],
        {v, Length[pairDerivativePolynomials]}
      ];
    ];
...
hrfFilterPolynomialCandidates[raw_List, vars_, kinAssumptions_, kinVars_, mode_String] := Module[
  {passed = {}, rejected = {}, auditRows = {}, row, acc, kinFreeQ, beforeUnresolved},
  hrfEnsurePolynomialMaxMonomialsFromRaw[raw, vars];
  kinFreeQ = hrfResolveKinFreeCancellationFactorsQ[kinVars];
  Do[
    beforeUnresolved = If[ValueQ[$HRFKinDomainUnresolvedCount],
      $HRFKinDomainUnresolvedCount, 0];
    ...
    row = <|
      ...
      "KinematicDomainDecisionUnresolvedQ" ->
        (If[ValueQ[$HRFKinDomainUnresolvedCount],
          $HRFKinDomainUnresolvedCount, 0] > beforeUnresolved),
      "PositiveZeroMethod" -> Lookup[acc, "PositiveZeroMethod", "NotRequired"],
...
hrfBinomialOnlySafeFactorsExtended[F_, vars_, kinAssumptions_, kinVars_, dimensionfulSpec_: Automatic] := Module[
  ...
  $HRFPolynomialLastFactorAudit = <|
    ...
    "SourceCounts" -> <|
      ...
      "CompleteChannelPolynomials" -> Length[pack["FromCompleteChannelPolynomials"]],
